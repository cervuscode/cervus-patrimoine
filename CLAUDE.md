...
- **Champ deal "Date simulation"** (type Date, résolu dynamiquement par nom) : écrit **à la création du deal** au format `YYYY-MM-DD` (date locale Europe/Paris, sans heure — `todaySimulationDate()`). Sert d'**ancre aux relances J+1/J+7/J+21**. **Set seulement si absent** : sur un deal existant déjà daté, la clé est retirée du payload de mise à jour (`stripDateSimIfAlreadySet`) pour ne jamais réinitialiser l'ancre (early-contact → submit, ou simu rejouée un autre jour). Présent sur PER et AV.
