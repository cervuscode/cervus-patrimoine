...
TESTCOMP = IRNET3 + AVRICIIR3 + TAXANET3 + PCAPNET3 + HAUTREVNET3 + CDHRNET3+ max(0 , CSGPAT) + max(0 , CRDSPAT) + max(0 , PRELPSOL) + PREVAUTSOC 
...
NAPTIR2 = IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM - IRESTITIR - RASSALIR - RASACOIR + RASCTXIR + AVRICIIR + CIADCREB3 ;

NAPTIRNET = IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM - IRESTITIR - RASSALIR - RASACOIR + RASCTXIR + AVRICIIR - IMPETAL + IMPETAL5 + CIADCREB3 ;
...
CONDCOMP = positif(NAPTIRNET) * ((1 - null(PREVSOCNET)) + null(PREVSOCNET) * null(2 - IND12));
...
AVRICIIR3 = max(0 , AVRICIIR + CIADCREB3 - max(0 , - PASIR - IRNETBA - (TAXANET + PCAPNET + HAUTREVNET + CDHRNET) * positif(NAPTIR1))) * CONDCOMP ;
...
COMPENSPAS = min(abs(PASIR - V_PASIRANT + V_PASIRNANT) , min(PREVSOCNET - V_PREVSOCANT + V_PREVSOCNANT - COMPENSACI , abs(NAPTIRNET - V_ANTIR + V_ANTREIR)))
              ...
              * (1 - positif(NAPTIRNET - V_ANTIR + V_ANTREIR)) * positif(20 - V_NOTRAIT) 
...
NAPTIR = IRNET + TAXANET + PCAPNET + HAUTREVNET + CDHRCUM - IRESTITIR ;

NAPTIR61 = NAPTIR * positif_ou_nul(IAMD1 - COD8EA - COD8EB - SEUIL_61) ; 

NAPTIR1 = IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM - IRESTITIR ; 
...
NAPI = IRD + PIRD - IRANT + TAXASSUR + IPCAPTAXT + IHAUTREVT + CDHR + CHRPVIMP + BRASAR ;
...
BINRIR = (1-V_CNR) * (max( 0 ,IRN-IRANT)
          + max(0,TAXASSUR- min(TAXASSUR+0,max(0,INE-IRB+AVFISCOPTER))+min(0,IRN - IRANT))
           ...
           + max(0,CDHRACI+min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT+IHAUTREVT+CHRPVIMP))
	   )
       + (V_CNR+0)* (max( 0 ,IRN-IRANT)
          + max(0,TAXASSUR- min(TAXASSUR+0,max(0,INE-IRB+AVFISCOPTER))+min(0,IRN - IRANT))
           ...
           + max(0,CDHRACI+min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT+IHAUTREVT+CHRPVIMP))) * INDTXMIN 
	        * positif_ou_nul(max( 0 ,IRN-IRANT)
                               + max(0,TAXASSUR- min(TAXASSUR+0,max(0,INE-IRB+AVFISCOPTER))+min(0,IRN - IRANT))
                               + max(0,IPCAPTAXT- min(IPCAPTAXT+0,max(0,INE-IRB+AVFISCOPTER-TAXASSUR))
                                                                             +min(0,IRN - IRANT+TAXASSUR))
                               + max(0,IHAUTREVT+CHRPVIMP+min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT))
                               + max(0,CDHRACI+min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT+IHAUTREVT+CHRPVIMP))- SEUIL_TXMIN)
       ;
...
BINRIRIR = max( 0 ,IRN-IRANT);
BINRIRTAXA = max(0,TAXASSUR- min(TAXASSUR+0,max(0,INE-IRB+AVFISCOPTER))+min(0,IRN - IRANT));
BINRIRCAP = max(0,IPCAPTAXT- min(IPCAPTAXT+0,max(0,INE-IRB+AVFISCOPTER-TAXASSUR))
                       +min(0,IRN - IRANT+TAXASSUR));
BINRIRCHR =  max(0,IHAUTREVT+CHRPVIMP+min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT));
BINRIRCDHR =  max(0,CDHRACI+min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT+IHAUTREVT+CHRPVIMP));
BINRPSCS = max(0,CSGC-CICSG-CSGIM);
BINRPSRD =  max(0,RDSC-CIRDS-CRDSIM);
BINRPSPS = max(0,MPSOL-CIPSOL-PRSPROV);
BINRPSCVN = max(0,CVNN - COD8YT);
BINRPSCDIS = max(0,CDIS - CDISPROV);
BINRPSGLO = max(0,CGLOA-CIGLOA-COD8YL);
BINRPSRSE1 = max(0,RSE1N-CSPROVYD);
BINRPSRSE2 = max(0,RSE2N-CSPROVRSE2);
BINRPSRSE3 = max(0,RSE3N-CSPROVYG);
BINRPSRSE4 = max(0,RSE4N-CSPROVRSE4);
BINRPSRSE5 = max(0,RSE5N-CSPROVYE);
BINRPSRSE6 = max(0,RSE6N-COD8YQ);
BINRPSC820 = max(0,MCSG820-COD8ZH);
BINRPSRSE8 = max(0,RSE8N);
regle 101083:
application : iliad ;

SOMPASIR =  arr((max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
         + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY))
              * max( 0 ,IRN-IRANT) / BINRIR ) * positif(BINRIR-BINRIRIR)
                   +
                (max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
              + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY)) * (1-positif(BINRIR-BINRIRIR))
               ;
SOMPASTAXA =  arr((max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
                + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY))
              * max(0,TAXASSUR- min(TAXASSUR+0,max(0,INE-IRB+AVFISCOPTER))+min(0,IRN - IRANT))/BINRIR) * positif(BINRIR-BINRIRIR-BINRIRTAXA) +0
                   +
                 max(0,max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
              + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY)-SOMPASIR) * (1-positif(BINRIR-BINRIRIR-BINRIRTAXA))
               ;
SOMPASCAP =  arr((max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
                + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY))
              * max(0,IPCAPTAXT- min(IPCAPTAXT+0,max(0,INE-IRB+AVFISCOPTER-TAXASSUR))
                                                   +min(0,IRN - IRANT+TAXASSUR))/BINRIR) * positif(BINRIR-BINRIRIR-BINRIRTAXA-BINRIRCAP) +0
                +
              max(0,max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
                  + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY)-SOMPASIR-SOMPASTAXA) * (1-positif(BINRIR-BINRIRIR-BINRIRTAXA-BINRIRCAP))
             ;
SOMPASCHR = max(0,max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
                  + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY)-SOMPASIR-SOMPASTAXA-SOMPASCAP)
           ;
SOMPASCDHR = max(0,max(0,COD8HV+COD8HW-COD8HY)+max(0,COD8IV + COD8IW-COD8IY)+max(0,COD8JV +COD8JW-COD8JY)
                  + max(0,COD8KV +COD8KW -COD8KY) + max(0, COD8LV +COD8LW-COD8LY) + max(0,COD8MV +COD8MW-COD8MY)-SOMPASIR-SOMPASTAXA-SOMPASCAP-SOMPASCHR)
           ;

SOMPASCS = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                   * max(0,CSGC-CICSG-CSGIM) / BINRPS) * positif(BINRPS-BINRPSCS)
                     + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)) * (1-positif(BINRPS-BINRPSCS))
              ;
SOMPASRD = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                    * max(0,RDSC-CIRDS-CRDSIM) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD)+0
                       + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)-SOMPASCS) * (1-positif(BINRPS-BINRPSCS-BINRPSRD))
         ;
SOMPASPS = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                  * max(0,MPSOL-CIPSOL-PRSPROV) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS)+0
                   + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)-SOMPASCS-SOMPASRD)
                                                          * (1-positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS))
             ;
SOMPASCVN = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                    * max(0,CVNN - COD8YT)  / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN)+0
                       + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)-SOMPASCS-SOMPASRD-SOMPASPS)
                                                   * (1-positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN))
              ;
SOMPASCDIS = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                 * max(0,CDIS - CDISPROV) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS)+0
                  + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)-SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN)
                                               * (1-positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS))
         ;
SOMPASGLO = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                  * max(0,CGLOA-CIGLOA-COD8YL) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO)+0
                  + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)-SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS)
                                            * (1-positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO))
       ;
SOMPASC820 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                   * max(0,MCSG820-COD8ZH) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820)+0
                    + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)-SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO)
                                                  * (1-positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820))
           ;
SOMPASRSE1 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                 * max(0,RSE1N-CSPROVYD) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1)+0
                     + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                              -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820)
                                * (1-positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1))
            ;
SOMPASRSE2 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                 * max(0,RSE2N-CSPROVRSE2) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2)+0
                  + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                              -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820-SOMPASRSE1)
                                                   * (1-positif(BINRPS-BINRPSCS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2))
             ;
SOMPASRSE3 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                      * max(0,RSE3N-CSPROVYG) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3)+0
                       + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                                 -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820-SOMPASRSE1-SOMPASRSE2)
                                            * (1-positif(BINRPS-BINRPSCS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3))
         ;
SOMPASRSE4 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                   * max(0,RSE4N-CSPROVRSE4) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3-BINRPSRSE4)+0
                    + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                              -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820-SOMPASRSE1-SOMPASRSE2-SOMPASRSE3)
                                              * (1-positif(BINRPS-BINRPSCS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3-BINRPSRSE4))
              ;
SOMPASRSE5 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                        * max(0,RSE5N-CSPROVYE)  / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3-BINRPSRSE4-BINRPSRSE5)+0
                 + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                             -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820-SOMPASRSE1-SOMPASRSE2-SOMPASRSE3-SOMPASRSE4)
                                         * (1-positif(BINRPS-BINRPSCS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3-BINRPSRSE4-BINRPSRSE5))
         ;
SOMPASRSE6 = arr((max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ))
                  * max(0,RSE6N-COD8YQ) / BINRPS)* positif(BINRPS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3-BINRPSRSE4-BINRPSRSE5-BINRPSRSE6)+0
                      + (max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                     -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820-SOMPASRSE1-SOMPASRSE2-SOMPASRSE3-SOMPASRSE4-SOMPASRSE5)
                                       * (1-positif(BINRPS-BINRPSCS-BINRPSCS-BINRPSRD-BINRPSPS-BINRPSCVN-BINRPSCDIS-BINRPSGLO-BINRPSC820-BINRPSRSE1-BINRPSRSE2-BINRPSRSE3-BINRPSRSE4-BINRPSRSE5-BINRPSRSE6))
         ;
SOMPASRSE8 = max(0,(max(0,COD8HX-COD8HZ)+max(0,COD8IX-COD8IZ)+max(0,COD8JX-COD8JZ)+max(0,COD8KX-COD8KZ)+max(0,COD8LX-COD8LZ)+max(0,COD8MX-COD8MZ)
                     -SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASGLO-SOMPASC820-SOMPASRSE1-SOMPASRSE2-SOMPASRSE3-SOMPASRSE4-SOMPASRSE5-SOMPASRSE6))
          ;
...
DIFFSOMPASIR= max(0,max(SOMPASIR_A+SOMPASTAXA_A+ SOMPASCAP_A+SOMPASCHR_A+SOMPASCDHR_A,SOMPASIR_P+SOMPASIR_PA+SOMPASTAXA_P+SOMPASTAXA_PA+SOMPASCAP_P+SOMPASCAP_PA+SOMPASCHR_P+SOMPASCHR_PA+SOMPASCDHR_P+SOMPASCDHR_PA)
                      -SOMPASIR- SOMPASTAXA- SOMPASCAP- SOMPASCHR-SOMPASCDHR);
DIFFSOMPASCS=   max(0,max(SOMPASCS_A+SOMPASRD_A+SOMPASPS_A+SOMPASCVN_A+SOMPASCDIS_A+SOMPASC820_A+SOMPASGLO_A+SOMPASRSE1_A+SOMPASRSE2_A
                     +SOMPASRSE3_A+SOMPASRSE4_A+SOMPASRSE5_A +SOMPASRSE6_A+SOMPASRSE8_A,SOMPASCS_P+SOMPASCS_PA+SOMPASRD_P+SOMPASRD_PA+SOMPASPS_P+SOMPASPS_PA+SOMPASCVN_P+SOMPASCVN_PA
                     +SOMPASCDIS_P+SOMPASCDIS_PA+SOMPASC820_P+SOMPASC820_PA+SOMPASGLO_P+SOMPASGLO_PA+SOMPASRSE1_P+SOMPASRSE1_PA+SOMPASRSE2_P+SOMPASRSE2_PA
                     +SOMPASRSE3_P+SOMPASRSE3_PA+SOMPASRSE4_P+SOMPASRSE4_PA+SOMPASRSE5_P+SOMPASRSE5_PA+SOMPASRSE6_P+SOMPASRSE6_PA+SOMPASRSE8_P+SOMPASRSE8_PA)
                     - SOMPASCS-SOMPASRD-SOMPASPS-SOMPASCVN-SOMPASCDIS-SOMPASC820-SOMPASGLO-SOMPASRSE1-SOMPASRSE2-SOMPASRSE3-SOMPASRSE4-SOMPASRSE5-SOMPASRSE6-SOMPASRSE8);
regle 101087:
application : iliad ;

SOM9YI= (SOM9YIINTER + 0) * positif(null(1 - IND_PASSAGE) + positif_ou_nul(20 - V_NOTRAIT));
VAR9YIIR= arr(SOM9YI * BINRIR/(BINRIR+BINRPS)) * positif(null(1 - IND_PASSAGE) + positif_ou_nul(20 - V_NOTRAIT));
VAR9YIPS = max(0,SOM9YI - VAR9YIIR) * positif(null(1 - IND_PASSAGE) + positif_ou_nul(20 - V_NOTRAIT)) * positif(BINRPS);
...
CHR9YI = positif(CDHRACI) * arr(VAR9YIIR * max(0,CDHRACI- min(CDHRACI+0,max(0,INE-IRB+AVFISCOPTER-TAXASSUR-IPCAPTAXT))
                          +min(0,IRN - IRANT+TAXASSUR+IPCAPTAXT))/BINRIR)
         + (1 - positif(CDHRACI)) * max(0,VAR9YIIR - IR9YI - TAXA9YI-CAP9YI);

CDHR9YI = max(0 , VAR9YIIR - IR9YI - TAXA9YI - CAP9YI - CHR9YI) ;
...
IAMD28EA = IAD11 + IBATMARG + ITP + PVMTS + REI + AVFISCOPTER + AUTOVERSSUP + IMPETAL19 + IMPETAL20 + IMPETAL21 + IMPETAL22 + IMPETAL23 + COD8UA + COD8UB ;
...
BTO = max(0 , IRN - IR9YI - IRANT + CODZRA) * (positif(IAMD1 + 1 - SEUIL_61) * (1 - positif(V_CNR2)) + positif(IAMD1 + 1 - SEUIL_TXMIN) * positif(V_CNR2)) ;
...
RSE2NET = max(0,(RSE2 + PRSE2 - CIRSE2 -  CSPROVYF - CSPROVYN))  ;	
...
BRSE8QV = COD8QV * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;


RSE2 = (arr(BRSE8TV * TXTV/100) + arr(BRSE8QV * TXTV/100) + arr(BRSE8SA * TXTV/100)) * (1 - positif(ANNUL2042)) ;
...
RDRCM1 = max(0 , TRCMABD + DRTNC + RCMAV + PROVIE + RCMNAB + RTCAR + REGPRIV + RESTUC
                 ...
                 + COD2TQ + COD2TT + COD2TZ - RCMRDS - RCMSOC - COD2DF - COD2DG) ;
...
RDRCM1APS = max(0 , RCMABD + RCMTNC + RCMAV + RCMHAD + RCMHAB + REGPRIV
	            ...
	            + RCMIMPAT + COD2TQ + CODRYY +  COD2TT + COD2TZ - RCMSOC - RCMRDS
	            - COD2DF - COD2DG) ;
...
PVTAUXPS = BPV18V + BPCOPTV + BPV40V + BPCOSAV + BPCOSAC + BPVSJ + BPVSK + COD3PI + COD3AN * (1-positif(COD3CN)) + COD3XN * positif(COD3WS) ;
...
BRSE2 = (ALLECS + COD8QV + COD8SA) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BRSE3 = (INDECS + COD8SW) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BRSE4 = (PENECS + COD8QX + COD8SB + COD8SX) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BRSE5 = (SALECSG + COD8SC)  * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BRSE6 = (COD8SC + COD8SA + ALLECS + COD8QV + COD8TH + COD8QH + COD8SD) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BRSE8 = (COD8TH + COD8QH + COD8SD) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;
...
BDCSG = (min (BCSG , max (0, RDRV + positif(COD2OP)* (max(0,RDRCM1+ RDRCM2)+ PVBARPS)  + RDRFPS + RDNP +(1-positif(COD2OP))*COD3XN * (1-positif(COD3WS)) - IPPNCS - 8SGAUTO - REVNONASSU + (1-positif(COD2OP))*(min(COD8RC,RDRCM1)+min(COD8RM,RDNCP) + COD3AN * positif(COD3CN)))) 
...
BDCSG3VAVB = max(0, RDRV + positif(COD2OP)* max(0,RDRCM1 + RDRCM2) + RDRFPS + RDNP + PVBAR3VAVB + PVTZBIS + (1-positif(COD2OP)) * COD3XN * (1-positif(COD3WS)) - IPPNCS - 8SGAUTO - REVNONASSU+ (1-positif(COD2OP)) * min(COD8RC,RDRCM1) + min(COD8RM,RDNCP) ) * (1-V_CNR) * (1 - positif(present(RE168)+present(TAX1649))) ;

regle 101260:
application : iliad ;


BDRSE1 = arr(max(0,SALECS - min( SALECS,REVCSXA ) - arr(CSPROVYD/(TX092/100))
            )) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;


BDRSE28TV = arr(max(0,(ALLECS - min(REVCSXC , ALLECS * (ALLECS/(ALLECS + COD8QV))))))
             * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BDRSE28QV = arr(max(0,(COD8QV - min(REVCSXC , COD8QV * (COD8QV/(ALLECS + COD8QV))))))
             * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BDRSE2 = BDRSE28TV + BDRSE28QV ;	     

BDRSE3 = arr(max(0,INDECS + COD8SW - min( INDECS + COD8SW,REVCSXD ) - arr(CSPROVYG/(TXTW/100))
            )) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;


BDRSE48TX = arr(max(0,(PENECS - min(REVCSXE, PENECS * (PENECS/(PENECS + COD8QX))))))
             * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BDRSE48QX = arr(max(0,(COD8QX - min(REVCSXE, COD8QX * (COD8QX/(PENECS + COD8QX))))))
             * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BDRSE4 = BDRSE48TX + BDRSE48QX ;

BDRSE5 = arr(max(0,SALECSG + COD8SC - min( SALECSG + COD8SC,REVCSXB ) - arr(CSPROVYE/(TX092/100))
            )) * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;




BDRSE8TH = arr(max(0,(COD8TH - min(COD8XO, COD8TH * (COD8TH/(COD8TH + COD8QH))))))
             * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BDRSE8QH = arr(max(0,(COD8QH - min(COD8XO, COD8QH * (COD8QH/(COD8TH + COD8QH))))))
             * (1 - positif(present(RE168) + present(TAX1649))) * (1-V_CNR) ;

BDRSE8 = BDRSE8TH + BDRSE8QH ; 




DRSED = (arr(BDRSE1 * TXTQDED/100)  + arr(BDRSE3 * TXTWDED/100) + arr(BDRSE5 * TXTRDED/100)) * positif(CSREC + V_IDRSEANT) * positif(NAPC61) ;


DPSEDEC1 = (arr(BDRSE28TV * TXTVDED/100)  + arr(BDRSE48TX * TXTXDED/100) + arr(BDRSE8TH * TX042/100)) * positif(CSREC + V_IDDPSED1ANT) * positif(NAPC61) ;

DPSEDEC2 = (arr(BDRSE28QV * TXTVDED/100)  + arr(BDRSE48QX * TXTXDED/100) + arr(BDRSE8QH * TX042/100)) * positif(CSREC + V_IDDPSED2ANT) * positif(NAPC61) ;

DPSEDEC = DPSEDEC1 + DPSEDEC2 ;

IDRSE = DRSED * positif(CSREC) * null(V_IND_TRAIT - 4) + abs(DRSED - V_IDRSEANT) * (1 - null(V_IND_TRAIT - 4)) ;

IDRSE1 = DPSEDEC1 * positif(CSREC) * null(V_IND_TRAIT - 4) + abs(DPSEDEC1 - V_IDDPSED1ANT) * (1 - null(V_IND_TRAIT - 4)) ;

IDRSE2 = DPSEDEC2 * positif(CSREC) * null(V_IND_TRAIT - 4) + abs(DPSEDEC2 - V_IDDPSED2ANT) * (1 - null(V_IND_TRAIT - 4)) ;
...
REVREMP = SALECS + SALECSG + ALLECS + INDECS + PENECS + COD8SA + COD8SB + COD8SC + COD8SW + COD8SX + COD8TH + COD8SD + COD8QV + COD8QX + COD8QH ;
...
REG8SH8RP = positif(positif(COD8SH)* (1-positif(COD8SI)) + positif(COD8RP)*(1-positif(COD8RQ))) * positif(V_0AM + V_0AO) * (CVNSALAV + GLDGRATC) ; 
...
BCVNSAL = (1-positif(COD8SH +COD8SI + COD8RP + COD8RQ))* (CVNSALAV + GLDGRATV + GLDGRATC) * (1-positif(present(TAX1649)+present(RE168)))
	...
	+ positif(positif(COD8SH)* (1-positif(COD8SI + COD8RQ)) + positif(COD8RP)*(1-positif(COD8RQ + COD8SI))) * positif(V_0AM + V_0AO) * (CVNSALAV + GLDGRATC)
	+ positif(positif(COD8SI)* (1-positif(COD8SH + COD8RP)) + positif(COD8RQ)*(1-positif(COD8RP + COD8SH))) * positif(V_0AM + V_0AO) * (CVNSALAV + GLDGRATV)  
