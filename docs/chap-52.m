...
WTXMARJ = ((RB51)*(1-INDTXMOY) + (RB55 * INDTXMOY)) / (NBPT * null(PLAFQF) + (1 + BOOL_0AM + BOOL_0AZ * V_0AV) *null(1-PLAFQF)) ;
...
TXMARJ = VARTMP1 * (1 - positif (INDTXMIN))
	 * (1 - positif(present(NRBASE) + present(NRINET) + present(BASRET) + present(IMPRET))) ;
...
CFRIAHP = ARESTIMO + ALOGDOM + ADUFREP + APIREPAI
         + APIREPCI 
	 + AILMPF + AILMPG + AILMPH + AILMPI + AILMPJ 
	 + AILMJY + AILMJX + AILMJW + AILMJV 
         + ACODJTJU + ACODOU + ACODOY + ACODPZ;

CFRIRHP =  RRESTIMO + RLOGDOM + RDUFREP 
...
CFRIADEP = AHEBE ;
CFRIRDEP = RHEBE ;
CFRIAFOR = BFCPI + ACOMP + AFORET + ANOUV + ALOCENT + ALOGSOC + ACOLENT + ADOMSOC1 + AFIPDOM;
CFRIRFOR = RINNO + RCOMP + RFORET + RNOUV + RLOCENT + RLOGSOC + RCOLENT + RDOMSOC1 + RFIPDOM;
CFRIAVIE = ASURV;
CFRIRVIE = RSURV;
CFRIAAUTRE = AFIPC + ADIFAGRI + ASOCREPR + ASOUFIP + ARIRENOV ;
CFRIRAUTRE = RFIPC + RDIFAGRI + RSOCREPR + RSOUFIP + RRIRENOV ;
CFCIDIV = CIGARD + CISYND + CIADCRE +CIRCOTFOR + CIRFOR + CIFORET;
