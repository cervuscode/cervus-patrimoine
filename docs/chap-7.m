...
REVQTOTQHTPROV =(positif(COD2OP))*(CODRWB * null(1-FLAG_EXIT) + (CODRWB+CODRWA) * null(2 -FLAG_EXIT) + (max (0,CODRVG-CODRSG))+(max(0,CODRUA-CODRVA-CODRSL))-IMPU3VR2 +GLN3 
                                            ...
                                            +  2REVF+3REVF+RFRTENET3+RFSTENET3+4REVF+RMFQUONET*positif(RFDANT))
...
REVQTOT = positif(COD2OP)*(CODRWB * null(1-FLAG_EXIT) + (CODRWB+CODRWA) * null(2 -FLAG_EXIT) + (max (0,CODRVG-CODRSG))+(max(0,CODRUA-CODRVA-CODRSL))+somme(i=1..3 : REVi)+REV4 + REVALIM + REVF+REVTS+REVTSREMP+REVPRR+REVRVO+REVRCM+REVRF+RFRTENET3+RFSTENET3+RMFQUONET*positif(RFDANT))
                                        + (1-positif(COD2OP))*(CODRWB * null(1-FLAG_EXIT) + (CODRWB+CODRWA) * null(2 -FLAG_EXIT) + CODRVG+somme(i=1..3 : REVi)+REV4 + REVALIM + REVF+REVTS+REVTSREMP+REVPRR+REVRVO+REVRCM+REVRF+max(0,RFRTENET3-2REVF)+max(0,RFSTENET3-3REVF)+max(0,RMFQUONET*positif(RFDANT)-4REVF));
...
RASREV4V = BAQV +GLN4V+ (max(0,BNCNPQCV -max(0,DABNCNPV-BNCNPHQCV))+BNCPQCV)*(1-INDTEFF)+(max(0,TBNCNPQCV -max(0,TDABNCNPV-TBNCNPHQCV))+TBNCPQCV)* INDTEFF+ BINTAQV + BIPTAQV;
RASREV4C = BAQC+ GLN4C+ (max(0,BNCNPQCC -max(0,DABNCNPC-BNCNPHQCC))+BNCPQCC)*(1-INDTEFF)+(max(0,TBNCNPQCC -max(0,TDABNCNPC-TBNCNPHQCC))+TBNCPQCC)* INDTEFF+ BINTAQC  +BIPTAQC;
RASREV4P = BAQP+ (max(0,BNCNPQCP -max(0,DABNCNPP-BNCNPHQCP))+BNCPQCP)*(1-INDTEFF)+(max(0,TBNCNPQCP -max(0,TDABNCNPP-TBNCNPHQCP))+TBNCPQCP)* INDTEFF+ BINTAQP +BIPTAQP;
RASREV4 = RASREV4V + RASREV4C;
RASREV4HTV = BAQV+ GLN4V+ max(0,BNCNPQCV -max(0,DABNCNPV-BNCNPHQCV))+BNCPQCV+ BINTAQV +BIPTAQV;
RASREV4HTC = BAQC+ GLN4C+ max(0,BNCNPQCC -max(0,DABNCNPC-BNCNPHQCC))+BNCPQCC+ BINTAQC +BIPTAQC;
RASREV4HTP = BAQP+ max(0,BNCNPQCP -max(0,DABNCNPP-BNCNPHQCP))+BNCPQCP+ BINTAQP +BIPTAQP;
...
RASREVPAC =  somme(i=1..4: PENFi) * (1-INDTEFF) + somme(i=1..4: TPENFi)  * INDTEFF
	    ...
	    + (somme (i=1..4:TSNN2TSi)+TSNN2RCF+TSNN2RDF+TSNN2REF+TSNN2RFF+TSNN2RCG+TSNN2RDG+TSNN2RGG+TSNN2RFG) * (1-INDTEFF)
            ...
            + BAQCP + (max(0,BNCNPQCP -max(0,DABNCNPP-BNCNPHQCP))+BNCPQCP)*(1-INDTEFF)+(max(0,TBNCNPQCP -max(0,TDABNCNPP-TBNCNPHQCP))+TBNCPQCP)* INDTEFF+ BINTAQP +BIPTAQP;
RASREVQTOT = arr(CODRWB/2) * null(1-FLAG_EXIT) + arr((CODRWB+CODRWA)/2) * null(2 -FLAG_EXIT) 
             + arr((REVRVO + REVRCM + RASREVRF+max(0,RFRTENET3-2REVF)+max(0,RFSTENET3-3REVF)+max(0,RMFQUONET*positif(RFDANT)-4REVF))/2) * (1-positif(COD2OP)) 
	     + arr((max(0 , CODRVG-CODRSG) 
	     + max(0 , CODRUA-CODRVA-CODRSL) + REVRVO + REVRCM + RASREVRF+RFRTENET3+RFSTENET3+RMFQUONET*positif(RFDANT))/2) * positif(COD2OP)
             + RASREV4V
             + RASREV4C
	     + RASREVFV
	     + RASREVFC
...
RASREVQTOTHT = arr(CODRWB/2) * null(1-FLAG_EXIT) + arr((CODRWB+CODRWA)/2) * null(2 -FLAG_EXIT) 
               + arr((REVRVO+REVRCM+2REVF+3REVF+4REVF)/2) * (1-positif(COD2OP)) + arr((max(0 , CODRVG-CODRSG) + max(0 , CODRUA-CODRVA-CODRSL) + REVRVO + REVRCM + 2REVF+3REVF+4REVF)/2) * positif(COD2OP)
               + RASREV4HTV 
               + RASREV4HTC
	       + PENFV
	       + PENFC
...
GLRF1 = inf(TONEQUOHT * (2REVF) / TOTALQUOHT) ;
GLRF2 = inf(TONEQUOHT * (3REVF) / TOTALQUOHT) ;
...
RASGLRF1 = inf(RASTONEQUOHT * (2REVF+max(0,RFRTENET3-2REVF)) / RASTOTALQUOHT)/2;
RASGLRF2 = inf(RASTONEQUOHT * (3REVF+max(0,RFSTENET3-3REVF)) / RASTOTALQUOHT)/2;
...
TRASGLRF1 = inf(RASTONEQUO * (2REVF+RFRTENET3) / RASTOTALQUO)/2;
TRASGLRF2 = inf(RASTONEQUO * (3REVF+RFSTENET3) / RASTOTALQUO)/2;
...
TGLRF1  = inf(TONEQUO * (2REVF+max(0,RFRTENET3-2REVF)) / TOTALQUO) ;
TGLRF2  = inf(TONEQUO * (3REVF+max(0,RFSTENET3-3REVF)) / TOTALQUO) ;
...
GLTOTM = GL4M + GLRVGM + GLRUAM + GLRWBM + GLRWAM ;
...
GLRF1M = arr(TONEQUOMHT * (2REVF+RFRTENET3+2REVF_1731) / TOTALQUOHT) ;
GLRF2M = arr(TONEQUOMHT * (3REVF+RFSTENET3+3REVF_1731) / TOTALQUOHT) ;
...
TGLTOTM = TGL4M + TGLRVGM + TGLRUAM + TGLRWBM + TGLRWAM ;
...
TGLRF1M  = inf(TONEQUOM * (2REVF+RFRTENET3+2REVF_1731) / TOTALQUO) ;
TGLRF2M  = inf(TONEQUOM * (3REVF+RFSTENET3+3REVF_1731) / TOTALQUO) ;
...
RBL1 =   RBG2 - min( RBG2 , ( DDPA + DDFA + DEDIV + DED6DG + APERPV + APERPC + APERPP + NUREPAR )) * (1-positif(RE168 + TAX1649)) * ( 1 - V_CNR ) 
              - min( RBG2 , R6DRNET + V_8ZT + CODZRE + CODZRF) * V_CNR ;
...
SDC1 = max(0, DDCSG + DDPA + DDFA + DEDIV - APERPV - APERPC - APERPP - max(0,RBG1)) 
          * (1 - V_CNR)
          + max (0, R6DRNET + V_8ZT+CODZRE+CODZRF - max(0,RBG1)) * V_CNR ;

regle 701270:
application : bareme , iliad ;


SDCC = max(0, DDCSG + (DDPA + DDFA + DEDIV + DED6DG + APERPV + APERPC + APERPP + NUREPAR) * (1 - V_CNR) + (R6DRNET + V_8ZT + CODZRE + CODZRF) * V_CNR - max(0 , RBG1)) ;
...
PVBAR3VG = (positif(COD2OP)*( BTPM3VG + PVTAXSB + BTPM3UA + BTPM3TJ +COD3SZ+GAINPEA * positif(COD2OP))) + positif(COD3CN) * COD3AN;
...
PVBARPS = max(0,(BPVRCM + COD3UA + ABIMPPV + COD3TJ + CODRVG + CODRUA + COD3XN * (1-(positif(COD3WS+0))) + GAINPEA + positif(COD3CN)*COD3AN)-ABIMPMV)+PVTITRESOC ; 

PVTZPS =  COD1TZ + COD1UZ + COD1VZ + COD1WZ ;
PVTERPS =  (COD3WI + COD3WJ + COD3WN) * (1-V_CNR) ;

SHBA =(ESFP + TSPR + RCM1 + BIN + BNN + NPLOCNETF
       + COD1TZ + positif(COD3CN) * COD3AN +positif(COD2OP)* (BTPM3VG+PVTAXSB+BTPM3TJ+BTPM3UA+COD3SZ+GAINPEA)+PVIMPOS * null(1-FLAG_EXIT)*positif(COD2OP)+(PVIMPOS + PVSURSI) *positif(COD2OP)* null(2 -FLAG_EXIT)
...
TSHBA =(ESFP + TTSPR + RCM1
       + max(0,max(0,MLOCDECV-TMIB_ABNPVLV-TMIB_ABNPPLV)+max(0,MLOCDECC-TMIB_ABNPVLC-TMIB_ABNPPLC)+max(0,MLOCDECP-TMIB_ABNPVLP-TMIB_ABNPPLP)
              +NPLOCNETV + NPLOCNETC + NPLOCNETPAC - TDNPLOCIMPU +DEFLOCNPF)
	             + TBICPF + TBICNPF + TBNN + BIPTAV + BIPTAC + BIPTAP
		            + COD1TZ + positif(COD3CN) * COD3AN +positif(COD2OP)*( BTPM3VG +PVTAXSB+BTPM3TJ+BTPM3UA+COD3SZ+GAINPEA)+ PVIMPOS *positif(COD2OP)* null(1-FLAG_EXIT) + (PVIMPOS + PVSURSI) *positif(COD2OP)* null(2 -FLAG_EXIT)
