...
XEXTSV = XTSBV + CARTSV + REMPLAV + CODRAG ;
XEXTSC = XTSBC + CARTSC + REMPLAC + CODRBG ;
XEXTS1 = XTSB1 + CARTSP1 + REMPLAP1 + CODRCG ;
XEXTS2 = XTSB2 + CARTSP2 + REMPLAP2 + CODRDG ;
XEXTS3 = XTSB3 + CARTSP3 + REMPLAP3 + CODRGG ;
XEXTS4 = XTSB4 + CARTSP4 + REMPLAP4 + CODRFG ;
...
PVTAUX =COD3AN * (1-positif(COD3CN)) + BPVSJ + BPVSK + BPV18V + BPCOPTV + BPV40V + COD3WI + COD3WJ+COD3PI ;
...
REVKIRE = (1-null(IND_TDR)) * (arr (
       max ( 0, RI1RFR * (1-present(IND_TDR))
              + max(0,IND_TDR-PVTAXSB*positif(COD2OP)-COD3SZ)
              +  VARREVKIRE
              +  PVTXEFF
	      + QUOKIRE
...
RFRTEFFEXO = TTSN1AC + TTSN1BC + TTSN1CC + TTSN1DC + TTSN1EC + TTSN1FC + TPRNN1AH+ TPRNN1BH+ TPRNN1CH+ TPRNN1DH+ TPRNN1EH+ TPRNN1FH 
                   + arr((AUTOBICVV + AUTOBICVC+ AUTOBICVP) * (1- (TX_MIBVEN/100)))
                   + arr((AUTOBICPV + AUTOBICPC+ AUTOBICPP) * (1- (TX_MIBPRES/100)))
                   + arr((AUTOBNCV + AUTOBNCC+ AUTOBNCP) * (1- (SPETXAB/100)))
                   + RFOTENET3EXO + RMFORDINETEXO
		   ;
QUOKIREHR =  TGL4 + somme (x=V,C,1..4 : TGLPRRx+TGLPRRZx+ TGLFx+ TGLTSx+ TGLTSREMPx+TGLPALIMx)
...
REVKIREHRTEF = REVKIREHR - TEFFP + RNI;
regle 871130 :
application :  iliad ;
...
REVKIRE1 = (1-null(IND_TDR)) * (arr (
	      ...
	      + BATMARGV
	      + TERPQRAF + TERPQRAG + TERPQRAL + TERPQRAM + COD5XA + BAEXV 
              + MIBEXV + BICEXV + MIBNPEXV + BICNPEXV
              + BNCPROEXV + BNCEXV + XSPENPV + BNCNPREXAAV 
	      + XETSNNV + XEXOCETV + XTSNNV
	      + BNCCRV + BNCCRFV
	      + (GAINABDET +PVTITRESOC +RCMIMPAT +PVIMMO +PVMOBNR + COD3EX +COD1UZ + COD1WZ +COD1VZ +COD3WN+COD3XN+RCMIMPTN)/2
	      ...
	      + arr((COD5ZA + BAEXP ) / 2)
	      + arr((MIBEXP + BICEXP + MIBNPEXP + BICNPEXP ) / 2) 
	      + arr((BNCPROEXP + BNCEXP + XSPENPP + BNCNPREXAAP) / 2)  
...
REVKIRE2 = (1-null(IND_TDR)) * (arr (
	      ...
	      + (APERPV + APERPC + APERPP)/2
	      + BATMARGC
	      + TERPQRBF + TERPQRBG + TERPQRBL + TERPQRBM + COD5YA + BAEXC 
              + MIBEXC +BICEXC +MIBNPEXC +BICNPEXC 
              + BNCPROEXC +BNCEXC +XSPENPC +BNCNPREXAAC 
