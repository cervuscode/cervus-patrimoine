	    ...
	    ( positif(FLAG_RETARD) * positif(FLAG_RECTIF) * positif(FLAG_RETARD0718 )
            ...
            + (1 - positif(FLAG_RETARD) * positif(FLAG_RECTIF) * positif(FLAG_RETARD0718 )) 
                              * (positif(IRBASE)*positif(TIRBASE[FLAG_DERSTTR])) * max(IRBASE,TIRBASE[FLAG_DERSTTR])
                              +  (1-positif(IRBASE)) * (1-positif(TIRBASE[FLAG_DERSTTR])) * (
                                                            positif(abs(IRBASE)-abs(TIRBASE[FLAG_DERSTTR])) * (TIRBASE[FLAG_DERSTTR] * (1-FLAG_DEFAUT)
                                                                                                            + IRBASE * FLAG_DEFAUT)
                                                            + (1-positif(abs(IRBASE)-abs(TIRBASE[FLAG_DERSTTR]))) * IRBASE
							                                    )
...
TPCAPBASE[X] = null(X) * 
   ...
   + (1 - positif(FLAG_RETARD) * positif(FLAG_RECTIF) ) 
                            * (positif(CAPBASE_MAJO)*positif(TPCAPBASE[FLAG_DERSTTR])) * max(CAPBASE_MAJO,TPCAPBASE[FLAG_DERSTTR])
                                  +  (1-positif(CAPBASE_MAJO)) * (1-positif(TPCAPBASE[FLAG_DERSTTR])) * (
				                                   positif(abs(CAPBASE_MAJO)-abs(TPCAPBASE[FLAG_DERSTTR])) * TPCAPBASE[FLAG_DERSTTR]
				                                   + (1-positif(abs(CAPBASE_MAJO)-abs(TPCAPBASE[FLAG_DERSTTR]))) * CAPBASE_MAJO
								                                        )
    ...
    ( positif(FLAG_RETARD) * positif(FLAG_RECTIF) * positif(FLAG_RETARD0718) 
            ...
            + (1 - positif(FLAG_RETARD) * positif(FLAG_RECTIF) * positif(FLAG_RETARD0718 )) 
                             * (positif(HRBASE_MAJO)*positif(TCHRBASE[FLAG_DERSTTR])) * max(HRBASE_MAJO,TCHRBASE[FLAG_DERSTTR])
                                    +  (1-positif(HRBASE_MAJO)) * (1-positif(TCHRBASE[FLAG_DERSTTR])) * (
				                                  positif(abs(HRBASE_MAJO)-abs(TCHRBASE[FLAG_DERSTTR]))*TCHRBASE[FLAG_DERSTTR]
				                                  + (1-positif(abs(HRBASE_MAJO)-abs(TCHRBASE[FLAG_DERSTTR])))*HRBASE_MAJO
								                                        )
	    ...
	    ( positif(FLAG_RETARD) * positif(FLAG_RECTIF) * positif(FLAG_RETARD0718)
            ...
            + (1 - positif(FLAG_RETARD) * positif(FLAG_RECTIF) * positif(FLAG_RETARD0718 )) 
                             * (positif(TAXABASE_MAJO)*positif(TTAXABASE[FLAG_DERSTTR])) * max(TAXABASE_MAJO,TTAXABASE[FLAG_DERSTTR])
                                +  (1-positif(TAXABASE_MAJO)) * (1-positif(TTAXABASE[FLAG_DERSTTR])) * (
				                                positif(abs(TAXABASE_MAJO)-abs(TTAXABASE[FLAG_DERSTTR])) * TTAXABASE[FLAG_DERSTTR] 
				                                + (1-positif(abs(TAXABASE_MAJO)-abs(TTAXABASE[FLAG_DERSTTR]))) * TAXABASE_MAJO
								                                       )
...
regle corrective base_stratemajo 130:
application :  iliad ;
...
regle corrective base_stratemajo 140:
application :  iliad ;
...
TIRBASE[0] = positif(IRN) * max(0,IRN) + (1-positif(IRN)) * IRN + CODZRA - IRANT - IR9YI;
TTAXABASE[0] = max(0,TAXASSUR ) - TAXA9YI;
TPCAPBASE[0] = max(0,IPCAPTAXT) - CAP9YI;
TCHRBASE[0] = max(0,IHAUTREVT + CHRPVIMP) - CHR9YI;
TNAPCR[0] = max(0,NAPCRINR - CSGIM - PRSPROV - CRDSIM) ;
TCSBASE[0] = max(0,CSG - CSGIM) - CS9YP;
TPSOLBASE[0] = max(0,PSOL - PRSPROV) - PS9YP;
TRDBASE[0] = max(0,RDSN - CRDSIM)- RD9YP;
TCVNBASE[0] = max(0,CVNN - COD8YT) - CVN9YP;
TCDISBASE[0] = max(0,CDIS - CDISPROV) - CDIS9YP;
TGLOBASE[0] = max(0,CGLOA - CGLOAPROV) - GLO9YP;
TC820BASE[0] = max(0,MCSG820)- C8209YP;
TRSE1BASE[0] = max(0,RSE1N - CSPROVYD) - RSE19YP ;
TRSE2BASE[0] = max(0,RSE2N - CSPROVRSE2) - RSE29YP;
TRSE3BASE[0] = max(0,RSE3N - CSPROVYG) - RSE39YP;
TRSE4BASE[0] = max(0,RSE4N - CSPROVRSE4) - RSE49YP;
TRSE5BASE[0] = max(0,RSE5N - CSPROVYE) - RSE59YP;
TRSE6BASE[0] = max(0,RSE6N - COD8YQ) - RSE69YP;
TRSE8BASE[0] = max(0,RSE8N)- RSE89YP;
...
MAJO4TOT = MAJO1729_TOT + MAJO1732_TOT + MAJO1758_TOT  + (COD8NA * LIM1500 + COD8NB * LIM10000) * (1-positif(MAJOTOT35)) * (1-positif(INDCODIFI));
