...
PERPBANV = BAFPVV + BAEXV + BACREV - BACDEV + BAPERPV + 4BACREV 
           + BAFORESTV + COD5XT - COD5XO + COD5AK + COD5XA + BAMICV ;

PERPBANC = BAFPVC + BAEXC + BACREC - BACDEC + BAPERPC + 4BACREC 
           + BAFORESTC + COD5XU - COD5YO + COD5BK + COD5YA + BAMICC ;

PERPBANP = (BAFPVP + BAEXP + BACREP - BACDEP + BAPERPP + 4BACREP  
            + BAFORESTP - COD5ZO + COD5CK + COD5ZA + BAMICP) * PERP_BOOL ;
...
PERPBICPNV = BICEXV + BICNOV - BICDNV + COD5DF + CODCKC ;

PERPBICPNC = BICEXC + BICNOC - BICDNC + COD5EF + CODCLC ;

PERPBICPNP = (BICEXP + BICNOP - BICDNP + COD5FF + CODCMC ) * PERP_BOOL ;
...
PERPBNCPNV = BNCEXV + BNCREV - BNCDEV + BNCCRV + COD5XJ + CODCQC ;

PERPBNCPNC = BNCEXC + BNCREC - BNCDEC + BNCCRC + COD5YJ + CODCRC ;

PERPBNCPNP = (BNCEXP + BNCREP - BNCDEP + BNCCRP + COD5ZJ + CODCSC ) * PERP_BOOL ; 
...
PERP_INDV = positif(0 + positif(PERPSALV) 
	              + positif(BAFPVV + BAEXV + BACREV + BACDEV + BAPERPV + 4BACREV 
	                        + BAFORESTV + COD5XT + COD5XO + COD5AK + COD5XA + BAMICV)
		      + positif(MIBEXV + MIBVENV + MIBPRESV + MIBPVV + AUTOBICVV + AUTOBICPV + MIBGITEV + LOCGITV )
                      + positif(BICEXV + BICNOV + BICDNV + COD5DF + CODCKC )
                      + positif(BNCPROEXV + BNCPROV + BNCPROPVV + AUTOBNCV) 
                      + positif(BNCEXV + BNCREV + BNCDEV + BNCCRV + COD5XJ + CODCQC )) ;

PERP_INDC = positif(0 + positif(PERPSALC)
                      + positif(BAFPVC + BAEXC + BACREC + BACDEC + BAPERPC + 4BACREC 
		                + BAFORESTC + COD5XU + COD5YO + COD5BK + COD5YA + BAMICC)
                      + positif(MIBEXC + MIBVENC + MIBPRESC + MIBPVC + AUTOBICVC + AUTOBICPC + MIBGITEC + LOCGITC + MIBMEUC)
                      + positif(BICEXC + BICNOC + BICDNC + COD5EF + CODCLC )
                      + positif(BNCPROEXC + BNCPROC + BNCPROPVC + AUTOBNCC) 
                      + positif(BNCEXC + BNCREC + BNCDEC + BNCCRC + COD5YJ + CODCRC )) ;

PERP_INDP = positif(0 + positif(PERPSALP)
                      + positif(BAFPVP + BAEXP + BACREP + 4BACREP + BACDEP + BAPERPP
                                + BAFORESTP + COD5ZO + COD5CK + COD5ZA + BAMICP)
                      + positif(MIBEXP + MIBVENP + MIBPRESP + MIBPVP + AUTOBICVP + AUTOBICPP+ MIBGITEP+LOCGITP+ MIBMEUP)
                      + positif(BICEXP + BICNOP + BICDNP + COD5FF + CODCMC )
                      + positif(BNCPROEXP + BNCPROP + BNCPROPVP + AUTOBNCP)
                      + positif(BNCEXP + BNCREP + BNCDEP + BNCCRP + COD5ZJ + CODCSC )) ;
...
PERPINDV = positif(((positif(positif(PERP_INDV)
                    + (1 - positif(PERP_INDV)) * (1 - positif_ou_nul(PRBRV + PALIV + COD1AI + PCAPTAXV))) * positif(INDREV1A8))
                    + (1 - positif(PERP_INDV)) * positif_ou_nul(PRBRV + PALIV + COD1AI + PCAPTAXV) * positif(PERP_COTV+COD6NS)
                    + PERPMUTU * (1 - positif(PERP_INDV + PERP_COTV+COD6NS)))
                             * (1 - positif_ou_nul(PRBRV + PALIV + COD1AI + PCAPTAXV))
	            + positif(PERP_INDV) * positif_ou_nul(PRBRV + PALIV + COD1AI + PCAPTAXV) 	
		    + PERPMUTU * (1-positif(PERPIMPATRIE))* BOOL_0AM
		    + (1 - positif(PERP_INDV)) * positif_ou_nul(PRBRV + PALIV + COD1AI + PCAPTAXV) * positif(PERP_COTV+COD6NS)
                    + positif(PERPREVTOTV) * (1-positif(PERP_COTV+COD6NS)) * (1-positif(PERPMUTU))
                    * positif_ou_nul(PRBRV + PALIV + COD1AI + PCAPTAXV))
                    * (1 - PERP_NONV) * (1 - V_CNR)
                     ;


PERPINDC = positif(((positif(positif(PERP_INDC)
	            + (1 - positif(PERP_INDC)) * (1 - positif_ou_nul(PRBRC + PALIC + COD1BI + PCAPTAXC))) * positif(INDREV1A8))
	            + (1 - positif(PERP_INDC)) * positif_ou_nul(PRBRC + PALIC + COD1BI + PCAPTAXC) * positif(PERP_COTC+COD6NT) 
	            + PERPMUTU * (1 - positif(PERP_INDC + PERP_COTC+COD6NT)))
	                     * (1 - positif_ou_nul(PRBRC + PALIC + COD1BI + PCAPTAXC))
	            + positif(PERP_INDC) * positif_ou_nul(PRBRC + PALIC + COD1BI + PCAPTAXC)
	  	    + PERPMUTU * (1-positif(PERPIMPATRIE)) * BOOL_0AM
		    + (1 - positif(PERP_INDC)) * positif_ou_nul(PRBRC + PALIC + COD1BI + PCAPTAXC) * positif(PERP_COTC+COD6NT)
		    + positif(PERPREVTOTC) * (1-positif(PERP_COTC+COD6NT)) * (1-positif(PERPMUTU))
		    * positif_ou_nul(PRBRC + PALIC + COD1BI + PCAPTAXC))
		    * (1 - PERP_NONC) * BOOL_0AM * (1 -V_CNR)
	             ;

PERPINDP = positif(((positif(positif(PERP_INDP)
                    + (1 - positif(PERP_INDP)) * (1 - positif_ou_nul(PRBR1 + PALIP + COD1CI + COD1DI + COD1EI + COD1FI + COD1CT + COD1DT + COD1ET + COD1FT))) * positif(INDREV1A8))
                    + (1 - positif(PERP_INDP)) * positif_ou_nul(PRBR1 + PALIP + COD1CI + COD1DI + COD1EI + COD1FI + COD1CT + COD1DT + COD1ET + COD1FT) * positif(PERP_COTP+COD6NU))
		     * (1 - positif_ou_nul(PRBR1 + PRBR2 + PRBR3 + PRBR4 + PALIP + COD1CI + COD1DI + COD1EI + COD1FI)) 
		    + positif(PERP_INDP)  * positif_ou_nul(PRBR1 + PALI1 + COD1CI + COD1CT)
		    + (1 - positif(PERP_INDP)) * positif_ou_nul(PRBR1 + PALI1 + COD1CI + COD1CT) * positif(PERP_COTP+COD6NU)
	            + positif(PERPREVTOTP) * positif_ou_nul(PRBR1 + PRBR2 + PRBR3 + PRBR4 + PALIP + COD1CI + COD1DI + COD1EI + COD1FI))
            * PERP_BOOL * (1 - V_CNR) 
	    ;
