...
INDPASV = positif( TSHALLOV + COD1GA + ALLOV + FRNV + CARTSV + REMPLAV + COD1GE + COD1AA + COD1GH + COD1AD
                   ...
                   + 4BACREV + BAFPVV + COD5XO + COD5XT 
                   + MIBVENV + MIBPRESV + MIBPVV + BICPMVCTV + BICNOV + BICDNV + MIBMEUV 
                   + MIBGITEV + LOCGITV + COD5NW + LOCNPCGAV + LOCGITCV + LOCDEFNPCGAV + COD5WE 
		   + MIBNPVENV + MIBNPPRESV + MIBNPPVV + MIBNPDCT + BICREV 
		   + BICDEV + CODCKC + CODCNC 
                   + BNCPROV + BNCPROPVV + BNCPMVCTV + BNCREV + BNCDEV + BNCNPV + BNCNPPVV
		   + BNCNPDCT + BNCAABV + BNCAADV + CODCQC + CODCJG + 0) ; 
...
INDPASC = positif( TSHALLOC + COD1HA + ALLOC + FRNC + CARTSC + REMPLAC + COD1HE + COD1BA + COD1HH + COD1BD
                   ...
                   + 4BACREC + BAFPVC + COD5YO + COD5XU 
                   + MIBVENC + MIBPRESC + MIBPVC + BICPMVCTC + BICNOC + BICDNC + MIBMEUC 
                   + MIBGITEC + LOCGITC + COD5OW + LOCNPCGAC + LOCGITCC + LOCDEFNPCGAC + COD5XE 
		   + MIBNPVENC + MIBNPPRESC + MIBNPPVC + COD5RZ + BICREC 
		   + BICDEC + CODCLC + CODCOC 
                   + BNCPROC + BNCPROPVC + BNCPMVCTC + BNCREC + BNCDEC + BNCNPC + BNCNPPVC
		   + COD5LD + BNCAABC + BNCAADC + CODCRC + CODCRF  + 0) ;
...
INDPASP = positif( TSHALLO1 + TSHALLO2 + TSHALLO3 + TSHALLO4 + ALLO1 + ALLO2 + ALLO3 + ALLO4
		   ...
		   + COD5ZB + BAFORESTP + BACREP + BACDEP + 4BACREP 
		   + BAFPVP + COD5ZO
		   + MIBVENP + MIBPRESP + MIBPVP + BICPMVCTP + BICNOP + BICDNP + MIBMEUP + MIBGITEP
		   + LOCGITP + COD5PW + LOCNPCGAPAC + LOCGITCP + LOCDEFNPCGAPAC + COD5YE 
		   + MIBNPVENP + MIBNPPRESP + MIBNPPVP + COD5SZ + BICREP + BICDEP 
		   + CODCMC + CODCPC 
		   + BNCPROP + BNCPROPVP + BNCPMVCTP + BNCREP + BNCDEP + BNCNPP + BNCNPPVP + COD5MD
		   + BNCAABP + BNCAADP + CODCSC + CODCSF ) ;
...
BAPASV = max(0 , arr(IBAMICV + BAFORESTV + R15HC 
                     - arr(DEFANTBAV * ((IBAMICV + BAFORESTV + R15HC ) / BAHQNODEFV)))) + BAQV + BATMARGV ;

BAPASC = max(0 , arr(IBAMICC + BAFORESTC + R15IC 
                     - arr(DEFANTBAC * ((IBAMICC + BAFORESTC + R15IC ) / BAHQNODEFC)))) + BAQC + BATMARGC ;

BAPASP = max(0 , arr(IBAMICP + BAFORESTP + R15JC 
                     - arr(DEFANTBAP * ((IBAMICP + BAFORESTP + R15JC ) / BAHQNODEFP)))) + BAQP ;

BATAUXF = BAPASV + BAPASC + BAPASP ;

BICPASVP = MIB_NETVV + MIB_NETPV + MIBPVV - BICPMVCTV + R15KC + BIPTAQV  ;
BICPASVNP = max(0 , max(0 , MIB_NETNPVV + MIB_NETNPPV + MIBNPPVV - MIBNPDCT
                            + arr(R15NC - (DEFANTBICNPV * (MIB_NETNPVV + MIB_NETNPPV + MIBNPPVV - MIBNPDCT + R15NC ) / BICNPONCV)))
	            + BICQV) ;
BICPASVLNP = max(0 , MLOCNETV + SNPLOCPASV - arr(DEFANTLOCV * (SNPLOCPASV  + MLOCNETV) / RNPILOCV)) ;
BICPASCP = MIB_NETVC + MIB_NETPC + MIBPVC - BICPMVCTC + R15LC + BIPTAQC  ;
BICPASCNP = max(0 , max(0 , MIB_NETNPVC + MIB_NETNPPC + MIBNPPVC - COD5RZ
                            + arr(R15OC - (DEFANTBICNPC * (MIB_NETNPVC + MIB_NETNPPC + MIBNPPVC - COD5RZ + R15OC ) / BICNPONCC)))
	            + BICQC) ;
BICPASCLNP = max(0 , MLOCNETC + SNPLOCPASC - arr(DEFANTLOCC * (SNPLOCPASC + MLOCNETC) / RNPILOCC)) ;
BICPASPP = MIB_NETVP + MIB_NETPP + MIBPVP - BICPMVCTP + R15MC + BIPTAQP  ;
BICPASPNP = max(0 , max(0 , MIB_NETNPVP + MIB_NETNPPP + MIBNPPVP - COD5SZ
                            + arr(R15PC  - (DEFANTBICNPP * (MIB_NETNPVP + MIB_NETNPPP + MIBNPPVP - COD5SZ + R15PC ) / BICNPONCP)))
	            + BICQP) ;
BICPASPLNP = max(0 , MLOCNETP + SNPLOCPASP - arr(DEFANTLOCP * (SNPLOCPASP + MLOCNETP) / RNPILOCP)) ;
...
BICPASV = max(0 , (MIB_NETVV + MIB_NETPV + MIBPVV - BICPMVCTV + R15KC + BIPTAQV ) 
                  + max(0 , max(0 , MIB_NETNPVV + MIB_NETNPPV + MIBNPPVV - MIBNPDCT 
		                    + arr(R15NC - (DEFANTBICNPV * (MIB_NETNPVV + MIB_NETNPPV + MIBNPPVV - MIBNPDCT + R15NC ) / BICNPONCV)))
                            + BICQV) 
                  + max(0 , MLOCNETV + SNPLOCPASV - arr(DEFANTLOCV * (SNPLOCPASV + MLOCNETV) / RNPILOCV))) ;

BICPASC = max(0 , (MIB_NETVC + MIB_NETPC + MIBPVC - BICPMVCTC + R15LC + BIPTAQC ) 
                  + max(0 , max(0 , MIB_NETNPVC + MIB_NETNPPC + MIBNPPVC - COD5RZ 
		                    + arr(R15OC - (DEFANTBICNPC * (MIB_NETNPVC + MIB_NETNPPC + MIBNPPVC - COD5RZ + R15OC ) / BICNPONCC)))
			    + BICQC) 
                  + max(0 , MLOCNETC + SNPLOCPASC - arr(DEFANTLOCC * (SNPLOCPASC + MLOCNETC) / RNPILOCC))) ;

BICPASP = max(0 , (MIB_NETVP + MIB_NETPP + MIBPVP - BICPMVCTP + R15MC + BIPTAQP ) 
                  + max(0 , max(0 , MIB_NETNPVP + MIB_NETNPPP + MIBNPPVP - COD5SZ 
		                    + arr(R15PC - (DEFANTBICNPP * (MIB_NETNPVP + MIB_NETNPPP + MIBNPPVP - COD5SZ + R15PC ) / BICNPONCP)))
			    + BICQP) 
                  + max(0 , MLOCNETP + SNPLOCPASP - arr(DEFANTLOCP * (SNPLOCPASP + MLOCNETP) / RNPILOCP))) ;
...
BNCPASVP = MICROBNCV + R15QC + R1CQC ;
BNCPASVNP = max(0 , max(0 , SPENETNPV + BNCNPPVV - BNCNPDCT
                            + arr(R15JG - arr(DABNCNPV * (SPENETNPV + BNCNPPVV - BNCNPDCT + R15JG ) / BNCNPHQNCV)))
	            + BNCNPQCV) ;
BNCPASCP = MICROBNCC + R15RC + R1CRC ;
BNCPASCNP = max(0 , max(0 , SPENETNPC + BNCNPPVC - COD5LD
                            + arr(R15RF - arr(DABNCNPC * (SPENETNPC + BNCNPPVC - COD5LD + R15RF ) / BNCNPHQNCC)))
	            + BNCNPQCC) ;
BNCPASPP = MICROBNCP + R15SC + R1CSC  ;
BNCPASPNP = max(0 , max(0 , SPENETNPP + BNCNPPVP - COD5MD
                            + arr(R15SF - arr(DABNCNPP * (SPENETNPP + BNCNPPVP - COD5MD + R15SF ) / BNCNPHQNCP)))
                    + BNCNPQCP) ;

BNCPROTAUX = max(0 , BNCPASVP + BNCPASCP + BNCPASPP) ;
BNCNPROTAUX = max(0 , BNCPASVNP + BNCPASCNP + BNCPASPNP) ;

BNCPASV = max(0 , (MICROBNCV + R15QC + R1CQC ) 
                  + max(0 , max(0 , SPENETNPV + BNCNPPVV - BNCNPDCT 
		                    + arr(R15JG - arr(DABNCNPV * (SPENETNPV + BNCNPPVV - BNCNPDCT + R15JG ) / BNCNPHQNCV))) 
			    + max(0 , BNCNPQCV - max(0 , DABNCNPV - BNCNPHQCV)))) ; 

BNCPASC = max(0 , (MICROBNCC + R15RC + R1CRC ) 
                  + max(0 , max(0 , SPENETNPC + BNCNPPVC - COD5LD 
		                    + arr(R15RF - arr(DABNCNPC * (SPENETNPC + BNCNPPVC - COD5LD + R15RF ) / BNCNPHQNCC))) 
			    + max(0 , BNCNPQCC - max(0 , DABNCNPC - BNCNPHQCC)))) ;

BNCPASP = max(0 , (MICROBNCP + R15SC  + R1CSC ) 
...
REVTOT = max(0 , TSPRV) + TSNN2VAFF + PRR2V + PRR2ZV + PRR2RAL + PRR2RAM + PENALIMV + PRNRAI + BAPAS1 + BICPAS1 + BNCPAS1 
         + max(0 , TSPRC) + TSNN2CAFF + PRR2C + PRR2ZC + PRR2RBL + PRR2RBM + PENALIMC + PRNRBI + BAPAS2 + BICPAS2 + BNCPAS2 
	 + max(0 , TSPR1) + max(0 , TSPR2) + max(0 , TSPR3) + max(0 , TSPR4) + TSNN2PAFF + PENSTOT1 + PENSTOT2 + PENSTOT3 + PENSTOT4 + PRNRCK 
	 + BAPAS3 + BICPAS3 + BNCPAS3 + PRNFAS + PRNFBS + PRNFCS + PRNFDS + PRNFES + PRNFFS + GLN3 + COD1TZ + RVTOT + T2RV 
...
RASBA1 = (max(0 , arr((BAMICV + BAFORESTV + PASBACV ) * 12 / min(12 , COD5AD + 12 * null(COD5AD + 0)))
                  - arr(DEFANTBAV * ((IBAMICV + BAFORESTV + R15HC ) / BAHQNODEFV))) + BATMARGV)
         * (1 - positif(COD5AF)) * (1 - null(2 - INDPAS)) ;

RASBA2 = (max(0 , arr((BAMICC + BAFORESTC + PASBACC ) * 12 / min(12 , COD5BD + 12 * null(COD5BD + 0)))
                  - arr(DEFANTBAC * ((IBAMICC + BAFORESTC + R15IC ) / BAHQNODEFC))) + BATMARGC)
         * (1 - positif(COD5AI)) * BOOL_0AM * (1 - null(2 - INDPAS)) ;

RASBAP = max(0 , arr((BAMICP + BAFORESTP + PASBACP ) * 12 / min(12 , COD5ED + 12 * null(COD5ED + 0)))
                 - arr(DEFANTBAP * ((IBAMICP + BAFORESTP + R15JC ) / BAHQNODEFP)))
         * (1 - positif(COD5AH)) * (1 - null(2 - INDPAS)) ;

RASBA1N = (max(0 , (BAMICV + BAFORESTV + PASBACV ) 
                   - arr(DEFANTBAV * ((IBAMICV + BAFORESTV + R15HC ) / BAHQNODEFV))) + BATMARGV)
          * (1 - null(2 - INDPAS)) ;

RASBA2N = (max(0 , (BAMICC + BAFORESTC + PASBACC ) 
                   - arr(DEFANTBAC * ((IBAMICC + BAFORESTC + R15IC ) / BAHQNODEFC))) + BATMARGC)
          * BOOL_0AM * (1 - null(2 - INDPAS)) ;
          
RASBAPN = max(0 , (BAMICP + BAFORESTP + PASBACP ) 
                  - arr(DEFANTBAP * ((IBAMICP + BAFORESTP + R15JC ) / BAHQNODEFP)))
          * (1 - null(2 - INDPAS)) ;
...
RASBNC1 = max(0 , arr((SPENETPV + PASBNNAV ) * 12 / min(12 , COD5XI + 12 * null(COD5XI + 0))) * (1 - positif(COD5AO))
                  + max(0 , arr((SPENETNPV + PASBNNAAV ) * 12 / min(12 , COD5XR + 12 * null(COD5XR + 0)))
                            - arr(DABNCNPV * (SPENETNPV + BNCNPPVV - BNCNPDCT + R15JG ) / BNCNPHQNCV)) * (1 - positif(COD5AP)))
          * (1 - null(2 - INDPAS)) ;

RASBNC2 = max(0 , arr((SPENETPC + PASBNNAC ) * 12 / min(12 , COD5YI + 12 * null(COD5YI + 0))) * (1 - positif(COD5BO))
                  + max(0 , arr((SPENETNPC + PASBNNAAC ) * 12 / min(12 , COD5YR + 12 * null(COD5YR + 0)))
                            - arr(DABNCNPC * (SPENETNPC + BNCNPPVC - COD5LD + R15RF ) / BNCNPHQNCC)) * (1 - positif(COD5BP)))
          * BOOL_0AM * (1 - null(2 - INDPAS)) ;

RASBNCP = max(0 , arr((SPENETPP + PASBNNAP ) * 12 / min(12 , COD5ZI + 12 * null(COD5ZI + 0))) * (1 - positif(COD5CQ))
                  + max(0 , arr((SPENETNPP + PASBNNAAP ) * 12 / min(12 , COD5ZR + 12 * null(COD5ZR + 0)))
                            - arr(DABNCNPP * (SPENETNPP + BNCNPPVP - COD5MD + R15SF ) / BNCNPHQNCP)) * (1 - positif(COD5CR)))
          * (1 - null(2 - INDPAS)) ;

RASBNC1N = max(0 , (SPENETPV + PASBNNAV ) 
                   + max(0 , (SPENETNPV + PASBNNAAV ) 
		             - arr(DABNCNPV * (SPENETNPV + BNCNPPVV - BNCNPDCT + R15JG ) / BNCNPHQNCV)))
	   * (1 - null(2 - INDPAS)) ;

RASBNC2N = max(0 , (SPENETPC + PASBNNAC ) 
                   + max(0 , (SPENETNPC + PASBNNAAC ) 
		             - arr(DABNCNPC * (SPENETNPC + BNCNPPVC - COD5LD + R15RF ) / BNCNPHQNCC)))
           * BOOL_0AM * (1 - null(2 - INDPAS)) ;

RASBNCPN = max(0 , (SPENETPP + PASBNNAP ) 
                   + max(0 , (SPENETNPP + PASBNNAAP ) 
	 	             - arr(DABNCNPP * (SPENETNPP + BNCNPPVP - COD5MD + R15SF ) / BNCNPHQNCP)))
 	   * (1 - null(2 - INDPAS)) ;
...
INDTEFFPAS = positif(positif(SALEXTV + COD1AE + COD1AH + AUTOBICVV + AUTOBICPV + AUTOBNCV + COD4EA + COD4EB) * null(1 - TXPASMIN)
                     + positif(SALEXTC + COD1BE + COD1BH + AUTOBICVC + AUTOBICPC + AUTOBNCC + COD4EA + COD4EB) * null(2 - TXPASMIN) 
	             + positif(SALEXT1 + SALEXT2 + SALEXT3 + SALEXT4 + COD1CH + COD1DH + COD1EH + COD1FH + AUTOBICVP + AUTOBICPP + AUTOBNCP + COD4EA + COD4EB)) 
	     * (1 - null(2 - VALREGCO)) ;
...
TREVORDI = (TTSPRV + TBNCPHQCV + max(0 , TBNCNPHQCV - DABNCNPV) + TBICPROOCV + max(0 , TBICNPOCV - DEFANTBICNPV) + max(0 , PASRNPLOCFV)
	      ...
	      + (BAHQC + arr(BAHQP/2)) * positif(BAHQC + arr(BAHQP/2)) + (BAHQC + arr(BAHQP/2)) * ((1 - positif(BAHQC + arr(BAHQP/2))) * positif(SEUIL_IMPDEFBA - SHBAPAS))) * null(2 - TXPASMIN)
           ...
           + arr(COD1TZ/2) + arr(RVTOT/2) + arr(RRCM/2) + arr(PVBAR3VG/2) + arr((RRFI * (1 - positif(COD4EA + COD4EB)) + (RMFORDINET + RFOTENET3 - DRFOTENET2) * positif(COD4EA + COD4EB))/2) 
	   + arr(DESV/2) + arr((ESFP + RE168 + TAX1649 + PREREV + R1649)/2) ;

REVQUOT = (TSNN2VAFF + PRR2V + PRR2ZV + PRR2RAL + PRR2RAM + PENALIMV + PRNRAI + PENFV + GLN3V + BAQV + BICPROQCV + BICQV + BNCQV + BNCPQCV) * null(1 - TXPASMIN)
	  + (TSNN2CAFF + PRR2C + PRR2ZC + PRR2RBL + PRR2RBM + PENALIMC + PRNRBI + PENFC + GLN3C + BAQC + BICPROQCC + BICQC + BNCQC + BNCPQCC) * null(2 - TXPASMIN)
	  + arr(TSNN2PAFF / 2) + arr((PENSTOT1 + PENSTOT2 + PENSTOT3 + PENSTOT4) / 2) + arr((BNCQP + BNCPQCP) / 2) + arr(PENFP / 2) + arr(BAQP / 2)
...
pour i=2..5 :
IRINDPASQ1i = positif(arr((REVINDIV + RASTONPAS)/NBPTPAS) - LIMINFBARi) * positif_ou_nul(LIMSUPBARi - arr((REVINDIV + RASTONPAS)/NBPTPAS))
                                                                        * arr(((REVINDIV + RASTONPAS) * TXBARi / 100) - (NBPTPAS * PLABARi)) ;
IRINDPASQ1 = somme(i=2..5: IRINDPASQ1i) ;

pour i=2..5 :
IRINDPASQ2i = positif((REVINDIV + RASTONPAS) - LIMINFBARi) * positif_ou_nul(LIMSUPBARi - (REVINDIV + RASTONPAS))
                                                           * arr(((REVINDIV + RASTONPAS) * TXBARi/100) - PLABARi - (2 * PLAF_DEMIPART * (NBPTPAS - 1))) ; 
IRINDPASQ2 = somme(i=2..5: IRINDPASQ2i) ;
...
RASTXMARJ = ((null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ12) + null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ22)) * TXBAR2
             + (null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ13) + null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ23)) * TXBAR3
             + (null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ14) + null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ24)) * TXBAR4
             + (null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ15) + null(max(IRINDPASQ1 , IRINDPASQ2) - IRINDPASQ25)) * TXBAR5) * positif(IRINDPASQ1 + IRINDPASQ2) ;

DECINDIV = arr(max( 0 , IRINDPAS + RASIPQ1001V * null(1 - TXPASMIN) + RASIPQ1001C * null(2 - TXPASMIN))
              * ((max(0 , REVORDI + REVQUOT - CHARGES) / max(0 , TREVORDI + REVQUOTEFF - CHARGES) * positif(INDTEFFPAS)) + null(INDTEFFPAS + 0))) ;

IRINDIV = arr(max( 0 , IRINDPAS + RASIPQ1001V * null(1 - TXPASMIN) + RASIPQ1001C * null(2 - TXPASMIN))
              * ((max(0 , REVORDI + REVQUOT - CHARGES) / max(0 , TREVORDI + REVQUOTEFF - CHARGES) * positif(INDTEFFPAS)) + null(INDTEFFPAS + 0))) 
...
REVTOTINDIV = (max(0 , TSPRV) + max(0 , BNCPHQCV + max(0 , BNCHQV)) + max(0 , BAHQV) * positif(BAHQV + arr(BAHQP/2)) + max(0 , BICPROOCV + max(0 , BICHQV) + max(0 , PASRNPLOCFV))
	       + TSNN2VAFF + PRR2V + PRR2ZV + PRR2RAL + PRR2RAM + PENALIMV + PRNRAI + PENFV + GLN3V + BAQV + BICQV + BICPROQCV + BNCQV + BNCPQCV + BATMARGV) * null(1 - TXPASMIN)
	         ...
	         + TSNN2CAFF + PRR2C + PRR2ZC + PRR2RBL + PRR2RBM + PENALIMC + PRNRBI + PENFC + GLN3C + BAQC + BICQC + BICPROQCC + BNCQC + BNCPQCC + BATMARGC) * null(2 - TXPASMIN)
	      ...
	      + arr(TSNN2PAFF / 2) + arr((PENSTOT1 + PENSTOT2 + PENSTOT3 + PENSTOT4) / 2) + arr(PENFP / 2) + arr(BAQP / 2) + arr((BICQP + BICPROQCP) / 2) + arr((BNCQP + BNCPQCP) / 2) 
...
pour i = 1..7 :
RASTXPACi = positif_ou_nul(3 - V_REGCO) * (positif(1620 - REVMENSPACi) * 0
                                           + positif_ou_nul(REVMENSPACi - 1620) * positif(1683 - REVMENSPACi) * TXPAC005
                                           + positif_ou_nul(REVMENSPACi - 1683) * positif(1791 - REVMENSPACi) * TXPAC013
                                           + positif_ou_nul(REVMENSPACi - 1791) * positif(1911 - REVMENSPACi) * TXPAC021
                                           + positif_ou_nul(REVMENSPACi - 1911) * positif(2042 - REVMENSPACi) * TXPAC029
                                           + positif_ou_nul(REVMENSPACi - 2042) * positif(2151 - REVMENSPACi) * TXPAC035
                                           + positif_ou_nul(REVMENSPACi - 2151) * positif(2294 - REVMENSPACi) * TXPAC041
                                           + positif_ou_nul(REVMENSPACi - 2294) * positif(2714 - REVMENSPACi) * TXPAC053
                                           + positif_ou_nul(REVMENSPACi - 2714) * positif(3107 - REVMENSPACi) * TXPAC075
                                           + positif_ou_nul(REVMENSPACi - 3107) * positif(3539 - REVMENSPACi) * TXPAC099
                                           + positif_ou_nul(REVMENSPACi - 3539) * positif(3983 - REVMENSPACi) * TXPAC119
                                           + positif_ou_nul(REVMENSPACi - 3983) * positif(4648 - REVMENSPACi) * TXPAC138
                                           + positif_ou_nul(REVMENSPACi - 4648) * positif(5574 - REVMENSPACi) * TXPAC158
                                           + positif_ou_nul(REVMENSPACi - 5574) * positif(6974 - REVMENSPACi) * TXPAC179
                                           + positif_ou_nul(REVMENSPACi - 6974) * positif(8711 - REVMENSPACi) * TXPAC20
                                           + positif_ou_nul(REVMENSPACi - 8711) * positif(12091 - REVMENSPACi) * TXPAC24
                                           + positif_ou_nul(REVMENSPACi - 12091) * positif(16376 - REVMENSPACi) * TXPAC28
                                           + positif_ou_nul(REVMENSPACi - 16376) * positif(25706 - REVMENSPACi) * TXPAC33
                                           + positif_ou_nul(REVMENSPACi - 25706) * positif(55062 - REVMENSPACi) * TXPAC38
                                           + positif_ou_nul(REVMENSPACi - 55062) * TXPAC43)
                    + null(5 - V_REGCO) * (positif(1858 - REVMENSPACi) * 0
                                           + positif_ou_nul(REVMENSPACi - 1858) * positif(1971 - REVMENSPACi) * TXPAC005
                                           + positif_ou_nul(REVMENSPACi - 1971) * positif(2171 - REVMENSPACi) * TXPAC013
                                           + positif_ou_nul(REVMENSPACi - 2171) * positif(2371 - REVMENSPACi) * TXPAC021
                                           + positif_ou_nul(REVMENSPACi - 2371) * positif(2618 - REVMENSPACi) * TXPAC029
                                           + positif_ou_nul(REVMENSPACi - 2618) * positif(2761 - REVMENSPACi) * TXPAC035
                                           + positif_ou_nul(REVMENSPACi - 2761) * positif(2855 - REVMENSPACi) * TXPAC041
                                           + positif_ou_nul(REVMENSPACi - 2855) * positif(3142 - REVMENSPACi) * TXPAC053
                                           + positif_ou_nul(REVMENSPACi - 3142) * positif(3885 - REVMENSPACi) * TXPAC075
                                           + positif_ou_nul(REVMENSPACi - 3885) * positif(4971 - REVMENSPACi) * TXPAC099
                                           + positif_ou_nul(REVMENSPACi - 4971) * positif(5646 - REVMENSPACi) * TXPAC119
                                           + positif_ou_nul(REVMENSPACi - 5646) * positif(6540 - REVMENSPACi) * TXPAC138
                                           + positif_ou_nul(REVMENSPACi - 6540) * positif(7836 - REVMENSPACi) * TXPAC158
                                           + positif_ou_nul(REVMENSPACi - 7836) * positif(8711 - REVMENSPACi) * TXPAC179
                                           + positif_ou_nul(REVMENSPACi - 8711) * positif(9900 - REVMENSPACi) * TXPAC20
                                           + positif_ou_nul(REVMENSPACi - 9900) * positif(13615 - REVMENSPACi) * TXPAC24
                                           + positif_ou_nul(REVMENSPACi - 13615) * positif(18090 - REVMENSPACi) * TXPAC28
                                           + positif_ou_nul(REVMENSPACi - 18090) * positif(27610 - REVMENSPACi) * TXPAC33
                                           + positif_ou_nul(REVMENSPACi - 27610) * positif(60350 - REVMENSPACi) * TXPAC38
                                           + positif_ou_nul(REVMENSPACi - 60350) * TXPAC43)
                    + null(6 - V_REGCO) * (positif(1990 - REVMENSPACi) * 0
                                           + positif_ou_nul(REVMENSPACi - 1990) * positif(2151 - REVMENSPACi) * TXPAC005
                                           + positif_ou_nul(REVMENSPACi - 2151) * positif(2398 - REVMENSPACi) * TXPAC013
                                           + positif_ou_nul(REVMENSPACi - 2398) * positif(2704 - REVMENSPACi) * TXPAC021
                                           + positif_ou_nul(REVMENSPACi - 2704) * positif(2808 - REVMENSPACi) * TXPAC029
                                           + positif_ou_nul(REVMENSPACi - 2808) * positif(2904 - REVMENSPACi) * TXPAC035
                                           + positif_ou_nul(REVMENSPACi - 2904) * positif(2999 - REVMENSPACi) * TXPAC041
                                           + positif_ou_nul(REVMENSPACi - 2999) * positif(3332 - REVMENSPACi) * TXPAC053
                                           + positif_ou_nul(REVMENSPACi - 3332) * positif(4598 - REVMENSPACi) * TXPAC075
                                           + positif_ou_nul(REVMENSPACi - 4598) * positif(5951 - REVMENSPACi) * TXPAC099
                                           + positif_ou_nul(REVMENSPACi - 5951) * positif(6712 - REVMENSPACi) * TXPAC119
                                           + positif_ou_nul(REVMENSPACi - 6712) * positif(7788 - REVMENSPACi) * TXPAC138
                                           + positif_ou_nul(REVMENSPACi - 7788) * positif(8567 - REVMENSPACi) * TXPAC158
                                           + positif_ou_nul(REVMENSPACi - 8567) * positif(9492 - REVMENSPACi) * TXPAC179
                                           + positif_ou_nul(REVMENSPACi - 9492) * positif(11016 - REVMENSPACi) * TXPAC20
                                           + positif_ou_nul(REVMENSPACi - 11016) * positif(14820 - REVMENSPACi) * TXPAC24
                                           + positif_ou_nul(REVMENSPACi - 14820) * positif(18850 - REVMENSPACi) * TXPAC28
                                           + positif_ou_nul(REVMENSPACi - 18850) * positif(30210 - REVMENSPACi) * TXPAC33
                                           + positif_ou_nul(REVMENSPACi - 30210) * positif(63767 - REVMENSPACi) * TXPAC38
                                           + positif_ou_nul(REVMENSPACi - 63767) * TXPAC43)
		  ;
...
RASPSRF = max(0 , RASRF - COD8RF * BOOL_0AM * null(1 - positif(COD8SH) - positif(COD8SI))) * (1 - positif(COD8SH) * (1 - BOOL_0AM + positif(COD8SI))) ;
...
RASPSRVTO = max(0 , RASRVTO - COD8RV * BOOL_0AM * null(1 - positif(COD8SH) - positif(COD8SI))) * (1 - positif(COD8SH) * (1 - BOOL_0AM + positif(COD8SI))) * (1 - V_CNR) ;
...
RASPSBIC1 = LOCNPASSPSV * (1 - positif(COD5CF)) * (1 - positif(COD8SH)) * (1 - null(2 - INDPAS)) ;

RASPSBIC2 = LOCNPASSPSC * (1 - positif(COD5CI)) * (1 - positif(COD8SI)) * BOOL_0AM * (1 - null(2 - INDPAS)) ;

RASPSBICP = LOCNPASSPSP * (1 - positif(COD5CM)) * (1 - positif(COD8SH + COD8SI)) * (1 - null(2 - INDPAS)) ;

RASPS5HY = RCSV * (1 - positif(COD5AF + COD5BF + COD5AN + COD5CF + COD5AO + COD5AP)) * (1 - positif(COD8SH)) * (1 - null(2 - INDPAS)) * (1 - V_CNR) ;

RASPS5IY = RCSC * (1 - positif(COD5AI + COD5BI + COD5BN + COD5CI + COD5BO + COD5BP)) * (1 - positif(COD8SI)) * (1 - null(2 - INDPAS)) * (1 - V_CNR) ;
...
RASRRVTOM1 = arr(max(0 , RASRVTOF1 - arr((COD8AI + COD8AJ)/(1 + BOOL_0AM))) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRRVTOM2 = arr(max(0 , RASRVTOF2 - arr((COD8AI + COD8AJ)/2)) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRRVTOT1 = arr(max(0 , RASRVTOF1 - arr((COD8AI + COD8AJ)/(1 + BOOL_0AM))) / (null(COD8MT) + min(4 , COD8MT))) ;

RASRRVTOT2 = arr(max(0 , RASRVTOF2 - arr((COD8AI + COD8AJ)/2)) / (null(COD8MT) + min(4 , COD8MT))) ;
...
RASRRFM1 = arr(max(0 , RASRFF1 - arr((COD8AK + COD8AL)/(1 + BOOL_0AM))) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRRFM2 = arr(max(0 , RASRFF2 - arr((COD8AK + COD8AL)/2)) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRRFT1 = arr(max(0 , RASRFF1 - arr((COD8AK + COD8AL)/(1 + BOOL_0AM))) / (null(COD8MT) + min(4 , COD8MT))) ;

RASRRFT2 = arr(max(0 , RASRFF2 - arr((COD8AK + COD8AL)/2)) / (null(COD8MT) + min(4 , COD8MT))) ;
...
RASRPACM1 = arr(max(0 , RASPACF1 - arr((COD8AW + COD8AX)/(1 + BOOL_0AM))) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRPACM2 = arr(max(0 , RASPACF2 - arr((COD8AW + COD8AX)/2)) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRPACT1 = arr(max(0 , RASPACF1 - arr((COD8AW + COD8AX)/(1 + BOOL_0AM))) / (null(COD8MT) + min(4 , COD8MT))) ;

RASRPACT2 = arr(max(0 , RASPACF2 - arr((COD8AW + COD8AX)/2)) / (null(COD8MT) + min(4 , COD8MT))) ;
...
RASRVTOPSM1 = arr(max(0 , RASRVTOPS1 - arr((COD8AQ + COD8AR)/(1 + BOOL_0AM))) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRVTOPSM2 = arr(max(0 , RASRVTOPS2 - arr((COD8AQ + COD8AR)/2)) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRVTOPST1 = arr(max(0 , RASRVTOPS1 - arr((COD8AQ + COD8AR)/(1 + BOOL_0AM))) / (null(COD8MT) + min(4 , COD8MT))) ;

RASRVTOPST2 = arr(max(0 , RASRVTOPS2 - arr((COD8AQ + COD8AR)/2)) / (null(COD8MT) + min(4 , COD8MT))) ;
...
RASRFPSM1 = arr(max(0 , RASRFPS1 - arr((COD8AS + COD8AT)/(1 + BOOL_0AM))) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRFPSM2 = arr(max(0 , RASRFPS2 - arr((COD8AS + COD8AT)/2)) / (null(COD8MM) + min(12 , COD8MM))) ;

RASRFPST1 = arr(max(0 , RASRFPS1 - arr((COD8AS + COD8AT)/(1 + BOOL_0AM))) / (null(COD8MT) + min(4 , COD8MT))) ;

RASRFPST2 = arr(max(0 , RASRFPS2 - arr((COD8AS + COD8AT)/2)) / (null(COD8MT) + min(4 , COD8MT))) ;
...
RASPACPSM1 = arr(max(0 , RASPACPS1 - arr((COD8AY + COD8AZ)/(1 + BOOL_0AM))) / (null(COD8MM) + min(12 , COD8MM))) ;

RASPACPSM2 = arr(max(0 , RASPACPS2 - arr((COD8AY + COD8AZ)/2)) / (null(COD8MM) + min(12 , COD8MM))) ;

RASPACPST1 = arr(max(0 , RASPACPS1 - arr((COD8AY + COD8AZ)/(1 + BOOL_0AM))) / (null(COD8MT) + min(4 , COD8MT))) ;

RASPACPST2 = arr(max(0 , RASPACPS2 - arr((COD8AY + COD8AZ)/2)) / (null(COD8MT) + min(4 , COD8MT))) ;
...
RASTSPEM1 = RASTSPEFM1 * (1 - BOOL_0AM) + RASTSPEIM1 * BOOL_0AM ;
RASAUSAM1 = RASAUSAFM1 * (1 - BOOL_0AM) + RASAUSAIM1 * BOOL_0AM ;
RASASSOM1 = RASASSOFM1 * (1 - BOOL_0AM) + RASASSOIM1 * BOOL_0AM ;
RASBAM1 = RASBAFM1 * (1 - BOOL_0AM) + RASBAIM1 * BOOL_0AM ;
RASBICM = RASBICFM1 * (1 - BOOL_0AM) + RASBICIM1 * BOOL_0AM ;
RASBNCM = RASBNCFM1 * (1 - BOOL_0AM) + RASBNCIM1 * BOOL_0AM ;

RASTOT1 = RASTSPEM1 + RASAUSAM1 + RASASSOM1 + RASBAM1 + RASBICM + RASBNCM + RASPSBICM1 + RASPS5HYM ;

RASTOT2 = RASTSPEIM2 + RASAUSAIM2 + RASASSOIM2 + RASBAIM2 + RASBICIM2 + RASBNCIM2 + RASPSBICM2 + RASPS5IYM ;

RASTOT = RASTOTF + RASTOT1 + RASTOT2 ;

RASTOTFM = RASTSPEFM1 + RASAUSAFM1 + RASASSOFM1 + RASBAFM1 + RASBICFM1 + RASBNCFM1 + RASPSBICM1 + RASPS5HYM
           + RASTSPEFM2 + RASAUSAFM2 + RASASSOFM2 + RASBAFM2 + RASBICFM2 + RASBNCFM2 + RASPSBICM2 + RASPS5IYM
...
RASTOTIM = RASTSPEIM1 + RASAUSAIM1 + RASASSOIM1 + RASBAIM1 + RASBICIM1 + RASBNCIM1 + RASPSBICM1 + RASPS5HYM
           + RASTSPEIM2 + RASAUSAIM2 + RASASSOIM2 + RASBAIM2 + RASBICIM2 + RASBNCIM2 + RASPSBICM2 + RASPS5IYM 
	   + RASRFFM1 + RASRFFM2 + RASRVTOFM1 + RASRVTOFM2 + RASPSRFM1 + RASPSRFM2 + RASPSRVTOM1 + RASPSRVTOM2 
	   + RASPACFM1 + RASPACFM2 + RASPSPACM1 + RASPSPACM2 ;
...
INDACPAS = positif_ou_nul(RASBAFM1 + RASBICFM1 + RASBNCFM1 + RASTSPEFM1 + RASAUSAFM1 + RASASSOFM1 + RASBAFM2 + RASBICFM2 
                          + RASBNCFM2 + RASTSPEFM2 + RASAUSAFM2 + RASASSOFM2 + RASRFA + RASRVTOA + RASPACA + RASPSPACA
...
LIGIRRAS = positif(LIGRASTSPE + LIGRASAUSA + LIGRASASSO + LIGRASPAC + LIGRASRVTO + LIGRASRF + LIGRASBA + LIGRASBIC + LIGRASBNC) * LIGPAS ;
...
LIGIRRAS1 = positif(LIGRASTSPE1 + LIGRASAUSA1 + LIGRASASSO1 + LIGRASPAC1 + LIGRASRVTO1 + LIGRASRF1 + LIGRASBA1 + LIGRASBIC1 + LIGRASBNC1) * LIGPAS1 ;
...
CONBASIR = (positif_ou_nul(PASEFFOYIR - arr(PASFARFAITIR * TX90/100)) + positif((arr(PASFARFAITIR * TX90/100) - PASEFFOYIR) *  max(0,(PASEFFOYIR - PASATFOYIR))));

SCBASIR = DECSANIR * (positif_ou_nul(PASEFFOYIR - arr(PASFARFAITIR * TX90/100)) * 0
                   + positif((arr(PASFARFAITIR * TX90/100) - PASEFFOYIR) *  max(0,(PASEFFOYIR - PASATFOYIR))) * 0
	           + (1-positif(CONBASIR)) * max(0 , (min(PASFARFAITIR , PASATFOYIR) - PASEFFOYIR)));
...
CONBASPS = (positif_ou_nul(PASEFFOYPS - arr(PASPARFAITPS * TX90/100)) + positif((arr(PASPARFAITPS * TX90/100) - PASEFFOYPS) *  max(0,(PASEFFOYPS - PASATFOYPS))));

SCBASPS = DECSANPS * (positif_ou_nul(PASEFFOYPS - arr(PASPARFAITPS * TX90/100)) * 0 
                   + positif((arr(PASPARFAITPS * TX90/100) - PASEFFOYPS) *  max(0,(PASEFFOYPS - PASATFOYPS))) * 0
                   + (1-positif(CONBASPS)) * max(0 , (min(PASPARFAITPS , PASATFOYPS) - PASEFFOYPS))); 
...
SPIDEC = positif(CODSCR) ;

DECSAN = NBPART * positif(SPIDEC) ;



PASCORRECTIR = DECSAN * PASTOTAL ;


RASPADEC1IR = DECSAN * arr((TSTAUXRASV * (CODSAG/TX10))/100) ;
RASPADEC2IR = DECSAN * arr((TSTAUXRASC * (CODSAH/TX10))/100) ;
ACPASDEC1IR = DECSAN * CODSAI * NOMBRE12 ;
ACPASDEC2IR = DECSAN * CODSAJ * NOMBRE12 ;

PASPADEC1IR = DECSAN * RASPADEC1IR + ACPASDEC1IR ;
PASPADEC2IR = DECSAN * RASPADEC2IR + ACPASDEC2IR ;

PASPASSIFIR = DECSAN * (PASPADEC1IR + PASPADEC2IR) ;


PASEDEC1IRB = DECSAN * max(0 , (COD8HV + CODSAW + CODSAS + CODSCS - COD8HY)) ;
PASEDEC2IRB = DECSAN * max(0 , (COD8IV + CODSAX + CODSAT + CODSDS - COD8IY)) ;

PASEFFBIR = DECSAN * (PASEDEC1IRB + PASEDEC2IRB) ;
...
COBIR = (positif_ou_nul((PASMODULEIR + ACNRIR) - arr(PASCORRECTIR * TX90/100))) ;

COBAIR = (positif_ou_nul((PASMODULEIR + ACNRIR) - arr(PASCORRECTIR * TX90/100)) + positif_ou_nul(PASEFFBIR - arr(PASCORRECTIR * TX70/100)) * (min(PASCORRECTIR,PASPASSIFIR - PASEFFBIR))) ;

SCBASIRB = max(0,DECSAN * (positif_ou_nul((PASMODULEIR + ACNRIR) - arr(PASCORRECTIR * TX90/100)) * 0
                  + (1-positif(COBIR)) * positif_ou_nul(PASEFFBIR - arr(PASCORRECTIR * TX70/100)) * (min(PASCORRECTIR,PASPASSIFIR - PASEFFBIR))
                  + (1-positif(COBAIR)) *  (min(PASCORRECTIR,PASPASSIFIR - PASEFFBIR)))) ;



SANTXIR = positif_ou_nul((PASMODULEIR + ACNRIR) - arr(PASCORRECTIR * TX90/100)) + positif_ou_nul(PASEFFBIR - arr(PASCORRECTIR * TX70/100)) ;
SANIR = positif(PASCORRECTIR) * positif(PASCORRECTIR - PASEFFBIR) ; 

SCTXIRB = DECSAN * positif(SCBASIRB) * positif(SANIR) * (positif_ou_nul((PASMODULEIR + ACNRIR) - arr(PASCORRECTIR * TX90/100)) * 0
                 + positif_ou_nul(PASEFFBIR - arr(PASCORRECTIR * TX70/100)) * TX10
                 + (1 - positif(SANTXIR)) * arr(TX050 * (min(PASCORRECTIR,PASPASSIFIR - PASEFFBIR)/min(PASCORRECTIR,PASPASSIFIR))*100 * 10)/10) ;

 
SCMTIRB = DECSAN * arr(SCBASIRB * SCTXIRB/100) ;




BASACPSCVD1 = DECSAN * (RASPSBIC1 + RASPS5HY + RASPSRVTO + RASPSRF + RASPSBICP + RASPSPAC);
  
PASCOFOYCVD = DECSAN * (arr(BASACPSCVD1 * TXPASPS / 100) * positif(V_0AC + V_0AD + V_0AV)) ;

BASACPSMP1 = DECSAN * (RASPSBIC1 + RASPS5HY + (RASPSRVTO/2) + (RASPSRF/2) + (RASPSBICP/2) + (RASPS5JY/2)) ;

BASACPSMP2 = DECSAN * (RASPSBIC2 + RASPS5IY + (RASPSRVTO/2) + (RASPSRF/2) + (RASPSBICP/2) + (RASPS5JY/2)) ;

ACTPSDEC1 = DECSAN * (arr(BASACPSMP1 * TXPASPS / 100)) ;
ACTPSDEC2 = DECSAN * (arr(BASACPSMP2 * TXPASPS / 100)) ;

PASCOFOYMP = DECSAN * ((ACTPSDEC1 + ACTPSDEC2) * positif(V_0AM + V_0AO)) ; 

PASCORRECTPS = DECSAN * (PASCOFOYCVD + PASCOFOYMP) ;


PASPASPSDEC1 = DECSAN * (CODSAK * NOMBRE12) ; 
PASPASPSDEC2 = DECSAN * (CODSAL * NOMBRE12) ;
PASPASSIFPS =  DECSAN * (PASPASPSDEC1 + PASPASPSDEC2) ;


PASEFBPSDEC1 = DECSAN * max(0,(CODSAQ - COD8HZ)) ; 
PASEFBPSDEC2 = DECSAN * max(0,(CODSAR - COD8IZ)) ; 

PASEFFBPS = DECSAN * (PASEFBPSDEC1 + PASEFBPSDEC2) ;


PASMODULEPS = DECSAN * CODSCV ;


ACNRPS = DECSAN * CODSDV ;


COBPS = (positif_ou_nul((PASMODULEPS + ACNRPS) - arr(PASCORRECTPS * TX90/100))) ; 

COBAPS = (positif_ou_nul((PASMODULEPS + ACNRPS) - arr(PASCORRECTPS * TX90/100)) + positif_ou_nul(PASEFFBPS - arr(PASCORRECTPS * TX70/100)) * (min(PASCORRECTPS,PASPASSIFPS - PASEFFBPS))) ;

SCBASPSB = max(0,DECSAN * (positif_ou_nul((PASMODULEPS + ACNRPS) - arr(PASCORRECTPS * TX90/100)) * 0
                  + (1-positif(COBPS)) * positif_ou_nul(PASEFFBPS - arr(PASCORRECTPS * TX70/100)) * (min(PASCORRECTPS,PASPASSIFPS - PASEFFBPS))
                  + (1-positif(COBAPS)) * (min(PASCORRECTPS,PASPASSIFPS - PASEFFBPS)))) ;


SANTXPS = positif_ou_nul((PASMODULEPS + ACNRPS) - arr(PASCORRECTPS * TX90/100)) + positif_ou_nul(PASEFFBPS - arr(PASCORRECTPS * TX70/100)) ;
SANPS = positif(PASCORRECTPS) * positif(PASCORRECTPS - PASEFFBPS) ;

SCTXPSB = DECSAN * positif(SCBASPSB) * positif(SANPS) * (positif_ou_nul((PASMODULEPS + ACNRPS) - arr(PASCORRECTPS * TX90/100)) * 0
        + positif_ou_nul(PASEFFBPS - arr(PASCORRECTPS * TX70/100)) * TX10
        + (1 - positif(SANTXPS)) * arr(TX050 * (min(PASCORRECTPS,PASPASSIFPS - PASEFFBPS)/min(PASCORRECTPS,PASPASSIFPS))*100 * 10)/10) ;

SCMTPSB = DECSAN * arr(SCBASPSB * SCTXPSB/100) ;
