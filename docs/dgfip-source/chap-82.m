...
RCMORDTOT = positif(COD2OP) * (RCMABD + RCMTNC + RCMAV + RCMHAD + RCMHAB + REGPRIV + COD2TT + COD2VV + COD2WW + COD2YY + COD2ZZ + COD2VN + COD2VO + COD2VP + COD2TQ + COD2TZ) 
          + (1-positif(COD2OP)) *  (RCMAV + COD2YY + COD2VN );
...
RAVC = positif(COD2OP)*(RCMAV + PROVIE + RCMLIB + COD2VV + COD2WW);
regle 821003:
application : iliad ;
...
RNET2DH = positif(COD2OP) * positif(RAVCNORM1) * max(0 ,RCMLIB - (ABTAV - ABT2CHRCH));

ABT2DH = positif(COD2OP) * positif(RAVCNORM1) * (RCMLIB - RNET2DH) + (1 - positif(RAVCNORM1)) * RCMLIB;

RNET2VV2WW = positif(COD2OP) * positif(RAVCNORM1) * positif(COD2VV + COD2WW) * max(0 , COD2VV + COD2WW - (ABTAV - ABT2CHRCH - ABT2DH));

regle 821004:
application : iliad ;


RCMORDTOTNET = positif(COD2OP) * (RCMRNABD + RCMRNTNC + RNET2CH + REGPRIVM + RCMHAB + RCMHAD + COD2TT + RNET2VV2WW + COD2YY + COD2ZZ + COD2VN + COD2VO + COD2VP + COD2TQ + COD2TZ) ;
...
RCMFRTEMP = min(RCMAB + RTNC + TRCMRNCH + RCMNAB + RTCAR + RCMPRIVM+COD2TT + COD2YY + COD2ZZ + COD2VN + COD2VO + COD2VP + COD2TQ + COD2TZ,RCMFR) ;
...
BRCMBISB = RCMRNABD + RCMRNTNC + RNET2CH + REGPRIVM + RCMHAB + RCMHAD + COD2TT + RNET2VV2WW + COD2YY + COD2ZZ + COD2VN + COD2VO + COD2VP + COD2TQ + COD2TZ ;
...
DEFRCMIMPU1 = min(DEFRCM + DEFRCM2 + DEFRCM3 + DEFRCM4 + DEFRCM5 + DEFRCM6,(BRCMBISB+BRCMBISQ-RCMFR));

DEFRCMIMPU2= min(DEFRCM + DEFRCM2 + DEFRCM3 + DEFRCM4 + DEFRCM5 + DEFRCM6,
                             max(BRCMBISB1731+BRCMBISQ1731,max(BRCMBISB_P + BRCMBISQ_P,BRCMBISB_PA + BRCMBISQ_PA))
                                               - min(RCMFR,max(RCMFR1731,RCMFR_P+RCMFR_PA)));

DEFRCMIMPU =  null(5-V_IND_TRAIT) * (positif(null(PREM8_11)*positif(DEFRCM + DEFRCM2 + DEFRCM3 + DEFRCM4 + DEFRCM5 + DEFRCM6))
                    * max(0,DEFRCMIMPU1 - DEFRCMIMPU2))
                + PREM8_11 * (DEFRCM + DEFRCM2 + DEFRCM3 + DEFRCM4 + DEFRCM5 + DEFRCM6)
            + 0;
regle 821080:
application : iliad ;
...
DFRCMN = positif(COD2OP)*(DFRCMNBIS * null(V_IND_TRAIT-4) + (RCMFR - RCMFRART1731+ abs(DEFRCMIMPU2) * (1-positif(DEFRCMIMPU2)) * (1-PREM8_11)) *  null(V_IND_TRAIT-5));
regle 821110:
application : iliad ;
...
REPRCMBIS = positif(COD2OP)*(( positif(positif(SOMMERCM_2)*null(PREM8_11) * positif(BRCMBISB + BRCMBISQ-RCMFR) + PREM8_11 * positif(BRCMBISB + BRCMBISQ-RCMFR))
           * max(0,REPRCM - DEFRCMIMPU)
             + (1-positif(positif(SOMMERCM_2)*null(PREM8_11) * positif(BRCMBISB + BRCMBISQ-RCMFR) + PREM8_11 * positif(BRCMBISB + BRCMBISQ-RCMFR)))
                * min(REPRCM,REPRCMB) + 0) * null(4-V_IND_TRAIT)
            + null(5-V_IND_TRAIT) * min(REPRCM,REPRCMB1731) + 0);
...
DFCM = positif(REPRCMBIS) * arr((REPRCMBIS * 1RCM_I)/ RCM_I) ;

DEFFFRCM = positif((REPRCM - RCMTOTNET)  + (COD2YY + COD2ZZ + COD2VN + COD2VO + COD2TQ + COD2TZ)) ; 

DFRCM5 =  null(4-V_IND_TRAIT) * (positif(COD2OP) * (positif(DEFFFRCM) * REPRCMAR
              + (1-positif(DEFFFRCM)) * positif(RCMFR) * (1-positif(DFCM)) * positif(RCMFR -RCMTOTNET) * DEFRCM6)
              + (1-positif(COD2OP)) * positif(DEFRCM6) * DEFRCM6) 
	      + null(5-V_IND_TRAIT) * min(DEFRCM6,REPRCM - REPRCMBIS1731) ;

DFRCM4 = null(4-V_IND_TRAIT) * (positif(COD2OP) * (positif(DEFFFRCM) * REPRCMAQ
              + (1-positif(DEFFFRCM)) * positif(RCMFR) * (1-positif(DFCM)) * positif(RCMFR -RCMTOTNET) * DEFRCM5)
              + (1-positif(COD2OP)) * positif(DEFRCM5) * DEFRCM5)
              + null(5-V_IND_TRAIT) * min(DEFRCM5,REPRCM - REPRCMBIS1731 - DFRCM5) ;

DFRCM3 = null(4-V_IND_TRAIT) * (positif(COD2OP) * (positif(DEFFFRCM) * REPRCMAN
              + (1-positif(DEFFFRCM)) * positif(RCMFR) * (1-positif(DFCM))  * positif(RCMFR -RCMTOTNET) * DEFRCM4)
              + (1-positif(COD2OP)) * positif(DEFRCM4) * DEFRCM4)
              + null(5-V_IND_TRAIT) * min(DEFRCM4 , REPRCM - REPRCMBIS1731 - DFRCM5 - DFRCM4 ) ;

DFRCM2 =  null(4-V_IND_TRAIT) * (positif(COD2OP) * (positif(DEFFFRCM) * REPRCMAM
              + (1-positif(DEFFFRCM)) * positif(RCMFR) * (1-positif(DFCM)) * positif(RCMFR -RCMTOTNET) * DEFRCM3)
              + (1-positif(COD2OP)) * positif(DEFRCM3) * DEFRCM3)
              + null(5-V_IND_TRAIT) * min(DEFRCM3 , REPRCM - REPRCMBIS1731 - DFRCM5 - DFRCM4-DFRCM3) ;

DFRCM1 = null(4-V_IND_TRAIT) * (positif(COD2OP) * (positif(DEFFFRCM)  * REPRCMAL
              + (1-positif(DEFFFRCM)) * positif(RCMFR) * (1-positif(DFCM)) * positif(RCMFR - RCMTOTNET) * DEFRCM2)
              + (1-positif(COD2OP)) * positif(DEFRCM2) * DEFRCM2)
              + null(5-V_IND_TRAIT) * min(DEFRCM2 , REPRCM-REPRCMBIS1731-DFRCM5-DFRCM4-DFRCM3-DFRCM2) ;

regle 821180:
application : iliad ;


RCM11 = positif(COD2OP)* (1-V_CNR) * (
              max(0,(1RCM_I-REPRCM1)) +0)+ abs(DEFRCMIMPU2) * (1-positif(DEFRCMIMPU2)) * (1-PREM8_11);
...
REVBRUTASSU = (1-positif(COD2OP)) * (RCMAV + PROVIE + RCMLIB + COD2VV + COD2WW) ;
...
RNABT2DH = (1-positif(COD2OP)) * max(0,RCMLIB-(ABTAV-ABT2CHRCH2)) ;

ABT2DH2 = (1-positif(COD2OP)) * (RCMLIB-RNABT2DH);

RNABT2VV = (1-positif(COD2OP)) * max(0, COD2VV-(ABTAV-(ABT2CHRCH2 + ABT2DH2))) ;

ABT2VV = (1-positif(COD2OP)) * (COD2VV-RNABT2VV);

RNABT2WW = (1-positif(COD2OP)) * max(0,COD2WW-(ABTAV-(ABT2CHRCH2 + ABT2DH2 + ABT2VV))) ;
...
RCMIMPTN = (1-positif(COD2OP)) * (1-positif( null(V_REGCO-2))) * arr(RCMABD + RCMTNC + RCMHAD + RCMHAB + COD2TT + REGPRIVMB + RNABT2WW + COD2ZZ + COD2VP + COD2TQ +  COD2TZ) ;


RCMIMPTR = (1-positif(COD2OP)) * (1-positif( null(V_REGCO-2)))*(RNABT2VV + COD2VO ) ;


RCMBAR = (1-positif(COD2OP)) * (RNABT2CH + COD2YY + COD2VN) ;
...
BPLIB = positif(COD2OP)*(ABT2DH) * (1 - V_CNR)
      + (1-positif(COD2OP))*(ABT2DH2) * (1-V_CNR) ;
