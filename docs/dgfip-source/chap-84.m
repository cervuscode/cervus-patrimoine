...
BAMICV = arr((BAMICNV + positif_ou_nul(COD5XB) * (BAMIC1V + BAMIC2V * positif(present(COD5XM) + present(V_BTBAMICV2)))) 
             / (1 + positif(present(COD5XM) + present(V_BTBAMICV2)) * (1 + positif(present(COD5YM) + present(V_BTBAMICV1))))) ;

BAMICC = arr((BAMICNC + positif_ou_nul(COD5YB) * (BAMIC1C + BAMIC2C * positif(present(COD5YT) + present(V_BTBAMICC2)))) 
             / (1 + positif(present(COD5YT) + present(V_BTBAMICC2)) * (1 + positif(present(COD5YU) + present(V_BTBAMICC1))))) ;

BAMICP = arr((BAMICNP + positif_ou_nul(COD5ZB) * (BAMIC1P + BAMIC2P * positif(present(COD5ZT) + present(V_BTBAMICP2)))) 
             / (1 + positif(present(COD5ZT) + present(V_BTBAMICP2)) * (1 + positif(present(COD5ZU) + present(V_BTBAMICP1))))) ;
...
BAHDEF = BAFORESTV + BAFPVV + BACREV  + BAFORESTC + BAFPVC + BACREC 
       + BAFORESTP + BAFPVP + BACREP  + 4BACREV  + 4BACREC + 4BACREP +0;
...
BAGF1A = BANV + BAMV + BANC + BAMC + BANP + BAMP  
         + (max(0,(4BACREV - 4DEFBACREV))*positif_ou_nul(BARAV))
         + (max(0,(4BACREC - 4DEFBACREC))*positif_ou_nul(BARAC))
         + (max(0,(4BACREP - 4DEFBACREP))*positif_ou_nul(BARAP)) ;
...
DEFBA1 = ((1-positif(BAHQNODEFF+BAQNODEFF)) * (abs(BAHQNODEFF+BAQNODEFF) - (DEFIBA))
                 ...
                 * (DAGRI5+DAGRI4+DAGRI3+DAGRI2+DAGRI1-BAHQNODEFF-BAQNODEFF-BAEV - BAEMV-BAEC - BAEMC-BAEP - BAEMP)
                 ...
                 +  (positif(DEFBANIF) * max(0,DEFBANIF - DBAIP) * positif(DEFBANIF+0)
                     + max(0,-(BAHQV+BAHQC+BAHQP+4BAQV+4BAQC+4BAQP+DEFIBA+DBAIP))* (1-positif(DEFBANIF+0))) * null(5 - V_IND_TRAIT);
...
SOMDEFBANI = max(0,BAFORESTV+BAFPVV+BACREV+BAFORESTC+BAFPVC+BACREC+BAFORESTP+BAFPVP+BACREP
...
DEFBANI = max(0,BAFORESTV+BAFPVV+BACREV+BAFORESTC+BAFPVC+BACREC+BAFORESTP+BAFPVP+BACREP
                 +4BACREV + 4BACREC + 4BACREP 
		 + COD5AK+ COD5BK+ COD5CK
                 + min(0,BAHQV+BAHQC+BAHQP+4BAQV+4BAQC+4BAQP) * (1-positif(SHBA-SEUIL_IMPDEFBA))) ;

DEFBANIH470 = max(0,BAFORESTV+BAFPVV+BACREV+BAFORESTC+BAFPVC+BACREC+BAFORESTP+BAFPVP+BACREP
                 +4BACREV + 4BACREC + 4BACREP
		 + COD5AK+ COD5BK+ COD5CK)
		 ;
...
PRORATABA = (4BACREV + 4BACREC +4BACREP -max(0,4BAQV+4BAQC+4BAQP+min(0,BAHQNODEFF)))/SOMDEFBANI+0;
regle 841290:
application : iliad ;
...
IMPDRFRAC = max(0 , IRNET + TAXANET + PCAPNET + HAUTREVNET + CDHRNET - IRESTITIR + AVRICIIR + CIADCREB3 - RASSALIR - RASACOIR + RASCTXIR + RASPENAC ) ; 
regle 841340:
application : iliad;
...
REVFRACDEC1 = positif_ou_nul(RONTOTDEC1-RN6DEC1) * RN6DEC1
            +  positif(RN6DEC1-RONTOTDEC1) * max(0,(BANV + BAEV - DEFANTBAV)) ;

REVFRACDEC2 = positif_ou_nul(RONTOTDEC2-RN6DEC2) * RN6DEC2
            +  positif(RN6DEC2-RONTOTDEC2) * max(0,(BANC + BAEC - DEFANTBAC)) ;       

REVFRACPAC = positif_ou_nul(RONTOTDEC3-RN6DEC3) * RN6DEC3
           +  positif(RN6DEC3 - RONTOTDEC3) * max(0,(BANP + BAEP - DEFANTBAP)) ;
