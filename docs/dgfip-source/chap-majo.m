...
regle 23111:
application : iliad ;
IRBASE = (1-ANNUL2042) * (min(0 , IAN + AVFISCOPTER - IRE + CODZRA - CODCOA)
         + max(0 , IAN + AVFISCOPTER - IRE - CODCOA + CODZRA) * positif(IAMD1 + V_ANTREIR + 1 - SEUIL_61) - IRANT
                             - max(IR9YI_P,IR9YI_PA) 
                                                + max(0,min(SOMPASIR_REF,max(SOMPASIR_A,SOMPASIR_P+SOMPASIR_PA))- SOMPASIR)
			     * positif(DIFFSOMPASIR)
...
TAXABASE_MAJO = (1-ANNUL2042) * (max(0,TAXASSUR-CODCOU)-max(TAXA9YI_P,TAXA9YI_PA) + max(0,max(SOMPASTAXA_A,max(SOMPASTAXA_P,SOMPASTAXA_P)) - SOMPASTAXA)
			     * positif(DIFFSOMPASIR)
                        ) * positif(IAMD1 + 1 + V_ANTREIR - SEUIL_61)*(1-APPLI_BATCH);
CAPBASE_MAJO = (1-ANNUL2042) * (max(0,IPCAPTAXT-CODCOV)-max(CAP9YI_P,CAP9YI_PA)+ max(0,max(SOMPASCAP_A,max(SOMPASCAP_P,SOMPASCAP_PA)) - SOMPASCAP)
			     * positif(DIFFSOMPASIR)
                         ) * positif(IAMD1 + 1 + V_ANTREIR - SEUIL_61);
HRBASE_MAJO = (1-ANNUL2042) * (max(0,IHAUTREVT+CHRPVIMP-CODCOX)-max(CHR9YI_P,CHR9YI_PA)+ max(0,max(SOMPASCHR_A,max(SOMPASCHR_P,SOMPASCHR_PA)) - SOMPASCHR)
			     * positif(DIFFSOMPASIR)
                      ) * positif(IAMD1  + 1 + V_ANTREIR - SEUIL_61);

CSBASE_MAJO = (1-ANNUL2042) * (max(0 , CSG - CSGIM - CODCOB)-max(CS9YP_P,CS9YP_PA)+ max(0,max(SOMPASCS_A,max(SOMPASCS_P,SOMPASCS_PA)) - SOMPASCS)
			     * positif(DIFFSOMPASCS)
                         ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);

RDBASE_MAJO = (1-ANNUL2042) * (max(0 , RDSN - CRDSIM - CODCOR)-max(RD9YP_P,RD9YP_PA)+ max(0,max(SOMPASRD_A,max(SOMPASRD_P,SOMPASRD_PA)) - SOMPASRD)
			     * positif(DIFFSOMPASCS)
                         ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);

PSOLBASE_MAJO = (1-ANNUL2042) * (max(0 , PSOL - PRSPROV - CODCOD)-max(PS9YP_P,PS9YP_PA)+ max(0,max(SOMPASPS_A,max(SOMPASPS_P,SOMPASPS_PA)) - SOMPASPS)
			     * positif(DIFFSOMPASCS)
                       ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);

CVNBASE_MAJO = (1-ANNUL2042) * (max(0,CVNSALC - CICVN - COD8YT-CODCOE)-max(CVN9YP_P,CVN9YP_PA)+ max(0,max(SOMPASCVN_A,max(SOMPASCVN_P,SOMPASCVN_PA)) - SOMPASCVN)
			     * positif(DIFFSOMPASCS)
                         ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);

CDISBASE_MAJO = (1-ANNUL2042) * (max(0,CDIS - CDISPROV-CODCOF)-max(CDIS9YP_P,CDIS9YP_PA)+ max(0,max(SOMPASCDIS_A,max(SOMPASCDIS_P,SOMPASCDIS_PA)) - SOMPASCDIS)
			     * positif(DIFFSOMPASCS)
                        ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);

GLOBASE_MAJO = (1-ANNUL2042) * (max(0,CGLOA - COD8YL-CODCOG)-max(GLO9YP_P,GLO9YP_PA)+ max(0,max(SOMPASGLO_A,max(SOMPASGLO_P,SOMPASGLO_PA)) - SOMPASGLO)
			     * positif(DIFFSOMPASCS)
                         ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);
C820BASE_MAJO = (1-ANNUL2042) * (max(0,MCSG820-CODCOQ)-max(C8209YP_P,C8209YP_PA)+ max(0,max(SOMPASC820_A,max(SOMPASC820_P,SOMPASC820_PA)) - SOMPASC820)
			     * positif(DIFFSOMPASCS)
                       ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);

RSE1BASE_MAJO = (1-ANNUL2042) * (max(0,RSE1N - CSPROVYD-CODCOT)-max(RSE19YP_P,RSE19YP_PA)+ max(0,max(SOMPASRSE1_A,max(SOMPASRSE1_P,SOMPASRSE1_PA)) - SOMPASRSE1)
			     * positif(DIFFSOMPASCS)
                       ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);

RSE2BASE_MAJO = (1-ANNUL2042) * (max(0,max(0, RSE8TV - CIRSE8TV - CSPROVYF) + RSE8QV + max(0, RSE8SA - CIRSE8SA - CSPROVYN)-CODCOL)-max(RSE29YP_P,RSE29YP_PA)+ max(0,max(SOMPASRSE2_A,max(SOMPASRSE2_P,SOMPASRSE2_PA)) - SOMPASRSE2)
			     * positif(DIFFSOMPASCS)
                   ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);

RSE3BASE_MAJO = (1-ANNUL2042) * (max(0,RSE3N - CSPROVYG-CODCOM)-max(RSE39YP_P,RSE39YP_PA)+ max(0,max(SOMPASRSE3_A,max(SOMPASRSE3_P,SOMPASRSE3_PA)) - SOMPASRSE3)
			     * positif(DIFFSOMPASCS)
                            ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);
RSE4BASE_MAJO = (1-ANNUL2042) * (max(0,RSE4N - CSPROVYH - CSPROVYP-CODCOO)-max(RSE49YP_P,RSE49YP_PA)+ max(0,max(SOMPASRSE4_A,max(SOMPASRSE4_P,SOMPASRSE4_PA)) - SOMPASRSE4)
			     * positif(DIFFSOMPASCS)
                        ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);

RSE5BASE_MAJO = (1-ANNUL2042) * (max(0,RSE5N - CSPROVYE-CODCOJ)-max(RSE59YP_P,RSE59YP_PA)+ max(0,max(SOMPASRSE5_A,max(SOMPASRSE5_P,SOMPASRSE5_PA)) - SOMPASRSE5)
			     * positif(DIFFSOMPASCS)
                        ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);

RSE6BASE_MAJO = (1-ANNUL2042) * (max(0,RSE6N-CODCOP)-max(RSE69YP_P,RSE69YP_PA) + max(0,max(SOMPASRSE6_A,max(SOMPASRSE6_P,SOMPASRSE6_PA)) - SOMPASRSE6)
			     * positif(DIFFSOMPASCS)
                        ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);
RSE8BASE_MAJO = (1-ANNUL2042) * (max(0,RSE8N-COD8YV-COD8YX-CODCOH)-max(RSE89YP_P,RSE89YP_PA)+ max(0,max(SOMPASRSE8_A,max(SOMPASRSE8_P,SOMPASRSE8_PA)) - SOMPASRSE8)
			     * positif(DIFFSOMPASCS)
                        ) * positif_ou_nul(CSTOTSSPENA - SEUIL_61)*(1-APPLI_BATCH);
...
VARIRDROIT = max( 0 , TOT_REF - IRNIN_P - TAXA_P - CHR_P - PCAP_P  + (COD8NA * LIM1500 + COD8NB * LIM10000) * (1-positif(MAJOTOT35))
                ) + 0 ;
...
TIFI4BASE[0] =   positif(FLAG_RETARD) *
	          ...
	          + (1 - positif(FLAG_RETARD+FLAG_DEFAUT)) * TIFI4BASE[0] + 0
	      + positif(FLAG_DEFAUT) * min(0,IFITARDIF_A) + 0
	       ;
