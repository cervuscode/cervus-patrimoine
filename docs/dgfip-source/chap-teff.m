...
ABICPNETV =  max(0,AUTOBICVV + AUTOBICPV - max(0,TMIB_AUTOAVV-TMIB_ABNPVLV) -max(0,TMIB_AUTOAPV-TMIB_ABNPPLV));
ABICPNETC =  max(0,AUTOBICVC + AUTOBICPC - max(0,TMIB_AUTOAVC-TMIB_ABNPVLC) -max(0,TMIB_AUTOAPC-TMIB_ABNPPLC));
ABICPNETP =  max(0,AUTOBICVP + AUTOBICPP - max(0,TMIB_AUTOAVP-TMIB_ABNPVLP) -max(0,TMIB_AUTOAPP-TMIB_ABNPPLP));
...
TBICPROONCV=BIPTAV+(TMIB_NETVV+TMIB_NETPV)+MIB_NETCV;
TBICPROONCC=BIPTAC+(TMIB_NETVC+TMIB_NETPC)+MIB_NETCC;
TBICPROONCP=BIPTAP+(TMIB_NETVP+TMIB_NETPP)+MIB_NETCP;
...
TBICNPONCV=BINTAV+(TMIB_NETNPVV+TMIB_NETNPPV)+MIBNPPVV-MIBNPDCT;
TBICNPONCC=BINTAC+(TMIB_NETNPVC+TMIB_NETNPPC)+MIBNPPVC-COD5RZ;
TBICNPONCP=BINTAP+(TMIB_NETNPVP+TMIB_NETNPPP)+MIBNPPVP-COD5SZ;
...
regle 99991023:
application : iliad   ;                          
TBNCPQNCV = max(0,TR1CQC );
TBNCPQNCC = max(0,TR1CRC );
TBNCPQNCP = max(0,TR1CSC );
...
TBNCPQCV =   positif(TBNCPHQNCV) * (TR1CQC)
           + (1-positif(TBNCPHQNCV)) * max(0,TR1CQC+TBNCPHQNCV);
TBNCPQCC =   positif(TBNCPHQNCC) * (TR1CRC)
	              + (1-positif(TBNCPHQNCC)) * max(0,TR1CRC+TBNCPHQNCC);
TBNCPQCP =   positif(TBNCPHQNCP) * (TR1CSC)
		                 + (1-positif(TBNCPHQNCP)) * max(0,TR1CSC+TBNCPHQNCP);
...
TBNCNPQCV =   positif(TBNCNPHQNCV) * (R1CJG)
           + (1-positif(TBNCNPHQNCV)) * max(0,R1CJG+TBNCNPHQNCV);
TBNCNPQCC =   positif(TBNCNPHQNCC) * (R1CRF)
           + (1-positif(TBNCNPHQNCC)) * max(0,R1CRF+TBNCNPHQNCC);
TBNCNPQCP =   positif(TBNCNPHQNCP) * (R1CSF)
           + (1-positif(TBNCNPHQNCP)) * max(0,R1CSF+TBNCNPHQNCP);
...
TREV4 = BAQTOTAVIS +GLN4V + GLN4C+TREV4_1731+TBNCPQF+TBNCNPQF+
       BICQV+BICQC+BICQP+BIPTAQV+BIPTAQC+BIPTAQP;
regle 99992000:
application : iliad   ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2 = TSHALLOV + COD1PM + COD1TP + COD1NX + COD1AF + COD1AG + SALEXTV + ALLOV + BPCOSAV + GLDGRATV + COD1GB + COD1AA
          + COD1GF + COD1GG + COD1GHRET + COD1ADRET + CODRAF + CODRAG + CARTSV + REMPLAV + CODDAJ + CODEAJ ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2 = TSHALLOC + COD1QM + COD1UP + COD1OX + COD1BF + COD1BG + SALEXTC + ALLOC + BPCOSAC + GLDGRATC + CARTSC 
          + REMPLAC + CODDBJ + CODEBJ + COD1HB + COD1BA + COD1HF + COD1HG + COD1HHRET + COD1BDRET + CODRBF + CODRBG ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2 = TSHALLO1 + COD1CF + COD1CG + SALEXT1 + ALLO1 + CARTSP1 + REMPLAP1 + COD1IB + COD1CA + COD1IF + COD1IG + COD1IHRET + COD1CDRET + CODRCF + CODRCG ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2 = TSHALLO2 + COD1DF + COD1DG + SALEXT2 + ALLO2 + CARTSP2 + REMPLAP2 + COD1JB + COD1DA + COD1JF + COD1JG + COD1JHRET + COD1DDRET + CODRDF + CODRDG ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2 = TSHALLO3 + COD1EF + COD1EG + SALEXT3 + ALLO3 + CARTSP3 + REMPLAP3 + COD1EA + COD1KF + COD1KG + COD1KHRET + COD1EDRET + CODREF + CODRGG ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2 = TSHALLO4 + COD1FF + COD1FG + SALEXT4 + ALLO4 + CARTSP4 + REMPLAP4 + COD1FA + COD1LF + COD1LG + COD1LHRET + COD1FDRET + CODRFF + CODRFG ;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2=PRBR3+COD1EL+COD1EM+PENIN3+PALI3+CARPEP3+CODREZ+PENSALP3+COD1EH+PEBF3+CODREL+CODREM;
...
VARTMP2=0;
VARTMP1=0;
VARTMP2=PRBR4+COD1FL+COD1FM+PENIN4+PALI4+CARPEP4+CODRFZ+PENSALP4+COD1FH+PEBF4+CODRFL+CODRFM;
...
TBNCDF1 = ((1-positif_ou_nul(NOCEPIMP+TSPENETNPF)) * abs(NOCEPIMP+TSPENETNPF)
          ...
          + null(5-V_IND_TRAIT) * (
              ...
              + (1-positif(TDEFBNCNPF)) *  max(0,-(TSPENETNPF)));
...
regle 99991016:
application : iliad   ;
TDEFBNCNP = arr(TSPENETNPV+TSPENETNPC+TSPENETNPP+BNCNPPVV+BNCNPPVC+BNCNPPVP+BNCAABV+BNCAABC+BNCAABP
...
TDEFBNCNPF = (1-PREM8_11) * positif(positif(SOMMEBNCND_2) * positif(BNCDF_P +BNCDFP2 +BNCDF1731))
                     * (max(0,min(min(max(BNCDF_P +BNCDF7_P,BNCDFP2+BNCDF7P2),BNCDF1731+BNCDF71731),
                                        BNCNPDCT+COD5MD+COD5LD+AUTOBNCV+AUTOBNCC+AUTOBNCP+TDIDABNCNP+BNCAABV+BNCAABC+BNCAABP
                            +COD5XS+ COD5YS+ COD5ZS+ CODCJG+ CODCSF
                                -NOCEPIMPNV -NOCEPIMPNC -NOCEPIMPNP -BNNAAV -BNNAAC -BNNAAP
                                                              -(max(TDEFBNCNP1731,max(TDEFBNCNP_P,TDEFBNCNPP2)))
                                                               - max(0,arr(TSPENETNPV+TSPENETNPC+TSPENETNPP+AUTOBNCV+AUTOBNCC+AUTOBNCP+BNCNPPVV
                                                                     +BNCNPPVC+BNCNPPVP+BNCAABV
                                                                  +BNCAABC+BNCAABP
                                                                      + COD5XS +COD5YS+COD5ZS+ CODCJG+ CODCSF
                                                                                      -TDEFBNCNPP3)))))
                        + PREM8_11 * positif(DEFBNCNPH470) *
                          (BNCNPDCT+COD5MD+COD5LD+TDIDABNCNP+AUTOBNCV+AUTOBNCC+AUTOBNCP+BNCAABV+BNCAABC+BNCAABP
                            + COD5XS+ COD5YS+ COD5ZS+ CODCJG+ CODCSF
                                -NOCEPIMPNV -NOCEPIMPNC -NOCEPIMPNP-BNNAAV -BNNAAC -BNNAAP
...
regle 99993100:
application : iliad   ;
TDFBICNPF = max(0,MIBNPRNETV + MIBNPPVV+MIBNPRNETC + MIBNPPVC+MIBNPRNETP + MIBNPPVP+ BICREV + BICREC + BICREP - TDEFNPI - TBICNPF);
regle 99993110:
application : iliad   ;
TDEFBICNPF = (1-PREM8_11) * positif(positif(SOMMEBICND_2)* positif(DLMRN_P +DLMRNP2 +DLMRN1731))
                * (max(0,min(min(max(DLMRN_P+DLMRN7_P,DLMRNP2+DLMRN7P2),DLMRN1731+DLMRN71731),
                       (MIBNPDCT + COD5RZ + COD5SZ + TDEFNPI + BICREV + BICREC + BICREP 
		                                             + COD5UR + COD5VR + COD5WR 
							     + CODCNC + CODCOC + CODCPC 
							     - (BINNV + BINNC + BINNP + BINTAQV + BINTAQC + BINTAQP )
                         -(max(TDFBICNPF1731 , max(TDFBICNPF_P , TDFBICNPFP2)))
                 - max(0 , MIBNPRNETV + MIBNPPVV + MIBNPRNETC + MIBNPPVC + MIBNPRNETP + MIBNPPVP + BICREV + BICREC + BICREP 
		                                                                                 + COD5UR + COD5VR + COD5WR 
												 + CODCNC + CODCOC + CODCPC  -TDFBICNPFP3)))))

            + PREM8_11 * positif(MIBNPRNETV + MIBNPRNETC + MIBNPRNETP + MIBNPPVV + MIBNPPVC + MIBNPPVP + TDEFNPI + BICREV + BICREC + BICREP 
	                                                                                                         + COD5UR + COD5VR + COD5WR 
														 + CODCNC + CODCOC + CODCPC )
                       * (MIBNPDCT + COD5RZ + COD5SZ + TDEFNPI + BICREV  + BICREC + BICREP 
		                                                + COD5UR  + COD5VR + COD5WR 
								+ CODCNC  + CODCOC + CODCPC 
								- (BINNV + BINNC + BINNP + BINTAQV + BINTAQC + BINTAQP ) - min(MIBNPDCT , max(TDFBICNPF1731 , TDFBICNPFP2))) ;
regle 99993330:
application : iliad   ;
...
TBICNPF = (1-PREM8_11) * max(0,BINNV + BINNC + BINNP + TMIBNETNPTOT- TDEFNPI + TDEFBICNPF)
          + PREM8_11 * (positif(BICREV + BICREC + BICREP ) *(BINNV + BINNC + BINNP + TMIBNETNPTOT)
                        +(1-positif(BICREV + BICREC + BICREP )) * max(0,BINNV + BINNC + BINNP + TMIBNETNPTOT- TDEFNPI)) ;
...
TDEFBANI = max(0,BAFORESTV+BAFPVV+BACREV+BAFORESTC+BAFPVC+BACREC+BAFORESTP+BAFPVP+BACREP
                 +4BACREV + 4BACREC + 4BACREP 
...
TDFANTIMPU =  max(0,SOMDEFICIT-max(TDFANTPROV1731-TDEFBANI1731*(1-positif(TSHBA1731-SEUIL_IMPDEFBA))* positif(TSHBA-SEUIL_IMPDEFBA)
                         ...
                         +  PREM8_11 * ( max(0,min(FRNV,min(-1 * TTSPRVP2,-1 * TTSPRV1731)))
                                      ...
                                      + (BICNOV + BICNOC + BICNOP - (BIPNV+BIPNC+BIPNP))
                                      + (BNCREV + BNCREC + BNCREP - (BNRV+BNRC+BNRP))
...
TEFFREV =   INDTEFF * 
                  ...
                  + (RFNTEO * V_INDTEO + RRFI) * (1 - positif(COD4EA + COD4EB)) 
                                      + RFOTENET3
                                      + RMFORDINET 
...
RDCSGTEF = max (min(DDCSG , RBGTEF + TOTALQUO - SDDTEF) , 0) ;
RDCSGTEFRFR = max (min(DDCSG , RBGTEFRFR + TOTALQUO - SDDTEFRFR) , 0) ;
R6DRTEF = max(0,min(COD6DR,RBGTEF - RDCSGTEF));
R6DRTEFRFR = max(0,min(COD6DR,RBGTEFRFR - RDCSGTEFRFR));
RPALETEF = max(0,min(somme(i=1..4:min(NCHENFi,LIM_PENSALENF)+min(arr(CHENFi*MAJREV),LIM_PENSALENF))+COD6GZ + COD6EZ,
                                    RBGTEF - R6DRTEF - DDCSG+TOTALQUO-SDDTEF)) *(1-V_CNR);
RPALETEFRFR = max(0,min(somme(i=1..4:min(NCHENFi,LIM_PENSALENF)+min(arr(CHENFi*MAJREV),LIM_PENSALENF))+COD6GZ + COD6EZ,
                                    RBGTEFRFR - R6DRTEFRFR - DDCSG + TOTALQUO - SDDTEFRFR)) *(1-V_CNR);
RPALPTEF = max( min(TOTPA,RBGTEF- R6DRTEF- RPALETEF - DDCSG + TOTALQUO - SDDTEF) , 0 ) * (1 -V_CNR);
RPALPTEFRFR = max( min(TOTPA,RBGTEFRFR -R6DRTEFRFR- RPALETEFRFR - DDCSG + TOTALQUO - SDDTEFRFR) , 0 ) * (1 -V_CNR);
RFACCTEF = max( min(DDFA,RBGTEF- R6DRTEF - RPALETEF - RPALPTEF  - DDCSG + TOTALQUO - SDDTEF) , 0);
RFACCTEFRFR = max( min(DDFA,RBGTEFRFR - R6DRTEFRFR - RPALETEFRFR - RPALPTEFRFR  - DDCSG + TOTALQUO - SDDTEFRFR) , 0);
RDDIVTEF = max( min(DEDIV * (1 - V_CNR),RBGTEF - R6DRTEF - RPALETEF - RPALPTEF - RFACCTEF - DDCSG + TOTALQUO - SDDTEF ) , 0 );
RDDIVTEFRFR = max( min(DEDIV * (1 - V_CNR),RBGTEFRFR - R6DRTEFRFR - RPALETEFRFR - RPALPTEFRFR - RFACCTEFRFR - DDCSG + TOTALQUO - SDDTEFRFR ) , 0 );
RD6DGTEF = max( min(DED6DG * (1 - V_CNR),RBGTEF - R6DRTEF - RPALETEF - RPALPTEF - RFACCTEF - RDDIVTEF - DDCSG + TOTALQUO - SDDTEF ) , 0 );
RD6DGTEFRFR = max( min(DED6DG * (1 - V_CNR),RBGTEFRFR - R6DRTEFRFR - RPALETEFRFR - RPALPTEFRFR - RFACCTEFRFR - RDDIVTEFRFR - DDCSG + TOTALQUO - SDDTEFRFR ) , 0 );
APERPVTEF = (1 - V_CNR) * max(min(RPERPV,RBGTEF - RPALETEF - RPALPTEF - RFACCTEF
                                    - RDDIVTEF - R6DRTEF - RD6DGTEF - DDCSG + TOTALQUO -SDDTEF), 0);
APERPVTEFRFR = (1 - V_CNR) * max(min(RPERPV,RBGTEFRFR - RPALETEFRFR - RPALPTEFRFR - RFACCTEFRFR
                                    - RDDIVTEFRFR - R6DRTEFRFR - RD6DGTEFRFR - DDCSG + TOTALQUO -SDDTEFRFR), 0);
APERPCTEF = (1 - V_CNR) * max(min(RPERPC,RBGTEF - R6DRTEF - RPALETEF - RPALPTEF  - RFACCTEF
...
APERPCTEFRFR = (1 - V_CNR) * max(min(RPERPC,RBGTEFRFR - RPALETEFRFR - RPALPTEFRFR  - RFACCTEFRFR
                                    - RDDIVTEFRFR - R6DRTEFRFR - RD6DGTEFRFR - DDCSG + TOTALQUO -SDDTEFRFR - APERPVTEFRFR), 0);
APERPPTEF = (1 - V_CNR) * max(min(RPERPP,RBGTEF - RPALETEF - RPALPTEF  - RFACCTEF
                                    - RDDIVTEF - R6DRTEF - RD6DGTEF - DDCSG + TOTALQUO -SDDTEF - APERPVTEF - APERPCTEF), 0);
APERPPTEFRFR = (1 - V_CNR) * max(min(RPERPP,RBGTEFRFR - RPALETEFRFR - RPALPTEFRFR  - RFACCTEFRFR
                                    - RDDIVTEFRFR - R6DRTEFRFR - RD6DGTEFRFR - DDCSG + TOTALQUO -SDDTEFRFR - APERPVTEFRFR - APERPCTEFRFR), 0);
...
NUREPARTEF = min(NUPROPT , max(0,min(PLAF_NUREPAR, RRBGTEF - RPALETEF - RPALPTEF - RFACCTEF
                                    - RDDIVTEF - R6DRTEF - RD6DGTEF - APERPVTEF - APERPCTEF - APERPPTEF - DDCSG + TOTALQUO - SDDTEF)))
                                    * (1 - V_CNR) ;
NUREPARTEFRFR = min(NUPROPT , max(0,min(PLAF_NUREPAR, RRBGTEFRFR - RPALETEFRFR - RPALPTEFRFR - RFACCTEFRFR
                                    - RDDIVTEFRFR - R6DRTEFRFR - RD6DGTEFRFR - APERPVTEFRFR - APERPCTEFRFR - APERPPTEFRFR - DDCSG + TOTALQUO - SDDTEFRFR)))
                                    * (1 - V_CNR) ;
...
RBLTEF =  RBG2TEF - max(0,min( RBG2TEF , (DDPA + DDFA + RDDIVTEF + R6DRTEF + RD6DGTEF + APERPVTEF + APERPCTEF + APERPPTEF + NUREPARTEF )))  *(1-positif(RE168+TAX1649))* ( 1 - V_CNR )
                                    - min( RBG2TEF , V_8ZT+ CODZRE + CODZRF) * V_CNR;
RBLTEFRFR = RBG2TEFRFR - max(0,min( RBG2TEFRFR , (DDPA + DDFA + RDDIVTEFRFR + R6DRTEFRFR + RD6DGTEFRFR + APERPVTEFRFR + APERPCTEFRFR + APERPPTEFRFR + NUREPARTEFRFR ))) *(1-positif(RE168+TAX1649))* ( 1 - V_CNR )
                                    - min( RBG2TEFRFR , V_8ZT+ CODZRE + CODZRF) * V_CNR;
RNGTEF = RBLTEF ;
RNGTEFRFR = RBLTEFRFR ;
SDCTEF = max(0, DDCSG + DDPA + DDFA + RDDIVTEF + R6DRTEF + RD6DGTEF + APERPVTEF + APERPCTEF + APERPPTEF + NUREPARTEF - max(0,RBGTEF)) * (1 - V_CNR)
	          + max(0, V_8ZT+ CODZRE + CODZRF - max(0,RBGTEF)) * V_CNR ;
SDCTEFRFR = max(0, DDCSG + DDPA + DDFA + RDDIVTEFRFR + R6DRTEFRFR + RD6DGTEFRFR + APERPVTEFRFR + APERPCTEFRFR + APERPPTEFRFR + NUREPARTEFRFR - max(0,RBGTEF)) * (1 - V_CNR)
	          + max(0, V_8ZT+ CODZRE + CODZRF - max(0,RBGTEF)) * V_CNR ;
...
TEFFREVINTER =    INDTEFF * 
                  ...
                  + max(0,max(0,MLOCDECV-TMIB_ABNPVLV-TMIB_ABNPPLV)+max(0,MLOCDECC-TMIB_ABNPVLC-TMIB_ABNPPLC)+max(0,MLOCDECP-TMIB_ABNPVLP-TMIB_ABNPPLP)
                           +NPLOCNETV + NPLOCNETC + NPLOCNETPAC - TDNPLOCIMPU +DEFLOCNPF)
                  + (RFNTEO * V_INDTEO + RRFI * (1-V_INDTEO)) * (1 - positif(COD4EA + COD4EB))
		                         + RFOTENET3
		                         + RMFORDINET
...
TEFFREVTOT =    INDTEFF * 
                  ...
                  + (RFNTEO * V_INDTEO + RRFI * (1-V_INDTEO)) * (1 - positif(COD4EA + COD4EB))
		                         + RFOTENET3
		                         + RMFORDINET
         	   + COD1TZ + positif(COD3CN) * COD3AN +positif(COD2OP)* (BTPM3VG+ PVTAXSB + BTPM3TJ + BTPM3UA+COD3SZ+GAINPEA)
                  + PVIMPOS * null(1-FLAG_EXIT)+(PVIMPOS + PVSURSI) * null(2 -FLAG_EXIT)		
                  + max(BANOR,0) + REB +
                  min(BANOR,0) *
                  positif(SEUIL_IMPDEFBA + 1
                  -SHBA- (REVTP-BA1)
                  - REVQTOTQHT))
                  + R1649 - (DAR + max(0,min(TEFFREVINTER,DDPA + DDFA + RDDIVTEF + R6DRTEF + RD6DGTEF 
		  + APERPVTEF + APERPCTEF + APERPPTEF + NUREPARTEF + ABTPATEF + ABTMA + DDCSG))) *(1-positif(RE168+TAX1649))
                  + DFANTIMPU)
                  ;
regle 99991085:
application :  iliad   ;                          
TEFFREVTOTRFR =    INDTEFF * 
                  ...
                  + BIPTAV + BIPTAC + BIPTAP 
                  + ESFP + TTSPR + RCM1 
                  + max(0,max(0,MLOCDECV - TMIB_ABNPVLV - TMIB_ABNPPLV )
		  + max(0,MLOCDECC-TMIB_ABNPVLC-TMIB_ABNPPLC)
		  + max(0,MLOCDECP-TMIB_ABNPVLP-TMIB_ABNPPLP)
                  + NPLOCNETV + NPLOCNETC + NPLOCNETPAC - TDNPLOCIMPU  + DEFLOCNPF)
                  + (RFNTEO * V_INDTEO + RRFI * (1-V_INDTEO)) * (1 - positif(COD4EA + COD4EB))
		  + RFOTENET3 + RMFORDINET
                  ...
                  -SHBA- (REVTP-BA1)
                  - REVQTOTQHT))
                  + R1649 - (DAR + max(0,min(TEFFREVINTER,DDPA + DDFA + RDDIVTEFRFR +  R6DRTEFRFR  + RD6DGTEFRFR + NUREPARTEFRFR + ABTPATEFRFR + ABTMA+DDCSG))) *(1-positif(RE168+TAX1649))
                  + DFANTIMPU)
                  ;
regle 99991090:
application :  iliad   ;                          
TEFFREVTOTRFRHR =    INDTEFF * 
		  ...
		  +(positif(COD2OP) * BTPM3TJ)+(positif(COD2OP)* BTPM3UA* (1-positif(ABIMPMV)))
                  ...
                  -SHBA- (REVTP-BA1)
                  - REVQTOTQHT))
                  + R1649 - (DAR + max(0,min(TEFFREVINTERHR,R6DRTEFRFR + DDPA + DDFA + RDDIVTEFRFR + NUREPARTEFRFR + ABTPATEFRFR + ABTMA + DDCSG))) *(1-positif(RE168 + TAX1649))
                  +DFANTIMPU)
                  ;
