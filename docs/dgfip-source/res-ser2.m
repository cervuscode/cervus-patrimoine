...
BASANCPTIR2 = (CODST1 + CODST2 + 0) ;
SANACOMIR2 = (arr(BASANCPTIR2 * TXSANCTION/100) * (1-positif(CODST5))) + CODST5 ;

SANCTXD = 10 ;
BASANCPTPS2 = (CODST3 + CODST4 + 0);

SANACOMPS2 = (arr(BASANCPTPS2 * TXSANCTION/100) * (1-positif(CODST6))) + CODST6 ;

SCBASIR2 = (CODSU1+ CODSU2 + 0) * null(V_IND_TRAIT - 5);

SCTXIR2 = ((CODSV1) *positif(CODSU1) * (1-positif(CODSV2)) + (CODSV2) * positif(CODSU2) * (1-positif(CODSV1))
        + (-1) * ((CODSV1 + CODSV2) * positif(CODSV1 * CODSV2))) * null(V_IND_TRAIT - 5) ;
SCMSU1 = arr(CODSU1 * CODSV1/100) ;
SCMSU2 = arr(CODSU2 * CODSV2/100) ;


SCMTIR2 = ((SCMSU1 + SCMSU2) * (1-positif(CODSZ1))) + CODSZ1 ;

SCBASPS2 = (CODSW1+ CODSW2 + 0) * null(V_IND_TRAIT - 5) ;
SCTXPS2 = ((CODSX1) * positif(CODSW1) * (1-positif(CODSX2)) + (CODSX2) * positif(CODSW2) * (1-positif(CODSX1))
          + (-1) * ((CODSX1 + CODSX2)  *positif(CODSX1 * CODSX2))) * null(V_IND_TRAIT - 5);
SCMSW1 = arr(CODSW1 * CODSX1/100) ;
SCMSW2 = arr(CODSW2 * CODSX2/100) ;

SCMTPS2 = (SCMSW1 + SCMSW2) * (1-positif(CODSZ2)) + CODSZ2 ;

PENAPAS2 = (SANACOMIR2 + SANACOMPS2 +  SCMTIR2 +  SCMTPS2) * null(V_IND_TRAIT - 5) ;

SANCPAS = (CODST1 + CODST2 + CODST3 + CODST4 + CODST5 +  CODST6 + CODSU1  + CODSU2 + CODSV1 + CODSV2 + CODSW1 + CODSW2 + CODSX1 + CODSX2 + CODSZ1 + CODSZ2 ) ;
regle 221030:
application : iliad  ;
...
PRELIB = PPLIB + RCMLIB + COD2XX + COD2VM ;
...
RETRSE2 = (RETRSE22 + arr(max(0, RSE2N-RSE29YP-CSPROVYF) * TXINT/100)) * positif_ou_nul(CSTOTSSPENA - SEUIL_61);			
...
NMAJRSE21 = max(0,MAJO1728RSE2 + arr(max(0, RSE2N-RSE29YP-CSPROVYF) * COPETO/100) * (1 - V_CNR)                                                                                                                         
...
NMAJ4    =      (somme (i=03..06,30,32,35,55: MAJOIRi)) + (COD8NA * LIM1500 + COD8NB * LIM10000) * (1-positif(MAJOTOT35)) * (1-positif(INDCODIFI))
...
IAVIM = IRB + PTOT + TAXASSUR + PTAXA + IPCAPTAXTOT + PPCAP + CHRAPRES + CHRPVIMP + PHAUTREV + CDHR + PCDHR;
...
IMPETAL20 = (V_BTIMPETAL0 * (1-positif(CODBJS)) + CODBJS);
IMPETAL21 = (V_BTIMPETAL1 * (1-positif(CODBKS)) + CODBKS);
IMPETAL22 = (V_BTIMPETAL2 * (1-positif(CODBLS)) + CODBLS);
IMPETAL23 = (V_BTIMPETAL3 * (1-positif(CODBMS)) + CODBMS);
regle 221220:
application : iliad  ;
...
CDHRNEG = (min(0 , CDHRACI + PCDHR + min(0 , CHRNEG)))*0 ;
CDHRNETB = (positif(CDHRACI) * max(0 , CDHRACI + PCDHR+ min(0 , CHRNEG)))*0 ;
CDHRNET = ((null(NRINET + IMPRET + CODZRA + BRASAR + 0) * CDHRNETB
              +
              positif(NRINET + IMPRET + CODZRA + BRASAR + 0)
              * ( positif_ou_nul(IAMD1 - SEUIL_61) * CDHRNETB
              + (1 - positif_ou_nul(IAMD1 - SEUIL_61)) * 0 )
              ) * (1-ANNUL2042) )*0
              ;
...
IRESTIT = abs(min(0 , IRN + PIR + NRINET + IMPRET + CODZRA + BRASAR 
                    ...
                    + ((CDHR + PCDHR) 
                      -min((CDHR + PCDHR),max(0,INE-IRB+AVFISCOPTER-TAXASSUR-PTAXA-IPCAPTAXT-PPCAP-IHAUTREVT- PHAUTREV-CHRPVIMP)))
...
IRESTITIR = abs(min(0 , IRN + PIR + NRINET + IMPRET + CODZRA + BRASAR
                    ...
                    + ((CDHR + PCDHR) -min((CDHR + PCDHR),max(0,INE-IRB+AVFISCOPTER-TAXASSUR-PTAXA-IPCAPTAXT-PPCAP-IHAUTREVT - PHAUTREV-CHRPVIMP)))
                   )
               ) * (1 - ANNUL2042) ;
...
TOTREC = positif_ou_nul(IRN + TAXANET + PIR + PCAPNET + HAUTREVNET + CDHRNET - SEUIL_12) ;
...
INDTXMOY = positif(positif(TX_MIN_MET - TMOY) * positif_ou_nul(max(0 , LIM_BAR3 - BMI))
           ...
           * positif( (present(RMOND) + present(DMOND))*null(4-V_IND_TRAIT)
	              +(positif(RMOND) + positif(DMOND))*null(5-V_IND_TRAIT) ) * V_CNR
...
INDNMR =  null(NAPT) * null(NAT1BIS) * (positif (IRNET + TAXANET + PCAPNET + HAUTREVNET + CDHRNET)) ;
...
IND_REST50 = positif(SEUIL_8 - IREST) * positif(IREST);
...
INDV = positif_ou_nul(ALLOV + REMPLAV + REMPLANBV + BACDEV + BACREV + 4BACREV + BAFPVV + BICDEV + BICDNV + BICNOV + BICREV + BNCAADV + BNCAABV + BNCDEV 
		      + BNCNPPVV + BNCNPV + BNCPROPVV + BNCPROV + BNCREV + BPCOSAV + CARPENBAV + CARPEV + CARTSNBAV + CARTSV + COTFV + FRNV + GLDGRATV 
		      + MIBNPPRESV + MIBNPPVV + MIBNPVENV + MIBPRESV + MIBPVV + MIBVENV + PALIV + PENSALV + PENSALNBV + PEBFV + PRBRV + TSHALLOV 
		      + BAFORESTV + LOCNPCGAV + LOCDEFNPCGAV + MIBMEUV + MIBGITEV + BICPMVCTV + BNCPMVCTV + LOCGITV + PENINV + CODRAZ + CODDAJ + CODEAJ 
		      + COD1AF + COD1AG + COD1AL + COD1AM + COD1GB + CODRAF + CODNAF + CODRAG + CODNAG + CODRAL + CODNAL + CODRAM + CODNAM + COD5AK 
		      + COD5AQ + COD5AY + COD5DF + COD5DK + COD5DM + COD5EY + BNCNPDCT + COD5UR + COD5UT + COD5UY + COD5VM + COD5XA + COD5XB + COD5XH 
		      + COD5XJ + COD5XN + COD5XO + COD5XP + COD5XR + COD5XS + COD5XY + CODCJG + CODCKC + CODCNC + COD5NW + CODCQC + COD1AA + COD1GE 
		      + COD1GF + COD5XM + COD5YM + COD1AI + COD1GG + COD1GH + COD1GK + COD1GL + COD5EA + COD5HA + COD5QA + COD5QJ + COD5TF 
		      + COD5UI + CODRAI + COD5WE + COD1AD + COD1AV + COD1PB + COD5AC + COD5AE + COD5AG + COD5AT + COD5AX 
                     ) ;

INDC = positif_ou_nul(ALLOC + REMPLAC + REMPLANBC + BACDEC + BACREC + 4BACREC + BAFPVC + BICDEC + BICDNC + BICNOC + BICREC + BNCAADC + BNCAABC + BNCDEC 
                      + BNCNPC + BNCNPPVC + BNCPROC + BNCPROPVC + BNCREC + BPCOSAC + CARPEC + CARPENBAC + CARTSC + CARTSNBAC + COTFC + FRNC + GLDGRATC 
		      + MIBNPPRESC + MIBNPPVC + MIBNPVENC + MIBPRESC + MIBPVC + MIBVENC + PALIC + PENSALC + PENSALNBC + PEBFC + PRBRC + TSHALLOC 
		      + BAFORESTC + LOCNPCGAC + LOCDEFNPCGAC + MIBMEUC + MIBGITEC + BICPMVCTC + BNCPMVCTC + LOCGITC + PENINC + CODRBZ + CODDBJ + CODEBJ 
		      + COD1BF + COD1BG + COD1BL + COD1BM + COD1HB + CODRBF + CODNBF + CODRBG + CODNBG + CODRBL + CODNBL + CODRBM + CODNBM + COD5BK 
		      + COD5BQ + COD5BY + COD5EF + COD5EK + COD5EM + COD5FY + COD5LD + COD5RZ + COD5VR + COD5VT + COD5VY + COD5WM + COD5YA + COD5YB 
		      + COD5YH + COD5YJ + COD5YN + COD5YO + COD5YP + COD5YR + COD5YS + COD5YY + CODCLC + CODCOC + COD5OW + CODCRC + CODCRF + COD1BA 
		      + COD1HE + COD1HF + COD5YT + COD5YU + COD1BI + COD1GL + COD1HG + COD1HH + COD1HL + COD5EI + COD5IA + COD5RA + COD5RJ + COD5UF 
		      + COD5VI + CODRBI + COD5XE + COD1BD + COD1BV + COD1PC + COD5BC + COD5BE + COD5BG + COD5BT + COD5BX 
                     ) ;

INDP = positif_ou_nul(ALLO1 + ALLO2 + ALLO3 + ALLO4 + CARTSP1 + CARTSP2 + CARTSP3 + CARTSP4 + CARTSNBAP1 + CARTSNBAP2 + CARTSNBAP3 + CARTSNBAP4 
                      + REMPLAP1 + REMPLAP2 + REMPLAP3 + REMPLAP4 + REMPLANBP1 + REMPLANBP2 + REMPLANBP3 + REMPLANBP4 + BACDEP + BACREP + 4BACREP 
		      + BAFPVP + BICDEP + BICDNP + BICNOP + BICREP  + BNCAADP + BNCAABP + BNCDEP + BNCNPP + BNCNPPVP + BNCPROP + BNCPROPVP 
		      + BNCREP + COTF1 + COTF2 + COTF3 + COTF4 + FRN1 + FRN2 + FRN3 + FRN4 + MIBNPPRESP + MIBNPPVP + MIBNPVENP + MIBPRESP 
		      + MIBPVP + MIBVENP + PALI1 + PALI2 + PALI3 + PALI4 + PENSALP1 + PENSALP2 + PENSALP3 + PENSALP4 + PENSALNBP1 + PENSALNBP2 
		      + PENSALNBP3 + PENSALNBP4 + PEBF1 + PEBF2 + PEBF3 + PEBF4 + PRBR1 + PRBR2 + PRBR3 + PRBR4 + CARPEP1 + CARPEP2 + CARPEP3 
		      + CARPEP4 + CARPENBAP1 + CARPENBAP2 + CARPENBAP3 + CARPENBAP4 + TSHALLO1 + TSHALLO2 + TSHALLO3 + TSHALLO4 + BAFORESTP + LOCNPCGAPAC 
		      + LOCDEFNPCGAPAC + MIBMEUP + MIBGITEP  + BICPMVCTP + BNCPMVCTP + LOCGITP + COD1CF + COD1CG + COD1CL + COD1CM + COD1DF + COD1DG 
		      + COD1DL + COD1DM + COD1EF + COD1EG + COD1EL + COD1EM + COD1FF + COD1FG + COD1FL + COD1FM + COD1IB + COD1JB + CODRCF + CODNCF 
		      + COD1CI + COD1DI + COD1EI + COD1FI + CODRCG + CODNCG + CODRDF + CODNDF + CODRDG + CODNDG + CODREF + CODNEF + CODRGG + CODNGG 
		      + CODRFF + CODNFF + CODRFG + CODNFG + CODRCL + CODNCL + CODRCM + CODNCM + CODRDL + CODNDL + CODRDM + CODNDM + CODREL + CODNEL 
		      + CODREM + CODNEM + CODRFL + CODNFL + CODRFM + CODNFM + PENIN1 + PENIN2 + PENIN3 + PENIN4 + CODRCZ + CODRDZ + CODREZ + CODRFZ 
		      + COD5CK + COD5FF + COD5GY + COD5MD + COD5SZ + COD5WR + COD5ZA + COD5ZB + COD5ZJ + COD5ZN + COD5ZO + COD5ZS + COD5AH + COD5BH 
		      + COD5CM + COD5CN + COD5CQ + COD5CR + COD5CU + COD5CV + COD5ED + COD5FB + COD5FD + COD5FK + COD5FM + CODCMC + CODCPC + COD5PW 
		      + CODCSC + CODCSF + COD5TP + COD5VQ + COD5VV + COD5ZH + COD5ZI + COD5ZM + COD5ZP + COD5ZR + COD5ZY + COD1CA + COD1DA + COD1EA 
		      + COD1FA + COD1IE + COD1IF + COD1JE + COD1JF + COD1KE + COD1KF + COD1LE + COD1LF + COD5ZT + COD5ZU + COD1GP + COD1GQ + COD1GR 
		      + COD1GS + COD1HP + COD1HQ + COD1HR + COD1HS + COD1IG + COD1IH + COD1JG + COD1JH + COD1KG + COD1KH + COD1LG + COD1LH + COD5EU 
		      + COD5JA + COD5SA + COD5SJ + COD5VF + COD5WI + CODRCK + COD5YE + COD1CD + COD1CV + COD1DD + COD1DV + COD1ED + COD1EV + COD1FD 
		      + COD1FV + COD1PD + COD1PE + COD1PF + COD1PG + COD5CC + COD5CE + COD5CG + COD5CT + COD5CX 
                     ) ;
...
INDREV1A8BIS = positif(
              positif(4BACREC) + positif(4BACREP) + positif(4BACREV) + positif(ABDETPLUS) + positif(ALLO1) + positif(ALLO2) + positif(ALLO3) + positif(ALLO4) 
	    + positif(ALLOC) + positif(ALLOV) + positif(AUTOBICPC) + positif(AUTOBICPP) + positif(AUTOBICPV) + positif(AUTOBICVC) + positif(AUTOBICVP) 
	    ...
	    + positif(BACREP) + positif(BACREV) + positif(BAEXC) + positif(BAEXP) + positif(BAEXV) + positif(BAF1AC) + positif(BAF1AP) + positif(BAF1AV) + positif(BAFORESTC) + positif(BAFORESTP) + positif(BAFORESTV) 
            + positif(BAFPVC) + positif(BAFPVP) + positif(BAFPVV) 
	    ...
	    + positif(BICNOC) + positif(BICNOP) + positif(BICNOV) + positif(BICNPEXC) + positif(BICNPEXP) + positif(BICNPEXV) 
	    + positif(BICREC) + positif(BICREP) + positif(BICREV) 
	    + positif(BN1AC) + positif(BN1AP) + positif(BN1AV) + positif(BNCAABC) + positif(BNCAABP) + positif(BNCAABV) + positif(BNCAADC) + positif(BNCAADP) + positif(BNCAADV)
	    ...
	    + positif(BNCEXV) + positif(BNCNP1AC) + positif(BNCNP1AP) + positif(BNCNP1AV) + positif(BNCNPC) + positif(BNCNPDCT) + positif(BNCNPDEC) + positif(BNCNPDEP) + positif(BNCNPDEV) + positif(BNCNPP) 
            + positif(BNCNPPVC) + positif(BNCNPPVP) + positif(BNCNPPVV) + positif(BNCNPREXAAC) + positif(BNCNPREXAAP) + positif(BNCNPREXAAV) 
	    + positif(BNCNPV) + positif(BNCPRO1AC) + positif(BNCPRO1AP) + positif(BNCPRO1AV) + positif(BNCPROC) + positif(BNCPMVCTV) + positif(BNCPMVCTC) + positif(BNCPMVCTP) 
            ...
            + positif(BNCPROPVV) + positif(BNCPROV) + positif(BNCREC) + positif(BNCREP) + positif(BNCREV)  
	    + positif(BPCOPTV) + positif(BPCOSAC) + positif(BPCOSAV) + positif(BPV18V) + positif(BPV40V) + positif(BPVRCM) + positif(CARPEC) + positif(CARPENBAC) 
	    ...
	    + positif(DEFBIC3) + positif(DEFBIC4) + positif(DEFBIC5) + positif(DEFBIC6) + positif(DPVRCM) + positif(FRN1) + positif(FRN2) + positif(FRN3) + positif(FRN4) 
	    + positif(FRNC) + positif(FRNV) + positif(GAINABDET) + positif(GLDGRATV) + positif(GLDGRATC) + positif(LOCDEFNPCGAC) + positif(LOCDEFNPCGAPAC) + positif(LOCDEFNPCGAV) 
	    + positif(LOCNPCGAC) + positif(LOCNPCGAPAC) + positif(LOCNPCGAV) + positif(MIB1AC) 
	    ...
	    + positif(RCMABD) + positif(RCMAV) + positif(RCMAVFT) + positif(RCMFR) + positif(RCMHAB) + positif(COD2TZ) + positif(RCMHAD) + positif(RCMLIB) + positif(RCMRDS) + positif(RCMSOC) + positif(RCMTNC) + positif(RCSC) + positif(RCSP) 
      ...
      + positif(PVIMPOS) + positif(INVENTV) + positif(INVENTC) + positif(INVENTP) + positif(LOCGITV) + positif(LOCGITC) + positif(LOCGITP) 
      + positif(LOCGITCV) + positif(LOCGITCC) + positif(LOCGITCP) + positif(PVTAXSB) + positif(PVMOBNR) + positif(BPVSJ) + positif(BPVSK) 
  ...
  + positif(COD1DE + COD1EE + COD1FE + COD1AH + COD1BH + COD1CH + COD1DH + COD1EH + COD1FH + COD1TZ + COD1AF + COD1AG + COD1AL + COD1AM + COD1AR + COD1BF 
            + COD1BG + COD1BL + COD1BM + COD1BR + COD1CF + COD1CG + COD1CL + COD1CM + COD1CR + COD1DF + COD1DG + COD1DL + COD1DM + COD1DR + COD1EF + COD1EG 
            + COD1EL + COD1EM + COD1FF + COD1FG + COD1FL + COD1FM + COD1NX + COD1OX + COD1PM + COD1QM + COD1TP + COD1UP + COD1UZ + COD1VZ + COD1GB + COD1HB 
            + COD1IB + COD1JB + CODRAF + CODNAF + CODRAG + CODNAG + CODRBF + CODNBF + CODRBG + CODNBG + CODRCF + CODNCF + CODRCG + CODNCG + CODRDF + CODNDF 
            + CODRDG + CODNDG + CODREF + CODNEF + CODRGG + CODNGG + CODRFF + CODNFF + CODRFG + CODNFG + CODRAL + CODNAL + CODRAM + CODNAM + CODRAR + CODNAR 
            + CODRBL + CODNBL + CODRBM + CODNBM + CODRBR + CODNBR + CODRCL + CODNCL + CODRCM + CODNCM + CODRCR + CODNCR + CODRDL + CODNDL + CODRDM + CODNDM 
            + CODRDR + CODNDR + CODREL + CODNEL + CODREM + CODNEM + CODRFL + CODNFL + CODRFM + CODNFM + COD1AA + COD1BA + COD1CA + COD1DA + COD1EA + COD1FA
            + COD1GE + COD1HE + COD1IE + COD1JE + COD1KE + COD1LE + COD1GF + COD1HF + COD1IF + COD1JF + COD1KF + COD1LF + COD2CK + COD2TT + COD2TU + COD2OP 
            + COD2TV + COD2TW + COD2UU + COD2VV + COD2VM + COD2VN + COD2VO + COD2VP + COD2TQ + COD2VQ + COD2VR + COD2WW + COD2XX + COD3CN + COD2YY + COD2ZZ 
	    + COD3SL + COD3UA + COD3WM + COD3WI + COD3WJ + COD3PI + COD3SG + COD3SZ + COD3TJ + COD3WG + COD3WN + COD3WP + COD3WR + COD3WT + COD3XA + COD3XD
	    + COD3WS 
	    + COD3XM + COD3XN + COD3YA + COD4BK + COD4BL + COD4BN + COD5AD + COD5AF + COD5AI + COD5AK + COD5AN + COD5AQ + COD5AY + COD5BD + COD5BF + COD5BI 
	    + COD5BK + COD5BN + COD5BQ + COD5BY + COD5CK + COD5DB + COD5DF + COD5DK + COD5DM + COD5EB + COD5EF + COD5EK + COD5EM + COD5EY + COD5FF + COD5FY 
	    + COD5GY + COD5LD + COD5MD + COD5RZ + COD5SZ + COD5UP + COD5UR + COD5UT + COD5UY + COD5VM + COD5VP + COD5VR + COD5VT + COD5VY + COD5WM + COD5WR 
	    + COD5XA + COD5XB + COD5XH + COD5XI + COD5XJ + COD5XN + COD5XO + COD5XP + COD5XR + COD5XS + COD5XY + COD5YA + COD5YB + COD5YH + COD5YI + COD5YJ 
	    + COD5YN + COD5YO + COD5YP + COD5YR + COD5YS + COD5YY + COD5ZA + COD5ZB + COD5ZJ + COD5ZN + COD5ZO + COD5ZS + COD5AH + COD5BH + COD5CM + COD5CN 
	    + COD5CQ + COD5CR + COD5CU + COD5CV + COD5ED + COD5FB + COD5FD + COD5FK + COD5FM + CODCJG + CODCKC + CODCLC + CODCMC + CODCNC + COD5NW + CODCOC 
	    + COD5OW + CODCPC + COD5PW + CODCQC + CODCRC + CODCRF + CODCSC + CODCSF + COD5TP + COD5VQ + COD5VV + COD5XT + COD5XU + COD5ZH + COD5ZI + COD5ZM 
	    + COD5ZP + COD5ZR + COD5ZY + COD5XM + COD5YM + COD5YT + COD5YU + COD5ZT + COD5ZU + CODNGG + CODRBT + COD5HA + COD5IA + COD5JA + COD5UI + COD5VI 
	    + COD5WI + COD5TF + COD5UF + COD5VF + COD5QA + COD5RA + COD5SA + COD5QJ + COD5RJ + COD5SJ + COD2DF + COD1AI + COD1BI + COD1CI + COD1DI + COD1EI 
	    + COD1FI + COD1GG + COD1GH + COD1GK + COD1GL + COD1GP + COD1GQ + COD1GR + COD1GS + COD1HG + COD1HH + COD1HK + COD1HL + COD1HP + COD1HQ + COD1HR 
	    + COD1HS + COD1IG + COD1IH + COD1JG + COD1JH + COD1KG + COD1KH + COD1LG + COD1LH + COD2DF + COD2DG + COD2DI + COD2TQ + COD2TX + COD3AN + COD3BN 
	    + COD3TK + COD5EA + COD5EI + COD5EU + COD5HA + COD5IA + COD5JA + COD5QA + COD5QJ + COD5RA + COD5RJ + COD5SA + COD5SJ + COD5TF + COD5UF + COD5UI 
	    + COD5VF + COD5VI + COD5WI + CODRBE + CODNBE + CODRBK + CODRAI + CODRBI + CODRCK + COD2TY + COD2VS + CODRYY + COD5WE + COD5XE + COD5YE)
  + positif(COD2VT + CODBJS) 
  + positif(COD1AD + COD1AV + COD1BD + COD1BV + COD1CD + COD1CV + COD1DD + COD1DV + COD1ED + COD1EV + COD1FD + COD1FV + COD1PB + COD1PC + COD1PD + COD1PE 
            + COD1PF + COD1PG + COD2VU + CODBKS + CODBLS + COD5AC + COD5AE + COD5AG + COD5AT + COD5AX + COD5BC + COD5BE + COD5BG + COD5BT + COD5BX + COD5CC 
            + COD5CE + COD5CG + COD5CT + COD5CX)
  + positif(COD3CN + CODRSL + COD4EA + COD4EB) 
  + positif(COD3EX + COD3WS + CODBMS)

   + present(ANNUL2042) + present(ASCAPA) + present(AUTOVERSLIB) + present(BRAS) 
   + present(CELREPWT) + present(CELREPWU) + present(CELREPWV) + present(CELREPWW) 
   + present(DUFLOFV)
   + present(PINELRZ) + present(PINELTZ)
   + present(CHENF1) + present(CHENF2) + present(CHENF3) + present(CHENF4) + present(CHNFAC) + present(CHRDED) + present(CHRFAC)
   ...
   + present(IPTXMO) + present(IRANT) + present(NBACT)
   ...
   + present(REPGROREP14) + present(COD6HP) + present(COD6HQ) + present(OPTPLAF15) + present(PAAP) + present(PAAV) 
   ...
   + present(PREMAIDE) + present(PRESCOMP2000) + present(PRESCOMPJUGE) 
   + present(PRODOM) + present(PROGUY) + present(PRSPROV) + present(R1649) + present(PREREV)
   + present(RCCURE) + present(RDCOM) + present(RDDOUP) + present(RDENL) + present(RDENLQAR) + present(RDENS)
   + present(RDENSQAR) + present(RDENU) + present(RDENUQAR) + present(RDDOUP) + present(RDFOREST)
   + present(RDFORESTRA) + present(RDREP) + present(COTFORET) 
   ...
   + present(COD6HR) + present(COD7NY) + present(COD7NX) + present(COD7UH)  
   + present(COD7CY) + present(COD7DY) + present(COD7CX) + present(COD7EY) 
   + present(COD7MX) + present(COD7MY) 
   + present(COD7WK) 
   + present(COD7EN) + present(COD7FY) + present(COD7OP) + present(COD7OQ) + present(COD7OR) + present(COD7OS) + present(COD7OT) 
   + present(COD7TP) + present(COD7TQ) + present(COD7TX) 
   + present(COD7TY) + present(COD7WQ) + present(COD7XX) + present(COD7FW) + present(COD7GY) 
   + present(COD7OY) + present(COD7RB) 
   + present(COD7RD) + present(COD7RT) + present(COD7RU) + present(COD7SA) + present(COD7SB) 
   + present(COD7SC) + present(COD7SU) + present(COD7TR) + present(COD7TS) + present(COD7WH) 
   + present(COD7WI) + present(CODHOD) + present(CODHOE) + present(CODHOF) + present(CODHOG) 
   ...
   + present(CODHUU) 
   + present(CODHVA) 
   + present(CODHVB) + present(CODHVC) + present(CODHVD) + present(CODHVE) + present(CODHVF) + present(CODHVG) 
   + present(CODHJC) 
   + present(CODHKC) + present(CSPROVYN) + present(CSPROVYP)
   ...
   + present(COD7HS) + present(COD7MS) + present(COD7MT) + present(COD7MU) + present(COD7MV) 
   + present(COD7PZ) + present(COD7MZ) 
   + present(COD7RF) + present(COD7RH) 
   + present(COD7SN) + present(COD7SO) + present(COD7TT) + present(COD7TU)   
   ...
   + present(CODHFU) + present(CODHFW) + present(CODHYC) + present(CODHYD)
   + present(COD6DG) 
   + present(COD7DR) 
   + present(COD7FU) + present(COD7GW) + present(COD7HB) 
   ...
   + present(COD7LM) + present(COD7MM) + present(COD7MO) + present(COD7MP) + present(COD7MQ) + present(COD7MR)
   + present(COD7QF) + present(COD7QH) + present(COD7SP) + present(COD7SV) + present(COD7TV)
   + present(COD7TW) + present(COD7XZ) + present(COD7ZI)
   ...
   + present(COD8YX) 
   + present(COD7BS) + present(COD7CS) + present(COD7HD) + present(COD7HE) + present(COD7HF) + present(COD7HG) + present(COD7HH)
   + present(COD7IK) + present(COD7IL) + present(COD7JN) + present(COD7JO) + present(COD7CO)
   + present(COD7JP) + present(COD7JQ) + present(COD7JR) + present(COD7JS) + present(COD7JT) + present(COD7JU) 
   + present(COD7MA) + present(COD7MB) + present(COD7MC) + present(COD7MD)
   + present(COD7MW) + present(COD7NI) + present(COD7NJ) + present(COD7NK) + present(COD7NL) + present(COD7PC)
   + present(COD7PD) + present(COD7PE) + present(COD7QI) + present(COD7QJ) + present(COD7QK) + present(COD7QL) 
   + present(COD7SM) + present(COD7SW) + present(COD7TA) + present(COD7TB) + present(COD7UU)
   + present(COD7UV) + present(COD7UW) + present(COD7UX) + present(COD7VM) + present(COD7VN) 
   + present(COD7XR) 
   + present(COD7ZQ) + present(COD7ZR) + present(COD7ZS) + present(COD7ZT) + present(COD7ZU) + present(COD7ZV) + present(CODHHS) + present(CODHHT)   
   ...
   + present(COD7AA) 
   + present(COD7BK) + present(COD7BL) + present(COD7BM) + present(COD7BN) + present(COD7BO) + present(COD7BT) + present(COD7CA) 
   + present(COD7CT) + present(COD7DC) + present(COD7EK) + present(COD7HZ) 
   + present(COD7KE) + present(COD7KF) + present(COD7KG) + present(COD7KH) + present(COD7KI) 
   + present(COD7KT) + present(COD7KU) + present(COD7KV) + present(COD7KZ) 
   + present(COD7MN) + present(COD7RK) + present(COD7RL) + present(COD7RM) 
   + present(COD7RN) + present(COD7SS) + present(COD7TE) + present(COD7TF) + present(COD7VQ) + present(COD7VR) 
   + present(COD7XG) + present(COD7XV) + present(COD7ZW) + present(COD7ZX) + present(COD7ZY)  
   + present(COD7ZZ) + present(CODHIS) + present(CODHIT) + present(CODHIU) + present(CODHIW) + present(COD7QM) + present(COD7QN) + present(COD7QO) + present(COD7QP) 
   + present(COD7JV) + present(COD7JW) + present(COD7JX) + present(COD7JY) + present(COD7NM) + present(COD7NN) + present(COD7PF) + present(COD7PG) + present(COD7LG) + present(COD7LH) 
   + present(COD7LI) + present(COD7LJ) + present(COD7MI) + present(COD7MJ) + present(COD7MK) + present(COD7ML) 
   + present(COD8SG) + present(COD8WK)
   + present(COD6DR) + present(COD7AK) + present(COD7AL) + present(COD7AM) + present(COD7AN) + present(COD7AO) 
   + present(COD7BP) + present(COD7BR) + present(COD7BU) + present(COD7BV) + present(COD7CG) + present(COD7CH) + present(COD7CP) + present(COD7CI) + present(COD7CQ)
   + present(COD7CJ) + present(COD7CK) + present(COD7CL) + present(COD7CM) + present(COD7CN) + present(COD7CU) + present(COD7CW) + present(COD7DA) + present(COD7DD)
   + present(COD7DE) + present(COD7DF) + present(COD7DH) + present(COD7DJ) + present(COD7DK) + present(COD7DM) + present(COD7DN) + present(COD7GH) + present(COD7GI)
   + present(COD7GJ) + present(COD7IU) + present(COD7IV) + present(COD7IX) + present(COD7IY) + present(COD7IZ) + present(COD7JE) + present(COD7JF) 
   + present(COD7JG) + present(COD7JH) + present(COD7JI) + present(COD7JJ) + present(COD7JK) + present(COD7JL) + present(COD7LK) + present(COD7LL) + present(COD7NS) 
   + present(COD7NT) + present(COD7NU) + present(COD7NV) + present(COD7RR) + present(COD7RS) + present(COD7TI) 
   + present(COD7VD) + present(COD7VE) + present(COD7VF) + present(COD7VG) + present(COD7VS) + present(COD7VU) + present(COD7WA) + present(COD7WB) + present(COD7ZA) 
   + present(COD7ZB) + present(COD7ZC) + present(COD7ZD) + present(COD7ZE) + present(COD7ZF) + present(COD7ZG) + present(COD7ZH) + present(COD8QH) + present(COD8QV) 
   + present(COD8QX) + present(CODBLS) + present(CODHJS) + present(CODHJT) + present(CODHJU) + present(CODHJW) + present(CODHVM) + present(CODHYH) 
   + present(COD8NZ) + present(COD8NA) + present(COD8NB)
   + present(CODAUT) + present(COD7BI) + present(COD7BQ) + present(COD7BW) + present(COD7BX) + present(COD7BY) + present(COD7BZ) + present(COD7CO) + present(COD7CP) + present(COD7CQ) + present(COD7CR)
   + present(COD7DI) + present(COD7DU) + present(COD7DV) + present(COD7DX) + present(COD7DZ) + present(COD7EL) + present(COD7EM) + present(COD7EQ) + present(COD7ER) + present(COD7EV) + present(COD7EW)
   + present(COD7EX) + present(COD7FA) + present(COD7FC) + present(COD7FD) + present(COD7HA) + present(COD7HJ) + present(COD7HK) + present(COD7HN) + present(COD7HY) + present(COD7IA) + present(COD7IB)
   + present(COD7IC) + present(COD7ID) + present(COD7IE) + present(COD7IF) + present(COD7IG) + present(COD7IH) + present(COD7IJ) + present(COD7IM) + present(COD7IN) + present(COD7IO) + present(COD7IP)
   + present(COD7IQ) + present(COD7IW) + present(COD7KA) + present(COD7KB) + present(COD7KK) + present(COD7KX) + present(COD7LQ) + present(COD7LR) + present(COD7LS) + present(COD7LT) + present(COD7NA)
   + present(COD7NB) + present(COD7NC) + present(COD7ND) + present(COD7OF) + present(COD7OG) + present(COD7OJ) + present(COD7OU) + present(COD7OV) + present(COD7OW) + present(COD7PO) + present(COD7PP)
   + present(COD7PQ) + present(COD7PR) + present(COD7PS) + present(COD7RX) + present(COD7RY) + present(COD7SD) + present(COD7SE) + present(COD7SF) + present(COD7SG) + present(COD7SH) + present(COD7SI)
   + present(COD7SJ) + present(COD7SK) + present(COD7SR) + present(COD7SX) + present(COD7SY) + present(COD7SZ) + present(COD7TC) + present(COD7TD) + present(COD7TJ) + present(COD7UA) + present(COD7UB)
   + present(COD7UE) + present(COD7UG) + present(COD7UI) + present(COD7UK) + present(COD7VJ) + present(COD7VK) + present(COD7VL) + present(COD7VO) + present(COD7VV) + present(COD7VW) + present(COD7VX)
   + present(COD7VY) + present(COD7VZ) + present(COD7XA) + present(COD7XB) + present(COD7XC) + present(COD7XD) + present(COD7XE) + present(COD7XF) + present(COD7XL) + present(COD7XM) + present(COD7XN)
   + present(COD7YA) + present(COD7YC) + present(COD7YG) + present(COD7YH) + present(COD7YM) + present(COD7YN) + present(COD7YO) + present(COD7YP)
   + present(CODHKS) + present(CODHKT) + present(CODHKU) + present(CODHKV) + present(CODHKW) + present(CODHVN) + present(CODHYI)
   + present(COD8CV) + present(COD8PZ) + present(COD8VN) + present(COD8VO) + present(COD8XZ) + present(COD8YZ)
...
INDTELEIR = positif(
              positif(4BACREC) + positif(4BACREP) + positif(4BACREV) 
	    + positif(ABDETPLUS) + positif(ALLO1) + positif(ALLO2) + positif(ALLO3) + positif(ALLO4) 
	    + positif(ALLOC) + positif(ALLOV) + positif(AUTOBICPC) 
	    ...
	    + positif(BICDEV) + positif(BICDNC) + positif(BICDNP) + positif(BICDNV) + positif(BICEXC) + positif(BICEXP) + positif(BICEXV)
	    + positif(BICNOC) + positif(BICNOP) + positif(BICNOV) + positif(BICNPEXC) + positif(BICNPEXP)
	    + positif(BICNPEXV) + positif(BICREC) + positif(BICREP) + positif(BICREV) 
	    + positif(BN1AC) + positif(BN1AP) + positif(BN1AV) + positif(BNCAABC) + positif(BNCAABP) + positif(BNCAABV)
	    ...
	    + positif(BNCNPV) + positif(BNCPRO1AC) + positif(BNCPRO1AP) + positif(BNCPRO1AV) + positif(BNCPROC) + positif(BNCPMVCTV) + positif(BNCPMVCTC) + positif(BNCPMVCTP) 
            + positif(BNCPRODEC) + positif(BNCPRODEP) + positif(BNCPRODEV) + positif(BNCPROEXC) + positif(BNCPROEXP) + positif(BNCPROEXV) + positif(BNCPROP) + positif(BNCPROPVC) + positif(BNCPROPVP) 
            + positif(BNCPROPVV) + positif(BNCPROV) + positif(BNCREC) + positif(BNCREP) + positif(BNCREV) 
	    + positif(BPCOPTV) + positif(BPCOSAC) + positif(BPCOSAV) 
	    ...
	    + positif(DEFBIC3) + positif(DEFBIC4) + positif(DEFBIC5) + positif(DEFBIC6) 
	    ...
	    + positif(RCMABD) + positif(RCMAV) + positif(RCMAVFT) + positif(RCMFR) + positif(RCMHAB) + positif(RCMHAD) + positif(RCMLIB) 
	    ...
	    + positif(COD3SL) + positif(COD3UA) + positif(COD3WM) + positif(COD3WI) + positif(COD3WJ) 
	    ...
	    + positif(COD5AF) + positif(COD5AI) + positif(COD5AK) + positif(COD5AN) + positif(COD5AQ)
	    + positif(COD5AY) + positif(COD5BD) + positif(COD5BF) + positif(COD5BI) 
	    + positif(COD5BK) + positif(COD5BN) + positif(COD5BQ) + positif(COD5BY)
	    + positif(COD5CK) + positif(COD5DB) + positif(COD5DF) 
	    + positif(COD5DK) + positif(COD5DM) + positif(COD5EB) + positif(COD5EF)
	    + positif(COD5EK) + positif(COD5EM) + positif(COD5EY) 
	    + positif(COD5FF) + positif(COD5FY) + positif(COD5GY)
	    + positif(COD5LD) + positif(COD5MD) + positif(COD5RZ) + positif(COD5SZ) + positif(COD5UP) 
            + positif(COD5UR) + positif(COD5UT) + positif(COD5UY)
	    + positif(COD5VM) + positif(COD5VP) + positif(COD5VR) + positif(COD5VT) 
	    + positif(COD5VY) + positif(COD5WM) + positif(COD5WR)
	    + positif(COD5XA) + positif(COD5XB) + positif(COD5XH) + positif(COD5XI) 
	    + positif(COD5XJ) + positif(COD5XN) + positif(COD5XO) + positif(COD5XP)
	    + positif(COD5XR) + positif(COD5XS) + positif(COD5XY) 
	    + positif(COD5YA) + positif(COD5YB) + positif(COD5YH) + positif(COD5YI) + positif(COD5YJ)
	    + positif(COD5YN) + positif(COD5YO) + positif(COD5YP)  
	    + positif(COD5YR) + positif(COD5YS) + positif(COD5YY) + positif(COD5ZA)
	    + positif(COD5ZB) + positif(COD5ZJ) + positif(COD5ZN) + positif(COD5ZO) 
	    + positif(COD5ZS) + positif(COD5AH) + positif(COD5BH) + positif(COD5CM) + positif(COD5CN)
	    + positif(COD5CQ) + positif(COD5CR) + positif(COD5CU) + positif(COD5CV)  
	    + positif(COD5ED) + positif(COD5FB) + positif(COD5FD) + positif(COD5FK) + positif(COD5FM)
	    + positif(CODCJG) + positif(CODCKC) + positif(CODCLC)  
            + positif(CODCMC) + positif(CODCNC) + positif(COD5NW)
	    + positif(CODCOC) + positif(COD5OW) + positif(CODCPC) 
	    + positif(COD5PW) + positif(CODCQC) + positif(CODCRC) + positif(CODCRF) 
	    + positif(CODCSC) + positif(CODCSF) + positif(COD5TP) + positif(COD5VQ) 
	    + positif(COD5VV) + positif(COD5XT) + positif(COD5XU) 
	    + positif(COD5ZH) + positif(COD5ZI) + positif(COD5ZM) + positif(COD5ZP) 
	    + positif(COD5ZR) + positif(COD5ZY)  
	    ...
	    + positif(COD1GG) + positif(COD1GH) + positif(COD1GK) + positif(COD1GL) + positif(COD1GP) + positif(COD1GQ)
            ...
            + positif(COD2DF) + positif(COD2DG) + positif(COD2DI) + positif(COD2TQ)
            + positif(COD2TX)
            + positif(COD3AN) + positif(COD3BN) + positif(COD3TK)
            + positif(COD5EA) + positif(COD5EI) + positif(COD5EU) + positif(COD5HA) + positif(COD5IA)
            ...
            + positif(CODRBE) + positif(CODNBE) + positif(CODRBK) 
	    + positif(CODRAI) + positif(CODRBI) + positif(CODRCK) + positif(COD2TY) + positif(COD2VS) + positif(CODRYY) 
	    + positif(COD5WE) + positif(COD5XE) + positif(COD5YE) + positif(COD2VT) + positif(CODBJS)
            + positif(COD1AD + COD1AV + COD1BD + COD1BV + COD1CD + COD1CV + COD1DD + COD1DV + COD1ED + COD1EV + COD1FD + COD1FV + COD1PB + COD1PC + COD1PD 
                      + COD1PE + COD1PF + COD1PG + COD2VU + CODBKS + CODBLS + COD5AC + COD5AE + COD5AG + COD5AT + COD5AX + COD5BC + COD5BE + COD5BG + COD5BT + COD5BX
		      + COD5CC + COD5CE + COD5CG + COD5CT + COD5CX)
            + positif(COD3CN + CODRSL + COD4EA + COD4EB) 
            + positif(COD3EX + COD3WS + CODBMS)

           + positif(ANNUL2042)
           + positif(ASCAPA) + positif(AUTOVERSLIB) + positif(BRAS)
	   + positif(CELREPWT) + positif(CELREPWU) + positif(CELREPWV) + positif(CELREPWW) 
           + positif(DUFLOFV)
           + positif(PINELRZ) + positif(PINELTZ)
	   ...
	   + positif(INVLGDEB2009)
           ...
           + positif(IPTXMO) + positif( IRANT ) + positif(NBACT)
           ...
           + positif( REPGROREP14 ) + positif( COD6HP ) + positif(COD6HQ)
           ...
           + positif( PREMAIDE )
           + positif(PRESCOMP2000) + positif( PRESCOMPJUGE ) 
           + positif( PRODOM ) + positif( PROGUY )
	   ...
	   + positif(RDDOUP) + positif(RDFOREST)
	   + positif( RDFORESTRA ) + positif( RDREP ) + positif( COTFORET )
	   ...
	   + positif(COD6HR)
	   + positif(COD7NY) + positif(COD7NX)
           + positif(COD7UH) + positif(COD7CY)
           + positif(COD7BS) + positif(COD7DY) 
           + positif(COD7CX) + positif(COD7CS)
   	   + positif(COD7EY) + positif(COD7MX) + positif(COD7MY) 
	   + positif(COD7WK) + positif(COD7EN) + positif(COD7FY)
	   + positif(COD7OP) + positif(COD7OQ) + positif(COD7OR) + positif(COD7OT) 
	   + positif(COD7TP) + positif(COD7TQ) + positif(COD7TX)
	   + positif(COD7TY) + positif(COD7WQ) + positif(COD7XX)
	   + positif(COD7FW) + positif(COD7GY) + positif(COD7OS) 
	   + positif(COD7OY) + positif(COD7CO) 
	   + positif(COD7RB) + positif(COD7RD) + positif(COD7RT) + positif(COD7RU)
	   + positif(COD7SA) + positif(COD7SB) + positif(COD7SC) + positif(COD7SU)
	   + positif(COD7TR) + positif(COD7TS) + positif(COD7WH) + positif(COD7WI) 
	   + positif(CODHOD) + positif(CODHOE) + positif(CODHOF) + positif(CODHOG) + positif(CODHOX) + positif(CODHOY) + positif(CODHOZ)
           ...
           + positif(CODHVA)
           + positif(CODHVB) + positif(CODHVC) + positif(CODHVD) + positif(CODHVE) + positif(CODHVF) + positif(CODHVG) 
	   + positif(CODHJC) + positif(CODHKC)
	   ...
	   + positif(COD6NS) + positif(COD6NT) + positif(COD6NU) + positif(COD6OS) + positif(COD6OT) + positif(COD6OU) + positif(COD6NS) + positif(COD6NT)
           ...
           + positif(COD7HR) + positif(COD7HS) + positif(COD7MS) + positif(COD7MT) + positif(COD7MU) + positif(COD7MV) 
           + positif(COD7PZ) + positif(COD7MZ) 
           + positif(COD7RF) + positif(COD7RH)
           + positif(COD7SN) + positif(COD7SO) + positif(COD7TT)
           + positif(COD7TU) + positif(COD7WS)
	   ...
	   + positif(CODHFS) + positif(CODHFT) + positif(CODHFU) + positif(CODHFW) + positif(CODHYC) + positif(CODHYD)
	   + positif(COD6DG)
           + positif(COD7CH)
	   + positif(COD7DR)
	   + positif(COD7FU)
	   + positif(COD7GW) + positif(COD7HB)
	   ...
	   + positif(COD7LM) + positif(COD7MM) + positif(COD7MO) + positif(COD7MP) + positif(COD7MQ) + positif(COD7MR)
	   + positif(COD7QF) + positif(COD7QH) + positif(COD7SP) + positif(COD7SV) + positif(COD7TV)
	   + positif(COD7TW) + positif(COD7XZ) + positif(COD7ZI)
	   ...
	   + positif(COD8YX)
  + COD7HD + COD7HE + COD7HF + COD7HG + COD7HH + COD7IK + COD7IL + COD7JN + COD7JO + COD7JP + COD7JQ + COD7JR + COD7JS + COD7JT + COD7JU 
  + COD7MA + COD7MB + COD7MC + COD7MD + COD7MW + COD7NI + COD7NJ + COD7NK + COD7NL + COD7PC + COD7PD + COD7PE + COD7QI + COD7QJ + COD7QK + COD7QL + COD7SM 
  + COD7SW + COD7TA + COD7TB + COD7UU + COD7UV + COD7UW + COD7UX + COD7VM + COD7VN + COD7XR + COD7ZQ + COD7ZR + COD7ZS + COD7ZT + COD7ZU + COD7ZV + CODHHS + CODHHT 
  + CODHHU + CODHHW + CODHVK + CODHVL + CODHYF + CODHYG + COD8UA + COD8UB + COD8UC + COD8WG + COD8WH + COD8ZQ + COD8ZX
  + COD6EX + COD6EZ + COD6GX + COD6GZ + COD7AA 
  + COD7BK + COD7BL + COD7BM + COD7BN + COD7BO + COD7BT + COD7CA + COD7CT + COD7DC + COD7EK 
  + COD7HZ + COD7KE + COD7KF + COD7KG + COD7KH + COD7KI + COD7KT + COD7KU 
  + COD7KV + COD7KZ + COD7MN + COD7RK + COD7RL + COD7RM + COD7RN + COD7SS + COD7TE + COD7TF + COD7VQ + COD7VR 
  + COD7XG + COD7XV + COD7ZW + COD7ZX + COD7ZY + COD7ZZ 
  + CODHIS + CODHIT + CODHIU + CODHIW + COD7QM + COD7QN + COD7QO + COD7QP + COD7JV + COD7JW + COD7JX + COD7JY 
  + COD7NM + COD7NN + COD7PF + COD7PG + COD7LG + COD7LH + COD7LI + COD7LJ + COD7MI + COD7MJ + COD7MK + COD7ML + COD8SG + COD8WK 
  + COD6DR + COD7AK + COD7AL + COD7AM + COD7AN + COD7AO + COD7BP + COD7BR + COD7BU + COD7BV + COD7CG + COD7CI + COD7CP + COD7CQ + COD7BW
  + COD7CJ + COD7CK + COD7CL + COD7CM + COD7CN + COD7CU + COD7CW + COD7DA + COD7DD + COD7DE + COD7DF + COD7DH + COD7DJ + COD7DK + COD7DM + COD7DN + COD7GH + COD7GI
  + COD7GJ + COD7IU + COD7IV + COD7IX + COD7IY + COD7IZ + COD7JE + COD7JF + COD7JG + COD7JH + COD7JI + COD7JJ + COD7JK + COD7JL + COD7LK  
  + COD7LL + COD7NS + COD7NT + COD7NU + COD7NV + COD7RR + COD7RS + COD7TI + COD7VD + COD7VE + COD7VF + COD7VG + COD7VS + COD7VU + COD7WA + COD7WB + COD7ZA 
  + COD7ZB + COD7ZC + COD7ZD + COD7ZE + COD7ZF + COD7ZG + COD7ZH + COD8QH + COD8QV + COD8QX + CODBLS + CODHJS + CODHJT + CODHJU + CODHJW + CODHVM + CODHYH 
  + CODAUT + COD7BI + COD7BQ + COD7BW + COD7BX + COD7BY + COD7BZ + COD7CO + COD7CP + COD7CQ + COD7CR
  + COD7DI + COD7DU + COD7DV + COD7DX + COD7DZ + COD7EL + COD7EM + COD7EQ + COD7ER + COD7EV + COD7EW
  + COD7EX + COD7FA + COD7FC + COD7FD + COD7HA + COD7HJ + COD7HK + COD7HN + COD7HY + COD7IA + COD7IB
  + COD7IC + COD7ID + COD7IE + COD7IF + COD7IG + COD7IH + COD7IJ + COD7IM + COD7IN + COD7IO + COD7IP
  + COD7IQ + COD7IW + COD7KA + COD7KB + COD7KK + COD7KX + COD7LQ + COD7LR + COD7LS + COD7LT + COD7NA
  + COD7NB + COD7NC + COD7ND + COD7OF + COD7OG + COD7OJ + COD7OU + COD7OV + COD7OW + COD7PO + COD7PP
  + COD7PQ + COD7PR + COD7PS + COD7RX + COD7RY + COD7SD + COD7SE + COD7SF + COD7SG + COD7SH + COD7SI
  + COD7SJ + COD7SK + COD7SR + COD7SX + COD7SY + COD7SZ + COD7TC + COD7TD + COD7TJ + COD7UA + COD7UB
  + COD7UE + COD7UG + COD7UI + COD7UK + COD7VJ + COD7VK + COD7VL + COD7VO + COD7VV + COD7VW + COD7VX
  + COD7VY + COD7VZ + COD7XA + COD7XB + COD7XC + COD7XD + COD7XE + COD7XF + COD7XL + COD7XM + COD7XN
  + COD7YA + COD7YC + COD7YG + COD7YH + COD7YM + COD7YN + COD7YO + COD7YP
  + CODHKS + CODHKT + CODHKU + CODHKV + CODHKW + CODHVN + CODHYI
  + COD8CV + COD8PZ + COD8VN + COD8VO + COD8XZ + COD8YZ
  + 0
 ) ;
regle 221610:
application :  iliad ;
...
INDTEFF = positif(AUTOBICVV + AUTOBICPV + AUTOBICVC + AUTOBICPC + AUTOBICVP + AUTOBICPP + AUTOBNCV + AUTOBNCC + AUTOBNCP 
                  ...
                  + COD1DH + SALEXT3 + COD1EE + COD1EH + SALEXT4 + COD1FE + COD1FH + COD4EA + COD4EB)
...
MAJTX3 = positif(NMAJ3) * (10 * positif(MAJOIR10_2 + MAJOPIR10_2 + MAJOIR07 + MAJOPIR07+MAJOIR07TARDIF_D+MAJOIR02TARDIF_D
                            ...
                            + 20 * positif(MAJOIR17_2 + MAJOPIR17_2 + MAJOIR17_2TARDIF_D+ MAJOIR17_2TARDIF_P
                                                                          + MAJOIR17_2TARDIF_R+ MAJOIR17_2TARDIF_A+ null(CMAJ - 17))) * (1-positif(COD8NZ)) +  positif(COD8NZ) * - 1 ;
...
MAJTX4 =  (positif(positif(MAJOIR03+MAJOIR55+MAJOIR30+FLAG_TRTARDIF * MAJOIR03TARDIF_D)
              +positif(MAJOIR04+MAJOIR35+MAJOIR05+MAJOIR32)+positif(MAJOIR06) +positif(COD8NA+COD8NB)* (1-positif(MAJOTOT35)) * 2 - 1) *-1
              + positif(MAJOIR03+MAJOIR55+MAJOIR30+FLAG_TRTARDIF * MAJOIR03TARDIF_D)* (1-positif(MAJOIR04+MAJOIR35+ MAJOIR05+ MAJOIR06+MAJOIR32)) * (1-positif(COD8NA+COD8NB)) * 40
...
RETXPROV = positif(CMAJ) * TXINT
       + (1-positif(DOTOT))* (
                       TXINR * positif(INRIR_NET+INRCSG_NET+INRRDS_NET+INRPSOL_NET+INRCDIS_NET+INRC820_NET +INRGLO_NET+INRCHR_NET+INRTAXA_NET
                              ...
                              +INCIR_NET +INCCS_NET +INCPSOL_NET +INCRD_NET +INCCDIS_NET +INCCVN_NET +INCC820_NET +INCGLOA_NET +INCRSE1_NET
                              +INCRSE2_NET +INCRSE3_NET +INCRSE4_NET +INCRSE5_NET +INCRSE6_NET +INCRSE8_NET +INCTAXA_NET +INCPCAP_NET +INCCHR_NET)
              *
                   ...
                   + (-1) * positif(INRIR_NET_1+INRCSG_NET_1+INRRDS_NET_1+INRPSOL_NET_1+INRCDIS_NET_1+INRC820_NET_1 +INRGLO_NET_1+INRCHR_NET_1+INRTAXA_NET_1
                                      +INRRSE1_NET_1+INRRSE2_NET_1+INRRSE3_NET_1+INRRSE4_NET_1+INRRSE5_NET_1+INRRSE6_NET_1+ INRRSE8_NET_1+INRTOT_NET1ADEF
                                     )
                       *
                                        positif(INRIR_NET+INRCSG_NET+INRRDS_NET+INRPSOL_NET+INRCDIS_NET+INRC820_NET +INRGLO_NET+INRCHR_NET+INRTAXA_NET
                                       +INRRSE1_NET+INRRSE2_NET+INRRSE3_NET+INRRSE4_NET+INRRSE5_NET+INRRSE6_NET+ INRRSE8_NET+INRTOT_NETADEF
                                               ))
                    + positif(DOTOT)* (
              ...
              + TINR_A * (1-positif(INRIR_NET1ADEF+INRCSG_NET1ADEF+INRRDS_NET1ADEF+INRPSOL_NET1ADEF+INRCDIS_NET1ADEF+INRC820_NET1ADEF
                       ...
                       * positif(INRIR_NETADEF+INRCSG_NETADEF+INRRDS_NETADEF+INRPSOL_NETADEF+INRCDIS_NETADEF+INRC820_NETADEF +INRGLO_NETADEF+INRCHR_NETADEF+INRTAXA_NETADEF
                                       ...
                                       ;
RETX = null(RETXPROV - 0)  * -1
                      * positif(INRIR_NET+INRCSG_NET+INRRDS_NET+INRPSOL_NET+INRCDIS_NET+INRC820_NET +INRGLO_NET+INRCHR_NET+INRTAXA_NET
                                 +INRRSE1_NET+INRRSE2_NET+INRRSE3_NET+INRRSE4_NET+INRRSE5_NET+INRRSE6_NET+ INRRSE8_NET+INRTOT_NETADEF+INRTOT_NET1ADEF)
                + (1-null(RETXPROV - 0)) * RETXPROV;
...
IMPNETIR = NAPTIR - V_ANTIR + V_ANTRE - V_PCAPANT - V_TAXANT - V_CHRANT ;
...
IRCUMBIS = si
               (( (V_ANTIR + IRCUM_A - (IRNET+IRANT) * positif(IRNET+IRANT) - TAXANET - PCAPNET - HAUTREVNET - CDHRNET
	          + (TOTCRA-CSTOT)) > 0 et
                 (V_ANTIR + IRCUM_A - (IRNET+IRANT) * positif(IRNET+IRANT) - TAXANET - PCAPNET - HAUTREVNET  - CDHRNET
		 + (TOTCRA-CSTOT)) < SEUIL_8 )
                 ou
                  ( (TAXANET + PCAPNET + HAUTREVNET + CDHRNET + (IRNET+IRANT) * positif(IRNET+IRANT) - V_ANTIR- IRCUM_A
		  + (CSTOT-TOTCRA)) > 0 et
                    (TAXANET + PCAPNET + HAUTREVNET + CDHRNET + (IRNET+IRANT) * positif(IRNET+IRANT) - V_ANTIR- IRCUM_A 
...
TOTAXAGA = si ((APPLI_COLBERT=0) et  (IRNET - V_ANTIR + TAXANET - V_TAXANT + PCAPNET - V_PCAPANT + HAUTREVNET - V_CHRANT + CDHRNET - V_CDHRANT >= SEUIL_12)
                ou ( (-IRNET + V_ANTIR - TAXANET + V_TAXANT  - PCAPNET + V_PCAPANT - HAUTREVNET + CDHRNET + V_CHRANT + CDHRNET - V_CDHRANT ) >= SEUIL_8) )
                alors(TAXANET * positif(TAXACUM))
                sinon(V_TAXANT * positif(TAXACUM) + 0 )
                finsi;

PCAPTOT = si ((APPLI_COLBERT=0) et ( (IRNET - V_ANTIR + TAXANET - V_TAXANT + PCAPNET - V_PCAPANT + HAUTREVNET - V_CHRANT+ CDHRNET - V_CDHRANT >= SEUIL_12)
                ou ( (-IRNET + V_ANTIR - TAXANET + V_TAXANT - PCAPNET + V_PCAPANT - HAUTREVNET + V_CHRANT+ CDHRNET - V_CDHRANT ) >= SEUIL_8) ))
                alors(PCAPNET * positif(PCAPCUM))
                sinon(V_PCAPANT * positif(PCAPCUM) + 0 )
                finsi;

HAUTREVTOT = si ((APPLI_COLBERT=0) et ( (IRNET - V_ANTIR + TAXANET - V_TAXANT + PCAPNET - V_PCAPANT + HAUTREVNET - V_CHRANT + CDHRNET - V_CDHRANT >= SEUIL_12)
                ou ( (-IRNET + V_ANTIR - TAXANET + V_TAXANT - PCAPNET + V_PCAPANT - HAUTREVNET + V_CHRANT + CDHRNET - V_CDHRANT ) >= SEUIL_8) ))
                alors(HAUTREVNET * positif(HAUTREVCUM))
                sinon(V_CHRANT * positif(HAUTREVCUM) + 0 )
                finsi;

CDHRTOT = si ((APPLI_COLBERT=0) et ( (IRNET - V_ANTIR + TAXANET - V_TAXANT + PCAPNET - V_PCAPANT + HAUTREVNET - V_CHRANT + CDHRNET - V_CDHRANT >= SEUIL_12)
                ou ( (-IRNET + V_ANTIR - TAXANET + V_TAXANT - PCAPNET + V_PCAPANT - HAUTREVNET + V_CHRANT + CDHRNET - V_CDHRANT ) >= SEUIL_8) ))
                alors(CDHRNET * positif(CDHRCUM))
                sinon(V_CDHRANT * positif(CDHRCUM) + 0 )
                finsi;
...
INDSEUIL12IR = positif_ou_nul(IRNET + TAXANET + PCAPNET + HAUTREVNET + CDHRNET - SEUIL_12) ;
...
TOTIRCUM = IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM + AVRICIIR + CIADCREB3 - RASSALIR - RASACOIR - IMPETAL + IMPETAL5 + RASCTXIR ; 

regle 221876:
application : iliad  ;

RECUM = max(0 , -(TOTIRCUM - RECUMIR + NAPCR61 + NAPCRP * (1 - positif(NAPCRP)) + PENAPAS2)) ;

IRPSCUM = max(0 , TOTIRCUM - RECUMIR + NAPCR61 + NAPCRP * (1 - positif(NAPCRP)) + PENAPAS2)  ;
...
CS_DEG = max(0 , TOTCRA - CSTOT * positif_ou_nul(CSTOT - SEUIL_61));
...
NATIMPIR = null(V_IND_TRAIT - 4) 
	   * positif(positif(IRANT) + positif(NAPTIR1) + positif(IRE + NAPTIR1) * (1 - positif_ou_nul(NAPTIR1)))

           + null(V_IND_TRAIT - 5) * (positif(positif(IRANT) + positif(NAPTIR1) + positif(IRE + NAPTIR1) * (1 - positif_ou_nul(NAPTIR1))) * (1 - ANNUL2042)
	                              + ANNUL2042 * positif(null(V_NOTRAIT - 26) + null(V_NOTRAIT - 36) + null(V_NOTRAIT - 46) + null(V_NOTRAIT - 56) + null(V_NOTRAIT - 66))) ;

IITAZIR = (IINETIR - IRESTITIR + COD7HB) * FLAG_BAREM + V_BARIITAZIR * (null(FLAG_BARIITANEG) - positif(FLAG_BARIITANEG)) * (1 - FLAG_BAREM) ; 
...
REVDECTAX = (
 ...
 - BNCNPDEV 
 - BNCNPDEC 
 - BNCNPDEP) * (1-positif(APPLI_COLBERT));
...
REVDECEXO =(
 ...
 + BNCEXV 
 + BNCEXC 
 + BNCEXP ) * (1-positif(APPLI_COLBERT));
...
TXMOYIMPC = arr(TXMOYIMPNUM/TXMOYIMPDEN*100)/100 * (1 - V_CNR);

TXMOYIMP = max(0 , TXMOYIMPC)
           * positif(IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM - RECUMIR + IRANT - NONMER)
           * positif(TX_BAR5 - TXMOYIMPC)  * (1 - V_CNR);

regle 222130:
application :  iliad ;


TXMOYIMPNUM = positif(IRCUM + IRANT + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM
               - max(0 , -(IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM + CDHRCUM - RECUMIR ))
               - PIR - PTAXA - PPCAP - PHAUTREV-PCDHR) * 
              (max(0 , (IRCUM + IRANT + TAXACUM + PCAPCUM + HAUTREVCUM+ CDHRCUM  
               - max(0 , -(IRCUM + TAXACUM + PCAPCUM + HAUTREVCUM - CDHRCUM - RECUMIR ))
	        - PIR - PTAXA - PPCAP - PHAUTREV-PCDHR)
                    * positif_ou_nul((IRNET2 + TAXASSUR + IPCAPTAXT + IHAUTREVT + CHRPVIMP+CDHR) - SEUIL_12) 
                 + (IRNET2 + TAXASSUR + IPCAPTAXT + IHAUTREVT + CHRPVIMP +CDHR+ IRANT)
                    * positif(SEUIL_12 - (IRNET2 + TAXASSUR + IPCAPTAXT + IHAUTREVT + CHRPVIMP+CDHR)) 

                 + arr((RCMLIB) * TX_PREVLIB / 100) + COD2CK * (1-V_CNR) - IPREP-IPPRICORSE
...
TXMOYIMPDEN4 = TTPVQ;
TXMOYIMPDEN5 = RCMLIB ;
TXMOYIMPDEN6 = CESSASSV + CESSASSC + BPCAPTAXV + BPCAPTAXC + BPCAPTAXP + COD5XT + COD5XU;
...
SEUILCIRIRFN1 = positif(V_BTNBP1)*(arr(
              (13048 + (3484 * ((V_BTNBP1/100) - 1) * 2 )
              ) * (1-null(V_REGCO - 5)) * (1-null(V_REGCO - 6))
            + (15437 + (3832 * ( min((V_BTNBP1/100) , 1.5) - 1) * 2)
            + (3484 * ( max(0 , (V_BTNBP1/100) - 1.5)) * 2)
              ) * null(V_REGCO - 5)
            + (16141 + (4006 * ( min((V_BTNBP1/100) , 1.5) - 1) * 2)
            + (3484 * ( max(0 , (V_BTNBP1/100) - 1.5)) * 2)
              ) * null(V_REGCO - 6)
                )) ;															                  
...
SEUILCIRIRF = arr( 
            ...
            + (15437 + (3832 * ( min(NBPT , 1.5) - 1) * 2)
                     + (3484 * ( max(0 , NBPT - 1.5)) * 2)
              ) * null(V_REGCO - 5)
            + (16141 + (4006 * ( min(NBPT , 1.5) - 1) * 2)
            + (3484 * ( max(0 , NBPT - 1.5)) * 2)
              ) * null(V_REGCO - 6) 
                ) ;
...
SEUILCIIMSIN1 =positif(V_BTNBP1)* (arr(
              (17057 + (4555 * ((V_BTNBP1/100) - 1) * 2 )
              ) * (1-null(V_REGCO - 5)) * (1-null(V_REGCO - 6))
            + (18661 + (5007 * ( min((V_BTNBP1/100) , 1.5) - 1) * 2)
            + (4555 * ( max(0 , (V_BTNBP1/100) - 1.5)) * 2)
              ) * null(V_REGCO - 5)
            + (19546 + (5237 * ( min((V_BTNBP1/100) , 1.5) - 1) * 2)
            + (4555 * ( max(0 , (V_BTNBP1/100) - 1.5)) * 2)
            ) * null(V_REGCO - 6)
                 )) ;
...
SEUILCIIMSI = arr(
            ...
            + (18661 + (5007 * ( min(NBPT , 1.5) - 1) * 2)
            + (4555 * ( max(0 , NBPT - 1.5)) * 2)
              ) * null(V_REGCO - 5)
            + (19546 + (5237 * ( min(NBPT , 1.5) - 1) * 2)
            + (4555 * ( max(0 , NBPT - 1.5)) * 2)
              ) * null(V_REGCO - 6)
                ) ;
...
RBGTH = 
 ...
 + COD5XS  
 + COD5XY  
 + COD5YH + COD5YJ + COD5YP + COD5YS + COD5YY   
...
INDEFCAP = positif(present(DEFRCM) + present(DEFRCM2) + present(DEFRCM3) + present(DEFRCM4) + present(DEFRCM5) + present(DEFRCM6) 
                   + present(RFDORD) + present(RFDHIS) + present(RFDANT) 
		   + present(LNPRODEF10) + present(LNPRODEF9) + present(LNPRODEF8) + present(LNPRODEF7) + present(LNPRODEF6) 
		   + present(LNPRODEF5) + present(LNPRODEF4) + present(LNPRODEF3) + present(LNPRODEF2) + present(LNPRODEF1) 
		   + present(BACDEV) + present(DABNCNP6) + present(BACDEC) + present(DABNCNP5) + present(BACDEP) + present(BNCAADV) + present(DABNCNP4)  
		   + present(BICDNV) + present(DABNCNP3) + present(BICDNC) + present(DABNCNP2) + present(BICDNP) + present(DABNCNP1)  
		   + present(BICDEV) + present(LOCDEFNPCGAV) + present(BICDEC) + present(LOCDEFNPCGAC) + present(BICDEP) + present(LOCDEFNPCGAPAC) 
		   + present(BNCDEV) + present(DAGRI6) + present(DAGRI5) + present(DAGRI4) + present(DAGRI3) + present(DAGRI2) + present(DAGRI1) 
		   + present(BNCDEC) + present(BNCAADC) + present(DEFBIC6) + present(DEFBIC5) + present(DEFBIC4) + present(DEFBIC3) + present(DEFBIC2) + present(DEFBIC1) 
		   + present(BNCDEP) + present(BNCAADP) + present(COD5WE) + present(COD5XE) + present(COD5YE)
		   + present(DEFAA5) + present(DEFAA4) + present(DEFAA3) + present(DEFAA2) + present(DEFAA1) + present(DEFAA0)    
		   + present(IPTEFN) + present(DEFZU) + present(DMOND) + 0) ;
...
TOT8NANB = positif((COD8NA * LIM1500 + COD8NB * LIM10000)- (MAJOIRPRO35 + MAJORDPRO35 + MAJOPSOLPRO35 + MAJOCSPRO35 + MAJOTAXAPRO35 + MAJOCAPPRO35 + MAJOHRPRO35 + MAJOCVNPRO35 + MAJOCDISPRO35 + MAJOGLOPRO35 + MAJOC820PRO35
                   + MAJORSE1PRO35+MAJORSE2PRO35+MAJORSE3PRO35+MAJORSE4PRO35+MAJORSE5PRO35+MAJORSE6PRO35+MAJORSE8PRO35))
                   * (COD8NA * LIM1500 + COD8NB * LIM10000) ;


TOT8NA = positif (TOT8NANB) * ( COD8NA * LIM1500 ) ;

TOT8NB =  positif (TOT8NANB) * ( COD8NB * LIM10000 ) ;
