...
BRCM = positif(COD2OP) * (RCMABD + RCMTNC + RCMAV + RCMHAD + RCMHAB + REGPRIV + COD2TT + COD2VV + COD2WW + COD2YY + COD2ZZ + COD2VN + COD2VO + COD2VP + COD2TQ + COD2TZ)
...
SOMDFANTIMPU = DFANTIMPU + TSPRT + RBAT + MIBNETPTOT + BIPN + BICNPF + NPLOCNETF + SPENETPF + BNRTOT + BNCIF + RVTOT + RRCM + RRFI + DESV + ESFP + RE168 + TAX1649 + PREREV + R1649 ;
...
DDCSG = ((V_BTCSGDED * (1-present(DCSG)) + DCSG) 
        + positif(RCMSOC+ COD2DF +0) * (1 - V_CNR)
          ...
          ((positif(COD2OP) * arr(min(RCMSOC + COD2DF , RCMABD + REVACT + RCMAV + PROVIE + RCMHAD + DISQUO + RCMHAB + INTERE + COD2TZ + COD2TT + RCMIMPAT 
	                                                + COD2VV + COD2WW + COD2YY +CODRYY + COD2ZZ + COD2VN + COD2VO + COD2VP ) 
                                  * (T_IDCSG/100))) 
          +((1-positif(COD2OP))* arr(min(COD2DF , RCMAV+ PROVIE + COD2YY + CODRYY + COD2VN )*(T_IDCSG/100))))); 
...
RPALE = max(0,min(somme(i=1..4:min(NCHENFi,LIM_PENSALENF)+min(arr(CHENFi*MAJREV),LIM_PENSALENF))+COD6GZ+COD6EZ,
                RBG1-DDCSG+TOTALQUO-SDDD-R6DRNET)) *(1-V_CNR) ;
...
RNETU = max(0,min(somme(i=1..4:min(CHENFi,LIM_PENSALENF))+COD6GZ,
                RBG1+TOTALQUO-SDDD-R6DRNET-RPALE)) *(1-V_CNR);
...
DFACC = CHRFAC ;
RFACC = max( min(DDFA,RBG1 - RPALE - RPALP  - DDCSG + TOTALQUO - SDDD - R6DRNET) , 0) ;
...
RPALP = max( min(TOTPA,RBG1 - RPALE - DDCSG + TOTALQUO - SDDD - R6DRNET) , 0 ) * (1 - V_CNR) ;
DPALP = PAAV + PAAP ;
...
R6DRNET = max(0,min(COD6DR,RRRBG - RDCSG)) ;
D6DR = COD6DR;
...
RDDIV = max( min(DEDIV * (1 - V_CNR), RBG1 - RPALE - RPALP - RFACC - DDCSG + TOTALQUO - SDDD - R6DRNET ) , 0 ) ;

regle 111555:
application : iliad  ;


DED6DG = (1-positif(RE168+TAX1649)) * COD6DG;
RD6DG = max( min(DED6DG * (1 - V_CNR),RBG1 - RPALE - RPALP - RFACC - DDCSG + TOTALQUO - SDDD - RDDIV - R6DRNET) , 0 ) ;

regle 111560:
application : iliad  ;


NUPROPT = REPGROREP14 + COD6HP + COD6HQ + COD6HR ;

NUREPAR = min(NUPROPT , max(0 , min(PLAF_NUREPAR, RRBG - RPALE - RPALP - RFACC - RDDIV - APERPV - APERPC - APERPP - DDCSG + TOTALQUO - SDDD - R6DRNET - RD6DG))) 
	  * ((V_REGCO + 0) dans (1,3,5,6)) ;

REPNUREPART = max(NUPROPT - NUREPAR , 0) ;
 

REPAR2 = (positif_ou_nul(REPGROREP14 - NUREPAR) * COD6HP
	  + (1 - positif_ou_nul(REPGROREP14 - NUREPAR)) 
              * max(COD6HP + REPGROREP14 - NUREPAR , 0)) * ((V_REGCO+0) dans (1,3,5,6)) ;

REPAR1 = (positif_ou_nul(REPGROREP14 + COD6HP - NUREPAR) * COD6HQ
	  + (1 - positif_ou_nul(REPGROREP14 + COD6HP - NUREPAR)) 
              * max(COD6HQ + COD6HP + REPGROREP14 - NUREPAR , 0)) * ((V_REGCO+0) dans (1,3,5,6)) ;

REPAR = (positif_ou_nul(REPGROREP14 + COD6HP + COD6HQ - NUREPAR) * COD6HR
	  + (1 - positif_ou_nul(REPGROREP14 + COD6HP + COD6HQ - NUREPAR)) 
              * max(COD6HR + COD6HQ + COD6HP + REPGROREP14 - NUREPAR , 0)) * ((V_REGCO+0) dans (1,3,5,6)) ;

REPNUREPAR = REPAR3 + REPAR2 + REPAR1 + REPAR ;

regle 111570:
application : iliad  ;


CHTOT = max(0 , min(COD6DR * V_CNR + (DDPA + DDFA + CHRDED * (1-positif(RE168 + TAX1649)) + DED6DG + APERPV + APERPC + APERPP + NUREPAR) * (1 - V_CNR) , RBG1 - DDCSG + TOTALQUO - SDDD)) ;
...
FRF = somme (i=V,C,1,2,3,4: FRDi * (1-IND_10i))*(1-positif(APPLI_COLBERT)) ;
...
INDM13 = positif((IREST-LIM_INFRESTIT)*(LIM_RESTIT-IREST))
         * positif_ou_nul((IPRECH + COD8TE + COD8TL + COD8CV + CREFAM + COD8WK + CREAGRIBIO + PRESINTER + CREFORMCHENT + CREARTS + CRECONGAGRI + AUTOVERSLIB)-IREST)
...
AVRICIPROV = positif(null(1-V_REGCO)+null(5-V_REGCO)+null(6-V_REGCO)+null(3-V_REGCO)*null(0-V_INDVB31)) * 
			   ...
			   + RHEBE + RDUFREP + RPINELTOT + RNORMAN + RNORMREP
	                   + RCELTOT 
                       + RILMNP1 + RILMNP3 + RILMNP4 + RLOCANAH +  RREPA + RDONDJ + RDONDO + RDONS + CISYND + RLOGHVJ + RLOGHVK + RLOGHVL + RLOGHVM + RLOGHVN
          ...
          + positif(null(2-V_REGCO)+null(3-V_REGCO)*null(1-V_INDVB31)) 
				 ...
				 + RPIJV + RPIJW + RPIJX + RPIJY + RPIJI + RPIJJ + RPIJK + RPIJL + RPIIA + RPIIB + RPIIC + RPIID
				 ...
				 + RNOJE + RNOJF + RNOJG + RNOJH + RNOIE + RNOIF + RNOIG + RNOIH
				 + RLOCANAH);
