...
/**
 * MODE CONNECTÉ (Lot 4) : ouvert depuis une fiche client. Mêmes champs, PRÉ-REMPLIS
 * priorité Découverte RDV > Simulation. Particularité : ICI tout reste éditable (y
 * compris revenu et situation familiale) — le pré-remplissage n'est qu'un point de
 * départ. Code client en évidence (pas le nom). N'écrit JAMAIS dans Pipedrive.
 */
export default async function SimulateurImpotConnectePage({
  params,
}: {
  ...
  let prefill: Partial<ImpotInputs> = {};
  ...
  return (
    <main className="min-h-screen">
      ...
      <ImpotSim prefill={prefill} client={{ personId, code }} />
    </main>
  );
}
